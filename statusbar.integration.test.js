...
describe('Status Bar Integration Tests', () => {
    ...
    describe('Status Bar Presence', () => {
        it('should have status bar item visible after extension activation', async () => {
            ...
            const hasRecordingCommands = allCommands.some(cmd => cmd.includes('speechToTextWhisper.record'));
                ...
                Object.keys(menus).forEach(menuType => {
                    ...
                    if (Array.isArray(menuItems)) {
                        menuItems.forEach(item => {
                            if (item.when) {
                                assert.ok(typeof item.when === 'string', `When clause should be a string: ${item.when}`);
                            }
                        });
                    }
                });
    ...
    describe('Status Bar Commands Integration', () => {
        it('should have status bar related commands registered', async () => {
            ...
            for (const commandId of statusBarRelatedCommands) {
                assert.ok(allCommands.includes(commandId), `Status bar command ${commandId} should be registered`);
            }
        });
        it('should handle status bar command execution gracefully', async () => {
            ...
            for (const commandId of statusBarCommands) {
                try {
                    ...
                    const isExpectedError = expectedErrors.some(pattern => errorMessage.includes(pattern));
                    if (isExpectedError) {
    ...
    describe('Status Bar Visual Feedback', () => {
        it('should have proper icons configured in package.json', () => {
            ...
            const recordingCommands = commands.filter((cmd) => cmd.command.includes('record'));
            assert.ok(recordingCommands.length > 0, 'Should have recording commands');
            recordingCommands.forEach((cmd) => {
                ...
                if (cmd.icon) {
                    assert.ok(typeof cmd.icon === 'string' || typeof cmd.icon === 'object', `Command ${cmd.command} icon should be string or object`);
                }
            });
        });
    ...
    describe('Status Bar Error Handling', () => {
        it('should handle status bar creation errors gracefully', async () => {
            ...
            try {
                ...
                assert.ok(!errorMessage.includes('status bar') && !errorMessage.includes('statusbar'), 'Error should not be related to status bar');
    ...
    describe('Status Bar Performance', () => {
        it('should update status bar efficiently', async function () {
            ...
            try {
                ...
                console.log('Toggle mode failed as expected:', error.message);
            }
            const executionTime = Date.now() - startTime;
            // Status bar update should be fast
            assert.ok(executionTime < 1000, `Status bar update should be fast (took ${executionTime}ms)`);
        });
    });
});
//# sourceMappingURL=statusbar.integration.test.js.map