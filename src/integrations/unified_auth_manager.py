    ...
    async def rotate_keys(self, key_name: str) -> bool:
        ...
        try:
            ...
            if old_key.exists():
                # Archive old keys
                archive_dir = (
                    self.key_dir / "archive" / datetime.now().strftime("%Y%m%d_%H%M%S")
                )
...
class SSHTunnelManager:
    ...
    async def create_tunnel(
        ...
        try:
            ...
            if key_path:
                ssh_client.connect(
                    ssh_host, port=ssh_port, username=ssh_user, key_filename=key_path
                )
            else:
            ...
            self.logger.info(
                f"🔌 Tunnel created: {tunnel_name} (localhost:{local_port} -> {remote_host}:{remote_port})"
            )
            return True
...
class UnifiedAuthService:
    ...
    async def register_credential(
        ...
        try:
            ...
            self.logger.info(
                f"✅ Registered credential for {username} ({auth_method.value})"
            )
    ...
    async def authenticate(
        ...
        try:
            ...
            self.logger.info(
                f"✅ Authenticated {username}, issued token {token.token_id[:16]}..."
            )
            return token
...
async def main():
    ...
    await auth.register_credential(
        username="m2ultra", auth_method=AuthMethod.SSH_KEY, secret=priv_key
    )
