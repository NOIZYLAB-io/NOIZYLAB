...
class Metric:
    ...
    def add_sample(self, value: float, labels: Dict[str, str] = None) -> None:
        ...
        self.samples.append(
            MetricSample(timestamp=datetime.now(), value=value, labels=labels or {})
        )
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "throughput": {
                ...
                "calls_per_second": (
                    self.request_count
                    / max(1, (datetime.now() - self.first_call_time).total_seconds())
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "upload_times_ms": {
                ...
                "mean": (
                    sum(self.upload_times_ms) / len(self.upload_times_ms)
                    if self.upload_times_ms
                    else 0
                ),
                "p95": (
                    sorted(self.upload_times_ms)[int(len(self.upload_times_ms) * 0.95)]
                    if self.upload_times_ms
                    else 0
                ),
            },
            "download_times_ms": {
                ...
                "mean": (
                    sum(self.download_times_ms) / len(self.download_times_ms)
                    if self.download_times_ms
                    else 0
                ),
                "p95": (
                    sorted(self.download_times_ms)[
                        int(len(self.download_times_ms) * 0.95)
                    ]
                    if self.download_times_ms
                    else 0
                ),
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "disk": {
                mount: {
                    "percent": (
                        list(self.disk_percent[mount])[-1]
                        if self.disk_percent[mount]
                        else 0
                    ),
                    "free_bytes": (
                        list(self.disk_bytes[mount])[-1]
                        if self.disk_bytes[mount]
                        else 0
                    ),
                }
            ...
            "network": {
                "bytes_in": (
                    list(self.network_bytes_in)[-1] if self.network_bytes_in else 0
                ),
                "bytes_out": (
                    list(self.network_bytes_out)[-1] if self.network_bytes_out else 0
                ),
            },
...
class UnifiedMetricsCollector:
    ...
    def get_file_sync_metrics(
        self, local_path: str, remote_host: str
    ) -> FileSyncMetrics:
    ...
    def get_file_sync_summary(self) -> Dict:
        ...
        return {
            key: metrics.get_stats() for key, metrics in self.file_sync_metrics.items()
        }
    ...
    def get_cluster_health(self) -> Dict:
        ...
        return {
            ...
            "nodes": {
                node_name: {
                    "healthy": metrics.last_update
                    and (datetime.now() - metrics.last_update).total_seconds() < 30,
                    "cpu": metrics.cpu_percent[-1] if metrics.cpu_percent else 0,
                    "memory_gb": (
                        metrics.memory_bytes[-1] if metrics.memory_bytes else 0
                    )
                    / (1024**3),
                }
                for node_name, metrics in self.system_metrics.items()
            ...
            "grpc": {
                ...
                "error_rate": sum(m.error_count for m in self.grpc_metrics.values())
                / max(1, sum(m.request_count for m in self.grpc_metrics.values())),
            },
            "file_sync": {
                "total_files": sum(
                    m.files_uploaded + m.files_downloaded
                    for m in self.file_sync_metrics.values()
                ),
                "total_bytes": sum(
                ...
                "conflicts_pending": sum(
                    m.conflicts_pending for m in self.file_sync_metrics.values()
                ),
            },
        }
    ...
    def export_prometheus(self) -> str:
        ...
        for method_name, metrics in self.grpc_metrics.items():
            lines.append(
                f'grpc_calls_total{{method="{method_name}"}} {metrics.request_count}'
            )
            lines.append(
                f'grpc_errors_total{{method="{method_name}"}} {metrics.error_count}'
            )

            if metrics.latencies_ms:
                latencies = list(metrics.latencies_ms)
                _mean = sum(latencies) / len(latencies)
                lines.append(
                    f'grpc_latency_ms{{method="{method_name}",quantile="0.5"}} {sorted(latencies)[len(latencies)//2]}'
                )
                lines.append(
                    f'grpc_latency_ms{{method="{method_name}",quantile="0.95"}} {sorted(latencies)[int(len(latencies)*0.95)]}'
                )
                lines.append(
                    f'grpc_latency_ms{{method="{method_name}",quantile="0.99"}} {sorted(latencies)[int(len(latencies)*0.99)]}'
                )
        ...
        for node_name, metrics in self.system_metrics.items():
            if metrics.cpu_percent:
                lines.append(
                    f'system_cpu_percent{{node="{node_name}"}} {list(metrics.cpu_percent)[-1]}'
                )
            if metrics.memory_percent:
                lines.append(
                    f'system_memory_percent{{node="{node_name}"}} {list(metrics.memory_percent)[-1]}'
                )
...
class BandwidthThrottler:
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "avg_transfer_bytes": (
                sum(t["bytes"] for t in self.transfer_history)
                / len(self.transfer_history)
                if self.transfer_history
                else 0
            ),
            "avg_wait_ms": (
                sum(t["wait_ms"] for t in self.transfer_history)
                / len(self.transfer_history)
                if self.transfer_history
                else 0
            ),
