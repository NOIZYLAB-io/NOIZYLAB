...
class InputInjectionEngine:
    ...
    async def inject_mouse(
        ...
        try:
            ...
            self.logger.debug(
                f"🖱️  Mouse ({x}, {y}) button {button} ({'down' if is_down else 'up'})"
            )
            return True
...
class UnifiedRemoteDisplay:
    ...
    async def start_streaming(
        ...
        try:
            ...
            self.logger.info(
                f"🎬 Streaming started [ID: {self.session_id}] ({display_id}, {frame_rate} FPS, {bitrate_kbps} kbps)"
            )
    ...
    async def stop_streaming(self) -> None:
        ...
        if self.stream_stats["frames_sent"] > 0:
            ...
            self.logger.info(
                f"📊 Stream stats: {fps:.1f} FPS, {mbps:.2f} Mbps, {self.stream_stats['frames_dropped']} dropped"
            )
            ...
            try:
                ...
                if frame:
                    ...
                    if frame.bitrate_kbps > self.bitrate_target_kbps * 1.2:
                        ...
                        self.logger.debug(
                            f"⬇️  Frame dropped (bitrate {frame.bitrate_kbps} > {self.bitrate_target_kbps})"
                        )
                    else:
