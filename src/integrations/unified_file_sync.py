...
class SFTPSyncEngine:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"🔌 Connecting to {self.config.hp_host}:{self.config.hp_sftp_port}..."
            )
    ...
    async def sync_file_to_remote(self, change: FileChange) -> bool:
        ...
        try:
            ...
            if change.change_type == FileChangeType.DELETED:
                ...
                self.logger.info(
                    f"⬆️  Synced to remote: {remote_path} ({change.size} bytes)"
                )
        ...
        while self.sync_enabled:
            try:
                ...
                if current_content != self.last_clipboard_content:
                    self.logger.info(
                        f"📋 Clipboard changed ({len(current_content)} bytes)"
                    )
...
async def main():
    ...
    try:
        ...
        while True:
            ...
            print(
                f"✅ Sync Status: {status['files_synced']} files, {status['conflicts_resolved']} conflicts"
            )
