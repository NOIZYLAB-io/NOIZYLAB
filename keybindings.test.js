...
describe('Keybindings Integration Tests', () => {
    ...
    describe('Expected Keybindings', () => {
        ...
        it('should have all expected keybindings defined in package.json', () => {
            ...
            for (const expectedBinding of expectedKeybindings) {
                const binding = keybindings.find((kb) => kb.command === expectedBinding.command);
                assert.ok(binding, `Keybinding for ${expectedBinding.command} should be defined`);
                if (expectedBinding.key) {
                    assert.strictEqual(binding.key, expectedBinding.key, `Key for ${expectedBinding.command} should be ${expectedBinding.key}`);
                }
                if (expectedBinding.mac) {
                    assert.strictEqual(binding.mac, expectedBinding.mac, `Mac key for ${expectedBinding.command} should be ${expectedBinding.mac}`);
                }
            }
        });
        it('should have commands registered for all keybindings', async () => {
            ...
            for (const binding of expectedKeybindings) {
                assert.ok(allCommands.includes(binding.command), `Command ${binding.command} should be registered for keybinding`);
            }
        });
    });
    ...
    describe('Platform-specific Keybindings', () => {
        it('should handle platform-specific key combinations', () => {
            ...
            const macBindings = keybindings.filter((kb) => kb.mac);
            ...
            for (const binding of macBindings) {
                if (binding.mac) {
                    assert.ok(binding.mac.includes('cmd'), `Mac keybinding ${binding.mac} should use cmd modifier`);
                }
            }
        });
        it('should have consistent key patterns', () => {
            ...
            for (const binding of keybindings) {
                if (binding.key && binding.key.includes('shift')) {
                    assert.ok(binding.key.includes('ctrl') || binding.key.includes('cmd'), `Keybinding ${binding.key} with shift should also have ctrl/cmd modifier`);
                }
            }
        });
    });
    describe('Keybinding Conflicts', () => {
        it('should not conflict with common VS Code keybindings', () => {
            ...
            for (const binding of keybindings) {
                ...
                if (key) {
                    assert.ok(!commonVSCodeKeys.includes(key), `Keybinding ${binding.key} should not conflict with common VS Code shortcuts`);
                }
            }
        });
        it('should use unique key combinations', () => {
            ...
            for (const binding of keybindings) {
                if (binding.key) {
                    assert.ok(!usedKeys.has(binding.key), `Key combination ${binding.key} should be unique`);
                    usedKeys.add(binding.key);
                }
                if (binding.mac) {
                    assert.ok(!usedKeys.has(binding.mac), `Mac key combination ${binding.mac} should be unique`);
