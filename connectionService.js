...
class ConnectionService {
    ...
    defaultSqlRetryTimeoutInSec = 10;
    defaultSqlNumberOfRetries = 3;
    /**
     ...
     * @returns
     */
    async connectToDatabase(profile, saveConnectionAndPassword, database) {
     ...
     */
    async formatConnectionResult(connection) {
        const azdataApi = utils.getAzdataApi();
        const connectionResult = connection !== undefined && azdataApi ? connection : undefined;
        return connectionResult?.connected ? connectionResult.connectionId : connection;
    }
    /**
     ...
     */
    async getConnection(profile, saveConnectionAndPassword, database) {
        const azdataApi = utils.getAzdataApi();
        let connection = await utils.retry(constants.connectingToSqlServerMessage, async () => {
            return await this.connectToDatabase(profile, saveConnectionAndPassword, database);
        }, this.validateConnection, this.formatConnectionResult, this._outputChannel, this.defaultSqlNumberOfRetries, profile.connectionRetryTimeout || this.defaultSqlRetryTimeoutInSec);
        if (connection) {
            const connectionResult = connection;
            if (azdataApi) {
