...
Features:
  - Multi-drive scanning for all XLN Audio content
...
@dataclass
class Config:
    """Master configuration for XLN Audio library management"""
    ...
    MASTER_LIBRARY: str = "/Volumes/SAMPLE_MASTER/06_Drums_Percussion/XLN_Audio_Master"
    
    # XLN Audio file patterns
    XLN_EXTENSIONS: Set[str] = field(default_factory=lambda: {
        '.xpak',  # XLN Audio sound packs
        '.adpak',  # Addictive Drums packs
        '.akpak',  # Addictive Keys packs
        '.xlnlib',  # XLN Library files
        '.midi', '.mid',  # MIDI files
        '.wav', '.aif', '.aiff',  # Audio samples
        '.xml', '.dat', '.db',  # Databases
        '.preset', '.adpreset',  # Presets
    })
    
    # Folder patterns to identify XLN content
    FOLDER_PATTERNS: List[str] = field(default_factory=lambda: [
        'xln', 'addictive', 'addrums', 'addkeys', 'xo_', 'trigger'
    ])
    
    # Product identification
    PRODUCTS: Dict[str, List[str]] = field(default_factory=lambda: {
        'Addictive_Drums': ['addictive_drums', 'addrums', 'ad2', 'attp'],
        'Addictive_Drums_2': ['addictive_drums_2', 'ad2_'],
        'Addictive_Keys': ['addictive_keys', 'addkeys', 'ak_'],
        'Addictive_Trigger': ['addictive_trigger', 'adtrigger'],
        'XO': ['xo_', 'xo-'],
    })
    
    # Category mappings for organization
    CATEGORY_MAP: Dict[str, str] = field(default_factory=lambda: {
        'addictive_drums': 'Addictive_Drums',
        'addictive_drums_2': 'Addictive_Drums_2',
        'addictive_keys': 'Addictive_Keys',
        'addictive_trigger': 'Addictive_Trigger',
        'xo': 'XO',
        'xpak': 'Sound_Packs',
        'midi': 'MIDI_Grooves',
        'preset': 'Presets',
    ...
    # Report output
    REPORT_DIR: str = "/Users/m2ultra/NOIZYLAB/CODE_MASTER/XLN_Audio/reports"
    
    # Cache file
    CACHE_FILE: str = "/Users/m2ultra/NOIZYLAB/CODE_MASTER/XLN_Audio/.scan_cache.json"
...
@dataclass
class XLNItem:
    """Represents a single XLN Audio file or folder"""
    path: str
    name: str
...
class ScanResult:
    ...
    files: List[XLNItem] = field(default_factory=list)
    folders: List[XLNItem] = field(default_factory=list)
...
class XLNScanner:
    """Multi-drive scanner for XLN Audio content"""
    ...
    def scan(self, quick: bool = False) -> ScanResult:
        ...
        print("\n" + "═" * 70)
        print("   🔍 XLN AUDIO SCANNER - Starting Multi-Volume Scan")
        print("═" * 70)
    ...
    def _scan_volume(self, volume: str, quick: bool = False):
        """Scan a single volume for XLN Audio content"""
        xln_paths = self._find_xln_paths(volume)
        
        for path in xln_paths:
            self._process_path(path, quick)
            
    def _find_xln_paths(self, volume: str) -> List[str]:
        """Find all XLN Audio-related paths in a volume"""
        paths = []
        ...
        search_dirs = [
            ...
            os.path.join(volume, "XLN_Audio"),
            os.path.join(volume, "XLN Audio"),
            volume,
        ]
        ...
        for search_dir in search_dirs:
            ...
            try:
                for entry in os.scandir(search_dir):
                    ...
                    # Also check for numbered drum library folders
                    if '01_drums' in name_lower or '03_drum' in name_lower:
                        # Check inside for XLN
                        if os.path.isdir(entry.path):
                            for sub in os.scandir(entry.path):
                                if 'xln' in sub.name.lower():
    ...
    def _process_path(self, path: str, quick: bool = False):
        """Process an XLN Audio path and catalog its contents"""
    ...
    def _process_file(self, file_path: str, quick: bool = False) -> Optional[XLNItem]:
    ...
    def _identify_product(self, path: str) -> str:
        """Identify which XLN product this belongs to"""
...
class IntegrityTester:
    ...
    def _test_item(self, item: XLNItem) -> List[Dict]:
        ...
        if item.size == 0 and item.extension in {'.xpak', '.adpak', '.wav', '.mid'}:
            issues.append({
                'path': item.path,
                'type': 'empty',
        ...
        # XLN-specific: Check .xpak file headers
        if item.extension == '.xpak' and item.size > 0:
            try:
                with open(item.path, 'rb') as f:
                    header = f.read(4)
                    # XLN xpak files should have specific headers
                    if header[:2] != b'PK' and header[:4] != b'XPAK':
                        # Could be proprietary format, just log
                        pass
            except:
                pass
...
class XLNHealer:
    ...
    def _heal_structure(self, scan_result: ScanResult, dry_run: bool):
        ...
        required_dirs = [
            master_lib / "Addictive_Drums",
            master_lib / "Addictive_Drums_2",
            master_lib / "Addictive_Keys",
            master_lib / "Addictive_Trigger",
            master_lib / "XO",
            master_lib / "Sound_Packs",
...
class DeepScanner:
    ...
    def deep_scan(self, scan_result: ScanResult) -> Dict:
        ...
        analysis = {
            ...
            'xpak_analysis': self._analyze_xpak_files(scan_result),
    ...
    def _generate_summary(self, result: ScanResult) -> Dict:
        ...
        return {
            ...
            'products_count': len(result.products_found),
        }
    
    def _analyze_by_product(self, result: ScanResult) -> Dict:
        """Analyze content by XLN product"""
    ...
    def _analyze_xpak_files(self, result: ScanResult) -> Dict:
        """Analyze .xpak files specifically"""
        xpak_files = [f for f in result.files if f.extension == '.xpak']
        
        return {
            'total_xpak': len(xpak_files),
            'total_size_gb': sum(f.size for f in xpak_files) / (1024**3),
            'by_type': self._categorize_xpak(xpak_files),
        }
    
    def _categorize_xpak(self, xpak_files: List[XLNItem]) -> Dict:
        """Categorize xpak files by type"""
        categories = defaultdict(int)
        
        for f in xpak_files:
            name_lower = f.name.lower()
            if 'preview' in name_lower:
                categories['Preview Packs'] += 1
            elif 'sound' in name_lower:
                categories['Sound Packs'] += 1
            elif 'midi' in name_lower:
                categories['MIDI Packs'] += 1
            elif 'gui' in name_lower:
                categories['GUI Data'] += 1
            elif 'resource' in name_lower:
                categories['Resources'] += 1
            elif 'base' in name_lower or 'content' in name_lower:
                categories['Base Content'] += 1
            else:
                categories['Other'] += 1
    ...
    def _find_largest(self, result: ScanResult, limit: int = 10) -> List[Dict]:
        ...
        return [
            {'path': f.path, 'size_gb': f.size / (1024**3), 'product': f.product}
    ...
    def _generate_recommendations(self, result: ScanResult) -> List[str]:
        ...
        if len(volumes) > 2:
            recs.append("Consider consolidating XLN content to fewer drives")
        
        # Check for orphaned previews
        preview_files = [f for f in result.files if 'preview' in f.name.lower()]
        if len(preview_files) > 20:
            recs.append(f"Found {len(preview_files)} preview files - consider organizing")
    ...
    def _print_analysis(self, analysis: Dict):
        ...
        print(f"   Products Found: {summary['products_count']}")
        ...
        print("\n   📦 XPAK ANALYSIS")
        print("   " + "-" * 40)
        xpak = analysis['xpak_analysis']
        print(f"   Total .xpak files: {xpak['total_xpak']}")
        print(f"   Total size: {xpak['total_size_gb']:.2f} GB")
        for cat, count in xpak['by_type'].items():
            print(f"     - {cat}: {count}")
...
class XLNOrganizer:
    ...
    def _normalize_name(self, name: str) -> str:
        ...
        name = re.sub(r'^(AD[2]?_|AK_|XLN_)', '', name)
...
class ReportGenerator:
    ...
    def generate(self, 
        ...
        report_path = report_dir / f"xln_audio_report_{timestamp}.md"
        ...
        # Also save JSON data
        json_path = report_dir / f"xln_audio_data_{timestamp}.json"
    ...
    def _build_report(self, scan_result, test_results, deep_analysis) -> str:
        ...
        if deep_analysis:
            lines.extend(["", "## 📁 Content by Product", ""])
            lines.extend([
            ...
            for prod, data in sorted(
                deep_analysis.get('by_product', {}).items(),
                key=lambda x: x[1]['size'],
                reverse=True
            ):
                size_gb = data['size'] / (1024**3)
                lines.append(f"| {prod} | {data['count']} | {size_gb:.2f} |")
...
║                                                                              ║
║       🎹 ADDICTIVE DRUMS • ADDICTIVE KEYS • XO • TRIGGER 🎹                  ║
║                  MASTER LIBRARY MANAGEMENT SYSTEM                            ║
║                                                                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
    """
    print(banner)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="XLN Audio Master Library Management"
    )
    
    subparsers = parser.add_subparsers(dest='command', help='Commands')
    
    # Scan command
    scan_parser = subparsers.add_parser('scan', help='Scan for XLN Audio content')
