...
exports.MusicTimeWebviewSidebar = void 0;
const vscode_1 = require("vscode");
const Util_1 = require("../Util");
const _404_1 = require("../local/404");
const Loading_1 = require("../local/Loading");
...
class MusicTimeWebviewSidebar {
    ...
    refresh(reloadData_1) {
        return __awaiter(this, arguments, void 0, function* (reloadData, refreshOpenFolder = false) {
            if (!this._webview) {
            ...
            if (!this._origHtml || reloadData) {
                ...
                this._webview.webview.html = yield this.buildPlaylistItems(this._origHtml, refreshOpenFolder);
            }
        });
    }
    get onDidClose() {
    ...
    loadWebview() {
        return __awaiter(this, arguments, void 0, function* (tries = 10) {
            const musicInitialized = MusicCommandManager_1.MusicCommandManager.isInitialized();
            // make sure the jwt is available. The session info may have
            // been removed while this view was open.
            if (((0, Util_1.getItem)("jwt") && musicInitialized) || tries <= 0) {
                this._webview.webview.html = yield this.getHtml();
            }
            else {
                tries--;
    ...
    getHtml() {
        return __awaiter(this, void 0, void 0, function* () {
            if ((0, SpotifyManager_1.hasSpotifyUser)() && !this._origHtml) {
                this._webview.webview.html = yield (0, Loading_1.getLoadingHtml)();
            }
            const params = {
                nav_view: (0, PlaylistDataManager_1.getSelectedTabView)()
            };
            const resp = yield (0, HttpClient_1.appGet)('/plugin/sidebar', params);
            if ((0, HttpClient_1.isResponseOk)(resp)) {
                this._origHtml = resp.data;
                return yield this.buildPlaylistItems(this._origHtml);
            }
            return yield (0, _404_1.getConnectionErrorHtml)();
        });
    }
    buildPlaylistItems(html_1) {
        return __awaiter(this, arguments, void 0, function* (html, refreshOpenFolder = false) {
            var _a, _b, _c;
            const spotifyUser = yield (0, SpotifyManager_1.getConnectedSpotifyUser)();
            const selectedTabView = (0, PlaylistDataManager_1.getSelectedTabView)();
            const playlistId = (0, PlaylistDataManager_1.getSelectedPlaylistId)();
            const data = yield this.getViewData(selectedTabView, playlistId, spotifyUser);
            let sidebarContent = '';
            if (selectedTabView === 'playlists' && ((_a = data === null || data === void 0 ? void 0 : data.spotifyPlaylists) === null || _a === void 0 ? void 0 : _a.length)) {
                let tracks = [];
                if (playlistId) {
                    if (playlistId === Constants_1.SPOTIFY_LIKED_SONGS_PLAYLIST_ID) {
                ...
                const playlistFolders = data.spotifyPlaylists.map((item) => this.buildPlaylistItem(item, playlistId, tracks, refreshOpenFolder)).join('\n');
                sidebarContent = yield this.buildPlaylistSidebar(likedFolder, playlistFolders, data.playerContext);
            }
            else if (selectedTabView === 'recommendations' && ((_c = (_b = data.recommendationInfo) === null || _b === void 0 ? void 0 : _b.tracks) === null || _c === void 0 ? void 0 : _c.length)) {
                sidebarContent = yield this.buildRecommendationSidebar(data.recommendationInfo, data.playerContext);
            }
    ...
    buildPlaylistSidebar(likedFolder, playlistFolders, playerContext) {
        return __awaiter(this, void 0, void 0, function* () {
      ...
      <div class="flex flex-col w-full pb-2">
        <div class="flex justify-between items-center space-x-2 py-3">
          ...
          <div class="flex items-center space-x-2">
            ...
            ${this.getSortButton()}
            ${yield this.getTrackControlButton(playerContext)}
          </div>
        </div>
        ${likedFolder}
      </div>
    ...
    buildRecommendationSidebar(recommendationInfo, playerContext) {
        return __awaiter(this, void 0, void 0, function* () {
    ...
    buildPlaylistItem(item, playlistId, tracks, refreshOpenFolder = false) {
        let chevronSvg = this.getChevronRight();
        let tracksHtml = '';
        if (item.id === playlistId) {
            if (((0, PlaylistDataManager_1.getExpandedPlaylistId)() !== playlistId) || (0, PlaylistDataManager_1.getExpandedPlaylistId)() === playlistId && refreshOpenFolder) {
                // expand or refresh
                (0, PlaylistDataManager_1.updateExpandedPlaylistId)(playlistId);
                chevronSvg = this.getChevronDown();
                if (tracks === null || tracks === void 0 ? void 0 : tracks.length) {
                    tracksHtml = [
                        '<div class="pl-2 -m-1">',
                        ...tracks.map((item) => this.buildTrackItem(item, playlistId)),
                        '</div>'
                    ].join('\n');
                }
            }
                ...
                // clear it
                (0, PlaylistDataManager_1.updateExpandedPlaylistId)('');
            }
        }
        return `
      <div class="flex flex-col">
    ...
    buildRecommendationTracks(recommendationInfo) {
        return [
            '<div>',
            ...recommendationInfo.tracks.map((item) => this.buildRecommendationTrackItem(item)),
            '</div>'
        ].join('\n');
    }
    buildTrackItem(track, playlistId = '') {
    ...
    buildRecommendationTrackItem(track) {
        const playlistId = Constants_1.RECOMMENDATION_PLAYLIST_ID;
        const trackPlaylistId = `${track.id}_${playlistId}`;
    ...
    getDotsVerticalMenuButton(track, playlistId = '') {
        const trackPlaylistId = `${track.id}_${playlistId}`;
    ...
    getLikedActionButton(track, playlistId) {
        if (playlistId === Constants_1.SPOTIFY_LIKED_SONGS_PLAYLIST_ID || track['liked'] === true) {
    ...
    getRemoveLikedSongButton(trackId, playlistId, showText) {
    ...
    getAddToLikedPlaylistButton(trackId, playlistId, showText) {
        return `<a href class="rounded block py-2 text-xs focus:outline-none"
    ...
    getShareTrackButton(track, playlistId) {
        return `<a href class="rounded block py-2 text-xs focus:outline-none"
    ...
    getTrackRecommendationsButton(track, playlistId) {
        return `<a href class="rounded block py-2 text-xs focus:outline-none"
    ...
    getAlbumButton(track, playlistId) {
        return `<a href class="rounded block py-2 text-xs focus:outline-none"
    ...
    getPlaylistAddButton(track, playlistId) {
        return `<a href class="rounded block py-2 text-xs focus:outline-none"
      onclick="onCmdClick('showAlbum', { trackId: '${track.id}', playlistId: '${playlistId}' })"
      role="menuitem" tabindex="-1" id="menu-item-1">
        <div class="flex items-center space-x-2">
    ...
    getTrackControlButton(playerContext) {
        return __awaiter(this, void 0, void 0, function* () {
      ...
      <div data-plugin--music-time--sidebar-target="trackControlMenu"
        ...
        <div class="text-sm p-1 space-y-2 divide-y focus:outline-none" role="none">
          ...
          <div class="flex flex-col pl-1 pr-2 py-2 space-y-2" role="none">
            ${yield this.getPlayingTrackItemHtml(playerContext)}
    ...
    getDeviceItemInfoHtml(playerContext) {
        var _a;
        const ctx = playerContext || new cody_music_1.PlayerContext();
        let selectedDeviceText = 'Connect to a Spotify device';
        let deviceInfoText = 'Launch the web or desktop player';
        if ((_a = ctx.device) === null || _a === void 0 ? void 0 : _a.name) {
    ...
    getPlayingTrackItemHtml(playerContext) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            if ((_a = playerContext === null || playerContext === void 0 ? void 0 : playerContext.item) === null || _a === void 0 ? void 0 : _a.name) {
                const isLikedTrack = yield (0, PlaylistDataManager_1.isLikedTrackId)(playerContext.item.id);
                const playlistId = (0, PlaylistDataManager_1.getSelectedPlaylistId)();
    ...
    getPlayControlButtonsItemHtml(playerContext) {
    ...
    getSortButton() {
        if ((0, PlaylistDataManager_1.sortingAlphabetically)()) {
    ...
    getPlayPauseButton(playerContext) {
    ...
    getMetricsSidebar(audioMetrics) {
    ...
    getGenerateFeatureRecommendationsButton() {
    ...
    getViewData(selectedTabView, playlist_id, spotifyUser) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a, _b;
            let playlistTracks = {};
            ...
            const likedPlaylistItem = (0, PlaylistDataManager_1.getSpotifyLikedPlaylist)();
            let likedTracks = [];
            let playerContext = undefined;
            if (spotifyUser === null || spotifyUser === void 0 ? void 0 : spotifyUser.id) {
                const playerContextP = (0, PlaylistDataManager_1.getPlayerContext)();
                if (selectedTabView === "playlists") {
                    playlistTracks = (0, PlaylistDataManager_1.getCachedPlaylistTracks)();
                    const softwareTop40PlaylistP = (0, PlaylistDataManager_1.getCachedSoftwareTop40Playlist)();
                    const spotifyPlaylistsP = (0, PlaylistDataManager_1.getCachedSpotifyPlaylists)();
                    const likedTracksP = (0, PlaylistDataManager_1.getCachedLikedSongsTracks)();
                    softwareTop40Playlist = (_a = yield softwareTop40PlaylistP) !== null && _a !== void 0 ? _a : [];
                    spotifyPlaylists = (_b = yield spotifyPlaylistsP) !== null && _b !== void 0 ? _b : [];
                    likedPlaylistItem.tracks.total = spotifyPlaylists.length || 1;
                    likedTracks = yield likedTracksP;
                    selectedPlaylistId = playlist_id ? playlist_id : (0, PlaylistDataManager_1.getSelectedPlaylistId)();
                }
                else if (selectedTabView === "metrics") {
                    audioMetrics = yield (0, PlaylistDataManager_1.getCachedAudioMetrics)();
                }
                else if (selectedTabView === "recommendations") {
                    recommendationInfo = (0, PlaylistDataManager_1.getCachedRecommendationInfo)();
                }
                playerContext = yield playerContextP;
            }
            return {
                playlistTracks,
                playerContext,
...
exports.MusicTimeWebviewSidebar = MusicTimeWebviewSidebar;
//# sourceMappingURL=MusicTimeWebviewSidebar.js.map