...
class Conductor:
    ...
    def decode_vibe(self, intent):
        ...
        # PROMPT ENGINEERING for VIBE VECTORS (MusicFX DJ Protocol)
        prompt = f"""
        ACT AS "GABRIEL DJ" (Powered by Google MusicFX).
        Decode the user's INTENT: "{intent}"
        
        Return a JSON object (NO MARKDOWN) with:
        - audio: {{ "bpm": int, "key": str, "style": str, "music_fx_prompt": str }}
        - visual: {{ "palette": str (Hex code + name), "style": str }}
        - text: {{ "tone": str }}
        
        RULES:
        - "music_fx_prompt" must be a rich, descriptive prompt suitable for MusicFX/MusicLM.
