...
def monitor_fleet():
    ...
    try:
        while True:
            ...
            for agent in FLEET:
                # Skip if max retries reached and not critical
                if agent.get("abandoned"): continue
                ...
                if proc and proc.poll() is not None:
                    ...
                    if agent["restarts"] >= MAX_RETRIES:
                        if agent["critical"]:
                            cfg.system_log(f"🛑 CRITICAL FAILURE: {agent['name']} failed too many times. System Unstable.", "ERROR")
                            # We keep trying criticals or exit? For a persistent server, maybe keep trying slowly?
                            # For now, slow down restart.
                            time.sleep(5)
                        else:
                            cfg.system_log(f"⛔ AGENT ABANDONED: {agent['name']} (Max Retries).", "ERROR")
                    ...
                    # RESTART
                    agent["restarts"] += 1
                    cfg.system_log(f"🔄 RESTARTING {agent['name']} (Attempt {agent['restarts']}/{MAX_RETRIES})...", "INFO")
                    launch_agent(agent)
