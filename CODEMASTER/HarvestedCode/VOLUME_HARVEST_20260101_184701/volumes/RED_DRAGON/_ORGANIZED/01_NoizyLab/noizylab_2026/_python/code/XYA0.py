...
import os, sys, shlex, subprocess, time, json, shutil
...
def disk_scan():
    ...
    code, du_out = run(["du", "-sh", "/Applications", "/Library", home, "/System", "/usr", "/opt", "/private"], allow_fail=True)
    out.append(du_out.strip())
    out.append("\n" + heading("Candidate Folders"))
    for c in candidates:
        code, size = run(["du", "-sh", os.path.expanduser(c)], allow_fail=True)
...
def spotlight_status(reindex=False):
    ...
    code, st = run(["mdutil", "-s", "/"], allow_fail=True)
    out.append(st.strip())
    if reindex:
        code, idx = run(["mdutil", "-E", "/"], allow_fail=True, sudo=True)
        out.append("\nTriggered reindex:\n" + idx.strip())
...
def logs_summary():
    ...
    code, syslog = run(["log", "show", "--style", "syslog", "--last", "30m"])
...
def brew_health():
    ...
    for desc, cmd in [
        ...
        code, o = run(cmd, allow_fail=True)
        out.append(f"\n## {desc}\n{o.strip()}")
...
def python_env_health():
    ...
    code, pipv = run(["python3", "-m", "pip", "--version"], allow_fail=True)
    out.append("\n## pip\n" + pipv.strip())
    code, pipc = run(["python3", "-m", "pip", "check"], allow_fail=True)
    out.append("\n" + pipc.strip())
...
def audio_tts_health():
    ...
    # coreaudiod running?
    code, ps = run(["pgrep", "coreaudiod"], allow_fail=True)
    out.append("coreaudiod PID(s): " + (ps.strip() or "not running"))
    # default audio devices snapshot
    code, sp = run(["system_profiler", "SPAudioDataType"], allow_fail=True)
...
def restart_coreaudio():
    code, out = run(["sudo", "pkill", "coreaudiod"], allow_fail=True)
    return "Requested coreaudiod restart."

def purge_ram():
    code, which = run(["which", "purge"], allow_fail=True)
    if code != 0:
...
def noizy_workspace_health():
    ...
    if (WORKSPACE / "chacha_bridge.py").exists():
        out.append("\nBridge present.")
    if (WORKSPACE / "check_vsbuddy.py").exists():
        code, res = run(["python3", str(WORKSPACE / "check_vsbuddy.py"), "status"], allow_fail=True)
        out.append("\nVS Buddy status:\n" + res.strip())
