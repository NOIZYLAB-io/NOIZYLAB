...
Onboarding & Advanced Extension Points:
---------------------------------------
To get started, create and activate a Python 3 virtual environment:
    python3 -m venv ~/noizyvenv
    source ~/noizyvenv/bin/activate
Install requirements:
    pip install prompt_toolkit rich pyperclip ffmpeg-python
Launch the assistant:
    python vsbuddy.py
Choose your workflow:
    - Audio/Media Automation (NoizyBrain)
    - General Automation (SuperBrain)
Extend by adding new handler functions and registering them in the menu.
For advanced features (previews, undo, multi-pane navigation), add new menu actions and handlers.
For NGR integration, add new handler functions for routing, approval, and feedback, and register NGR-specific actions and workflows using register_module('NGR', ngr_menu()).
For market adaptation, create new menu functions (e.g., finance_menu, health_menu) and register them with register_module(). Handlers can be reused or customized per market.
...
def handle_ng_voice():
        print("[NGR] Voice command handler")
...
# Handler functions and menu registry are defined below
