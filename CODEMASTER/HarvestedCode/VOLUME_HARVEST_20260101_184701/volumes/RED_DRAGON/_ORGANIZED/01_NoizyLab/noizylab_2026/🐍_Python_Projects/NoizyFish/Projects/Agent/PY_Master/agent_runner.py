...
Prints confirmation time when successful.
"""

import concurrent.futures, subprocess, sys, time, os
...
def run_with_agents(num_agents=96):
    while True:
        ...
        if all("finished clean" in r for r in results):
            print(f"✅ All 96 agents ran clean! [{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}]")
            os.system('afplay /System/Library/Sounds/Glass.aiff')  # macOS chime
            break
        else:
