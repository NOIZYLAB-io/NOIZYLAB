def generate_account_csv(filepath):
    import csv
    accounts = [
        {
            "Platform": "Microsoft",
...
class AccountManager:
    ...
    def log_failed_login(self, device="HP OMEN", platform="Microsoft"):
        for acc in self.accounts:
            if acc.get("Platform", "").lower() == platform.lower():
                note = acc.get("Notes", "")
                log_entry = f"Failed login attempt from {device} on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                if log_entry not in note:
                    acc["Notes"] = (note + "\n" if note else "") + log_entry
    ...
    def __init__(self, filename=ACCOUNTS_FILE):
        ...
        # Remove unfinished Microsoft profiles (missing required fields)
        required_fields = {"Platform", "Login Email", "Primary Email", "2FA"}
        self.accounts = [
            acc for acc in self.accounts
            if not (
                acc.get("Platform", "").lower() == "microsoft" and (
                    any(acc.get(field, "").strip() == "" for field in required_fields)
                )
            )
        ]
...
class NotificationAgent:
        ...
        def main():
            ...
            # Example: log a failed login attempt (uncomment if needed)
            # manager.log_failed_login(device="HP OMEN", platform="Microsoft")
            while True:
