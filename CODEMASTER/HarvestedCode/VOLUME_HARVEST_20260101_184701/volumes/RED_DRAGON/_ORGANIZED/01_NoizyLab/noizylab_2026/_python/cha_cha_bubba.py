...
def run_diagnostics():
    return "\n".join([
        ...
        "Disk: " + run(["df", "-h", "/"]).splitlines()[1],
        "Memory: " + " | ".join(run(["vm_stat"]).splitlines()[:5])
    ])
...
def cleanup_desktop():
    ...
    # After normal cleanup, run project-aware pass
    proj_log = project_aware_cleanup()
    log_txt = log_txt + "\n\n--- Project-aware pass ---\n" + proj_log
    save_log("cleanup_desktop", log_txt)
    return log_txt

def project_aware_cleanup():
    """Move Desktop files into project folders based on filename prefix."""
    desktop = Path.home() / "Desktop"
    if not desktop.exists():
        return "No Desktop folder."
    moved = []
    for f in desktop.iterdir():
        if f.is_file():
            ...
            if prefix.lower() in ["heritage", "noizybrain", "superbrain", "vsbuddy", "bubba"]:
                dest = WORKSPACE / prefix.capitalize()
                dest.mkdir(parents=True, exist_ok=True)
                shutil.move(str(f), str(dest / f.name))
                moved.append(f"{f.name} → {dest}")
    return "Project-aware moved:\n" + ("\n".join(moved) if moved else "No matching project files.")
...
def announce_health():
    ...
    summary = []
    summary.append("Parallels running" if "running" in report else "Parallels not running")
...
def quick_health():
    disk = run(["df", "-h", "/"]).splitlines()[1]
    parallels = run(["pgrep", "-fl", "Parallels"])
    status = f"Disk {disk.split()[3]} free | Parallels: {'running' if parallels else 'not running'}"
    log("QuickHealth → " + status)
    speak("Health check: " + status.replace("|", "and"))
    return status
...
def listen_loop():
    if not have_sr():
        ...
        print(msg); speak("Microphone not ready."); return
    announce_health()
    wake = ["hey cha-cha", "cha-cha my dear", "hey cha cha", "cha cha"]
    speak("Cha-Cha is listening, my dear.")
    while True:
        said = listen_once()
        if not said: continue
        text = said.lower()
        if any(p in text for p in wake):
            for p in wake: text = text.replace(p, "").strip()
            if text:
                result = handle_chain(text)
...
COMMANDS = {
    ...
    "project cleanup": project_aware_cleanup,
