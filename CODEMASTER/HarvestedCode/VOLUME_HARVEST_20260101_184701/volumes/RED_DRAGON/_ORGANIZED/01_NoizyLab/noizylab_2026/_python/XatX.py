...
def main():
    ...
    # Parallel sweep for Documents and Home
    AGENTS_CHA_CHA = 48
    AGENTS_BUBBA = 48
    ...
    with ThreadPoolExecutor(max_workers=AGENTS_CHA_CHA) as cha_cha_pool, \
        ...
        cha_cha_tasks = [
            lambda: move_documents_to_workspace(WORKSPACE, voice),
            lambda: move_home_files_to_workspace(WORKSPACE, voice),
        ]
        bubba_tasks = [
            lambda: delete_empty_folders(WORKSPACE),
        ]
    ...
    # Sweep Desktop files (single-threaded for safety)
    moved, skipped = 0, 0
    log_lines = []
