...
@app.route('/get_agent_activity')
def get_agent_activity():
    # Simulate 8 agents with random completed tasks
    activity = [random.randint(0, 20) for _ in range(8)]
    return jsonify({'activity': activity})

@app.route('/get_scan_progress')
def get_scan_progress():
    # Simulate scan progress
    scanned = random.randint(0, 100)
    total = 100
    return jsonify({'scanned': scanned, 'total': total})
