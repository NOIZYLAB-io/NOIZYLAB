...
def main():
    # List available drives or top-level folders
    if sys.platform == "win32":
        import string
        drives = [f"{d}:\\" for d in string.ascii_uppercase if os.path.exists(f"{d}:\\")]
    else:
        # On macOS/Linux, list /Volumes and / (root)
        drives = ["/Volumes/" + d for d in os.listdir("/Volumes") if not d.startswith('.')]
        drives.insert(0, "/")

    print("Select a drive or folder to scan:")
    for idx, d in enumerate(drives):
        print(f"  [{idx}] {d}")
    selection = input("Enter the number of your choice: ")
    try:
        selection = int(selection)
        search_path = drives[selection]
