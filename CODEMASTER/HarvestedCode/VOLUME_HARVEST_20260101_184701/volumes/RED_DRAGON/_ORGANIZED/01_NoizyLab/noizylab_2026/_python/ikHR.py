...
def clean_big_house(copy_mode=True):
    drives = [d for d in Path("/Volumes").iterdir() if d.is_dir() and d.name not in ("Macintosh HD", "Macintosh HD - Data")]
    if not drives:
        return "No /Volumes drives found."
    ...
    def process_file(item: Path):
        try:
            ...
            for project, exts in categories.items():
                if item.suffix.lower() in exts:
                    ...
                    if copy_mode:
                        ...
                        return f"Copied {item} → {project}"
                    else:
                        shutil.move(str(item), str(dest))
                        return f"Moved {item} → {project}"
            # fallback
            ...
            if copy_mode:
                shutil.copy2(item, dest)
                return f"Copied {item} → Misc_Project"
            else:
                shutil.move(str(item), str(dest))
                return f"Moved {item} → Misc_Project"
        except Exception as e:
            return f"Error {item}: {e}"

    report = ["Bubba Big House Parallel Cleanup Report:\n"]
    ...
    log = "\n".join(report)
    save_log("cleanup_big_house_parallel", log)
    return log
