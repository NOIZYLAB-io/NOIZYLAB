"""
Noizy Saver: Robust text file management toolkit for ~/Documents/NoizyAI_Documents.
Features: save, read, list, delete, search, rename, info, bulk delete, export.
"""
...
import argparse
import asyncio
from noizy_saver import save_to_noizy  # Add this import
...
def main():
    ...
    parser.add_argument("prompt", help="Prompt to send to the nine")
    parser.add_argument("--fuse", choices=["concat", "vote", "summary"], default="concat")
    args = parser.parse_args()

    fused = asyncio.run(call_nine(args.prompt, args.fuse))
    log_content = f"PROMPT: {args.prompt}\n\n{fused}"
    # Use save_to_noizy for robust logging
    success, log_path = save_to_noizy(
        log_content,
        filename=f"nine_ai_party_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
        folder="NoizyAI_Documents"
    )
    print(f"--- Nine AI Party Results ({args.fuse}) ---\n{fused}\n\n[Log saved to {log_path}]")
