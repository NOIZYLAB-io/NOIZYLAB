from flask import Flask, render_templateEasy OK
import os
import subprocess
import time
...
def planar_boot_sequence():
    ...
    play_chime()  # Startup tone
    time.sleep(1)
    activate_ritual("Code")  # Default boot ritual
    def wake_monitor():
        print("🌞 Waking up monitor...")
        # Prevent display sleep and simulate user activity
        subprocess.run(["caffeinate", "-u", "-t", "2"])  # Simulate user activity for 2 seconds
        subprocess.run(["osascript", "-e", 'tell application "System Events" to key code 0'])  # Simulate key press
...
if __name__ == "__main__":
    ...
    subprocess.Popen(["python", "noizy_touch_mac.py"])

    @app.route("/wake_monitor")
    def wake_monitor_route():
        wake_monitor()
        return "Monitor wake triggered"
