...
# -*- coding: utf-8 -*-
"""
Noizy Fish Orchestrator (Unified)
Author: R.S Plowman (Fish Music Inc.)

Purpose:
- One Python command center to consolidate digital landscape (accounts, 2FA, vault)
- Enforce rp@fishmusicinc.com as the single public identity
- Trigger VS Code save/commit so changes are unanimous (code + editor)
- Open security/admin portals
- Scan drives for audio libraries, deduplicate, and assemble a master index
- Generate .vscode/tasks.json + keybindings.json so VS Code is permanently wired

Usage (examples):
  python noizyfish_orchestrator.py init
  python noizyfish_orchestrator.py audit
  python noizyfish_orchestrator.py enforce
  python noizyfish_orchestrator.py open "Microsoft 365"
  python noizyfish_orchestrator.py set2fa "YouTube" enabled
  python noizyfish_orchestrator.py store "Microsoft 365" "code1,code2,..."
  python noizyfish_orchestrator.py save_vscode
  python noizyfish_orchestrator.py git_commit "Landscape update"
  python noizyfish_orchestrator.py write_vscode_tasks
  python noizyfish_orchestrator.py scan_audio /Volumes/Archive1 /Volumes/Archive2 ~/Music
  python noizyfish_orchestrator.py assemble_audio --csv audio_library.csv --yaml audio_library.yaml
"""
...
# --- VS Code control (unanimous save/commit) ---
def vscode_save_front():
    # AppleScript: tell VS Code to save front document
...
def git_commit(message: str = "Noizy Fish orchestrator commit"):
    try:
...
# --- VS Code template wiring ---
TASKS_JSON = {
    "version": "2.0.0",
    "tasks": [
        {
            "label": "Noizy Fish: Orchestrator Status",
            "type": "shell",
            "command": "python",
            "args": ["noizyfish_orchestrator.py", "status"],
            "problemMatcher": []
        },
        {
            ...
            "type": "shell",
            "command": "python",
            "args": ["noizyfish_orchestrator.py", "audit"],
            "problemMatcher": []
        },
        {
            ...
            "type": "shell",
            "command": "python",
            "args": ["noizyfish_orchestrator.py", "enforce"],
            "problemMatcher": []
        },
        {
            ...
            "type": "shell",
            "command": "python",
            "args": ["noizyfish_orchestrator.py", "scan_audio"],
            "problemMatcher": []
        },
        {
            ...
            "type": "shell",
            "command": "python",
            "args": ["noizyfish_orchestrator.py", "assemble_audio"],
            "problemMatcher": []
        }
    ]
}
...
KEYBINDINGS_JSON = [
    {
        ...
        "args": "Noizy Fish: Orchestrator Status",
...
def write_vscode_files():
    ...
    keybindings_path.write_text(json.dumps(KEYBINDINGS_JSON, indent=2))
    console.print(f"[green]Wrote:[/green] {tasks_path}")
    console.print(f"[green]Wrote:[/green] {keybindings_path}")
...
def probe_basic_audio(path: Path) -> Dict[str, Any]:
    # Minimal probe: size, extension, optional WAV duration (fallback None)
...
def dedupe_scan() -> Tuple[int, int]:
    ...
    with AUDIO_SCAN_DB.open("r") as f:
        for line in f:
            ...
            if not checksum:
                continue
            # Choose "best": prefer highest resolution formats (wav/aiff/flac > mp3/m4a/ogg/aac)
    ...
    return sum(1 for _ in AUDIO_SCAN_DB.open("r")), len(best)
...
def export_audio_library(csv_path: Path, yaml_path: Path):
    ...
    with csv_path.open("w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=list(rows[0].keys()) if rows else ["checksum","path","name","ext","size_bytes","duration_sec"])
        w.writeheader()
        for r in rows:
            w.writerow(r)
...
def enforce():
    ...
    vscode_save_front()  # unanimous save in VS Code
...
def set2fa(service: str, status: str):
    ...
    for acc in cfg.get("accounts", []):
        if acc["service"].lower() == service.lower():
            ...
            vscode_save_front()  # unanimous save
            return
...
@cli.command()
def save_vscode():
    """Ask VS Code to save the front document."""
    vscode_save_front()


@cli.command()
@click.argument("message", required=False, default="Noizy Fish orchestrator commit")
def git_commit_cmd(message: str):
...
def write_vscode_tasks():
    """Write .vscode/tasks.json and keybindings.json."""
    write_vscode_files()
