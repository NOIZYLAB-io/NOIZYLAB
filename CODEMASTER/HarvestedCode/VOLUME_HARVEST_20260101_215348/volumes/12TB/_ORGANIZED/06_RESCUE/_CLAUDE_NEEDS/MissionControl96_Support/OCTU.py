...
def dynamic_load_agent(agent: dict[str, Any]) -> tuple[Any, list[Any]]:
    mod = importlib.import_module(str(agent["module"]))
    if agent["class"]:
        instance = getattr(mod, str(agent["class"]))()
        func = getattr(instance, str(agent["async_func"]))
    else:
        func = getattr(mod, str(agent["async_func"]))
