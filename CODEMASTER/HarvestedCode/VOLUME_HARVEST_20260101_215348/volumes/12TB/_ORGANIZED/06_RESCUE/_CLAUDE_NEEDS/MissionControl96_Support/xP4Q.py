#!/usr/bin/env python3
"""
Native Instruments File Organizer
Finds .nki, .nkm, .nkc files and organizes them by library in KONTAKT_LAB
"""

import o    def organize_files(self):
        """Organize files by complete library groups - keep libraries intact"""
        if not self.found_files:
            print("❌ No Native Instruments files found!")
            return False
        
        print(f"\n📦 Organizing {len(self.found_files)} files by complete libraries...")
        print("🔒 PRESERVING LIBRARY INTEGRITY - No libraries will be broken up!")
        
        # Group files by exact library path to preserve structure
        library_groups = defaultdict(list)
        
        for file_path in self.found_files:
            # Use the complete parent directory path as library identifier
            library_path = self.get_complete_library_path(file_path)
            library_name = self.extract_library_name(file_path)
            library_groups[f"{library_name}_{hash(str(library_path)) % 10000}"].append(file_path)
        
        # Keep libraries completely intact - no merging
        merged_groups = library_groupsmport re
