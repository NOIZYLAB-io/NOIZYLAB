...
class BobbyOrb(tk.Tk):
    def __init__(self):
        ...
        try:
            icon_path = 'bobby_icon.png'  # Place your icon in the same directory as this script
    ...
    def wake_word_listener(self):
        try:
            # Use built-in 'porcupine' keyword for testing
    ...
    def animate_listening(self):
        # Swap to bright icon (or fallback to text if not using image)
        try:
            self.canvas.itemconfig(self.orb, image=self.tk_icon_img_bright)
            self.after(1000, lambda: self.canvas.itemconfig(self.orb, image=self.tk_icon_img_normal))
        except Exception:
            # Fallback: flash text
