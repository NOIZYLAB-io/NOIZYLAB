...
def main():
    ...
    try:
        print("Executing fleet_status.sh on all nodes...")
        for node in NODES:
            print(f"Running fleet_status.sh on {node['name']} ({node['ip']})...")
            if "Mac" in node["name"]:
                subprocess.run([
                    "ssh", f"user@{node['ip']}", "/Users/rsp_ms/noizy_vista_demo/tools/fleet_status.sh"
                ], check=True)
            elif node["name"] in ["OMEN", "Inspiron"]:
                print(f"[Simulated] Would run fleet_status.sh on Windows node: {node['name']}")
            else:
                print(f"Unknown node type for fleet_status.sh: {node['name']}")
    except Exception as e:
        print(f"Error executing fleet_status.sh: {e}")

    # Restart NOIZYWIN VM
    try:
        print("Restarting NOIZYWIN VM...")
        subprocess.run(["prlctl", "stop", "NOIZYWIN", "--kill"], check=True)
        subprocess.run(["prlctl", "start", "NOIZYWIN"], check=True)
        print("NOIZYWIN VM restarted successfully.")
    except Exception as e:
        print(f"Error restarting NOIZYWIN VM: {e}")

    # Inject assets
    try:
        print("Injecting assets to NOIZYWIN VM...")
        inject_assets()
        print("Assets injected successfully.")
    except Exception as e:
        print(f"Error injecting assets: {e}")
