...
async def admin_restart(req: Request):
    ...
    valid = {"core": "noizy_part1_core.py", "mind": "noizy_part2_intelligence.py"}
    if target not in valid:
        raise HTTPException(400, f"invalid target {target}")
    script = valid[target]
    try:
        subprocess.Popen(["pkill", "-f", script], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        time.sleep(1)
        subprocess.Popen(["python3", script], cwd=str(ROOT))
