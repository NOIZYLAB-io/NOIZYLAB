...
class SecureStorage:
    def __init__(self, key_path='nda_key.key'):
        try:
    ...
    async def read_all(self, filename):
        try:
...
'''.format(date=datetime.date.today())
    ...
    async def record_signature(self, entity, signed_by, timestamp=None):
        if not timestamp:
    ...
    async def check_status(self, entity):
