...
def main():
    # Print cost policy banner (non-blocking)
    try:
        cfg = json.loads(CONFIG.read_text()) if CONFIG.exists() else {}
        cp = cfg.get('cost_policy', {})
        banner = f"[Cost Policy] free_only={cp.get('free_only', True)} allow_ai={cp.get('allow_ai', True)} background_ai={cp.get('allow_background_ai', False)}"
