...
# Try importing keyboard and pyaudio, handle ImportError gracefully
try:
    import keyboard
except ImportError:
    print("[Sarah] 'keyboard' module not found. Hotkey features will be disabled.")
    keyboard = None

try:
    import pyaudio
except ImportError:
    print("[Sarah] 'pyaudio' module not found. Voice features may be limited.")
    pyaudio = None
...
custom_encouragement = "Rob, remember: you are capable, creative, and ready for anything. Let's make today amazin!"

# Function to read encouragement aloud
def sarah_encourage():
    sarah_11labs_speak(custom_encouragement)
# Define your ElevenLabs API key here
api_key = os.environ.get("ELEVENLABS_API_KEY", "")  # Set your API key as an environment variable

def sarah_11labs_speak(text, volume=70):
    # Print spoken message for screen reader compatibility
    print(f"[Sarah says]: {text}")
    print(f"[Sarah says]: {text}")
...
def display_points(points):
    try:
        ...
        root = tk.Tk()
        root.title("Sarah's Important Points")
    text = tk.Text(root, font=("Arial", 12), wrap="word", bg="#FFF8E1", fg="#4444FF")
        text.pack(expand=True, fill="both")
        for point in points:
            text.insert("end", f"• {point}\n\n")
        text.config(state="disabled")
        root.geometry("500x400")
        threading.Thread(target=root.mainloop, daemon=True).start()
    except Exception as e:
        print(f"[Sarah] Could not display points: {e}")

def control_key_listener():
    if keyboard is None:
        print("[Sarah] 'keyboard' module not available. Control key listener disabled.")
        return
    while True:
...
def idle_reminder():
    idle_time = 0
    if keyboard is None:
        print("[Sarah] 'keyboard' module not available. Idle reminder disabled.")
        return
    while True:
