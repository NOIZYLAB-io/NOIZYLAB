...
# Restart Core Audio to apply changes
subprocess.run(["sudo", "launchctl", "kickstart", "-k", "system/com.apple.audio.coreaudiod"])
subprocess.run(["killall", "UA Mixer Engine", "UAD Meter & Control Panel", "Console"])
subprocess.run(["sleep", "2"])
subprocess.run(["open", "-a", "Console"])

print("✅ Audio environment reset. Apollo DSPs should be clean.")

# Make the flush_audio_env.sh script executable
subprocess.run(["chmod", "+x", os.path.expanduser("~/Desktop/NoizyFish/scripts/shell/flush_audio_env.sh")])
