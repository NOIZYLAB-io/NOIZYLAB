...
from support_intake import support_manager as support_intake

# Import Voice Assistant System
from voice_assistant import auto_reply_system as voice_assistant

# Load environment variables
load_dotenv()

# Define cyberpunk CSS for use across pages
cyberpunk_css = """
    body { 
    ...
    .nav-links a:hover { 
        background: linear-gradient(135deg, #00ff9f, #00e676); 
        color: #1a1a1a; 
...
def dashboard():
    ...
    <body>
        ...
        <div class="nav-links">
            ...
            <a href='/support_intake'>🎤 AI Support</a>
        </div>
    </body>
    </html>
    """
...
# Support Intake Routes
@app.get("/support_intake")
async def support_intake_page():
    """Support intake page with voice and image upload"""
    return HTMLResponse(f"""
    <!DOCTYPE html>
    <html>
    <head>
        <title>Noizyfish Support - AI Powered</title>
        <meta charset="UTF-8">
    ...
    <body>
        <div class="container">
            <h1>🎤 AI Support Intake</h1>
            <p>Upload an image and record a voice message describing your issue. Our AI will analyze and categorize it automatically.</p>
            ...
            <form id="supportForm" enctype="multipart/form-data">
                ...
                <div class="voice-recorder">
                    <h3>🎙️ Voice Message</h3>
                    <p>Describe your issue in detail</p>
                    <button type="button" id="recordBtn" class="record-btn">🎤</button>
                    <div id="audioPreview" class="audio-preview" style="display: none;">
                        <audio controls id="audioElement"></audio>
                        <button type="button" id="deleteAudio" style="margin-left: 10px;">Delete</button>
                    </div>
                </div>
        ...
        <script>
            ...
            function handleImageSelect() {{
                ...
                if (file) {{
                    ...
                    reader.onload = e => {{
                        imagePreview.innerHTML = `<img src="${{e.target.result}}" style="max-width: 300px; border-radius: 5px;">`;
                    }};
            ...
            async function handleSubmit(e) {{
                ...
                if (!audioBlob) {{
                    showStatus('Please record a voice message describing your issue.', 'error');
                    return;
                }}
                
                if (!imageFile.files[0]) {{
                    showStatus('Please upload an image of your issue.', 'error');
                    return;
                }}
                ...
                showStatus('Processing your request with AI...', 'success');
                ...
                try {{
                    ...
                    if (response.ok) {{
                        showStatus('Support request submitted successfully! You will receive an AI-powered response shortly.', 'success');
            ...
            function displayAIAnalysis(result) {{
                ...
                analysisEl.innerHTML = `
                    ...
                    <p><strong>Issue Description:</strong> ${{result.description}}</p>
                    <p><strong>Priority:</strong> ${{result.priority}}</p>
                    ${{result.ai_response ? `<p><strong>AI Response:</strong> ${{result.ai_response}}</p>` : ''}}
                `;
...
async def submit_support_request(
    ...
    """Submit support request with voice and image"""
    try:
        ...
        # Process with AI support system
        ticket_data = await support_intake.process_support_request(
        ...
        voice_response = await voice_assistant.generate_auto_reply(
            ticket_data['transcription'],
        ...
        return {
            ...
            "transcription": ticket_data['transcription'],
            "description": ticket_data['description'],
            "priority": ticket_data['priority'],
            "ai_response": voice_response.get('text_response'),
            "voice_url": voice_response.get('audio_url')
        }
        
    except Exception as e:
        logger.error(f"Support submission error: {str(e)}")
...
async def get_support_tickets():
    ...
    try:
        tickets = support_intake.ticket_manager.get_all_tickets()
        return {"tickets": tickets}
    except Exception as e:
        logger.error(f"Error getting tickets: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/api/support/voice_reply/{ticket_id}")
async def generate_voice_reply(ticket_id: str, message: str = Form(...)):
    """Generate voice reply for a ticket"""
    try:
        ticket = support_intake.ticket_manager.get_ticket(ticket_id)
        if not ticket:
            raise HTTPException(status_code=404, detail="Ticket not found")
        
        voice_response = await voice_assistant.generate_contextual_response(
            message, 
            ticket['category'],
            ticket['client_name']
        )
        ...
        logger.error(f"Voice reply error: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
