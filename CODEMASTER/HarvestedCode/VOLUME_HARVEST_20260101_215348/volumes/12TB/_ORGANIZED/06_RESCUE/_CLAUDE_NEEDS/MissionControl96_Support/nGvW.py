...
def run_task(task: str):
    ...
    if "read vault" in t:
        subprocess.run(["python", "authenticator_orchestrator.py", "vault", "read"])
    ...
    if "set 2fa" in t and "microsoft 365" in t:
        subprocess.run(["python", "authenticator_orchestrator.py", "set2fa", "Microsoft 365", "enabled"])
        return "Microsoft 365 two-factor authentication enabled."
    if "save all" in t:
        subprocess.run(["python", "authenticator_orchestrator.py", "save_all"])
        return "All credentials and settings saved."
    if "totp" in t and "service" in t:
        subprocess.run(["python", "authenticator_orchestrator.py", "totp", "Service", "SECRET"])
        return "TOTP for Service executed."
