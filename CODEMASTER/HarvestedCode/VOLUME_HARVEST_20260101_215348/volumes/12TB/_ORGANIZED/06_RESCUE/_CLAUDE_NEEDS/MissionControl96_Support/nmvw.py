...
def check_assets():
    ...
    if missing:
        ...
        with open(os.path.expanduser("~/Desktop/MissionControl96/logs/isobuilder.err"), "a") as log:
            log.write(f"{missing} missing for build\n")
        return False
    return True
...
def main():
    ...
    if not check_assets():
        print("⚠️ Build aborted due to missing assets. Check logs and move required files to ~/NOIZYWIN_ASSETS/.")
        return
    mount_iso()
    copy_iso_contents()
    unmount_iso()
