...
# NI monolith/container extensions indicating embedded samples
...
def scan_library(lib: Path) -> Tuple[bool, Dict[str, object]]:
    ...
    # Consider NI-style libraries complete with monoliths or nicnt even if raw audio count is low
    ...
    while queue:
        ...
        if d > 0:
            ...
            if has_instr:
                ...
                continue
            if name.lower().endswith(" library"):
                roots.append(curr)
                continue
            if has_nicnt or has_monolith:
                roots.append(curr)
                continue
        # descend
        if d < max_depth:
...
def run(execute: bool = False) -> None:
    ...
    for entry in candidates:
        ...
        # Normalize destination library name
        ...
        if execute:
            ...
            if action == "moved":
                totals["moved"] += 1
                if not complete:
                    # bytes for partials can be tracked too
                    totals["bytes"] += int(meta["bytes"]) if isinstance(meta["bytes"], int) else 0
...
# One-line safe cleanup (user areas only)
find /Volumes -type d -empty -not -path "*/Mission Control*" -not -path "*/.Spotlight*" -not -path "*/.fseventsd*" -not -path "*/System*" -delete 2>/dev/null && echo "🧹 Empty folders cleaned safely!"
