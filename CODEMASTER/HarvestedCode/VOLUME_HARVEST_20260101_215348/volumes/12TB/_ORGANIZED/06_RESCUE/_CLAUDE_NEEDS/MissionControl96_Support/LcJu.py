...
# If you need yaml, install it via pip and uncomment the next line
# import yaml
try:
    import yaml
except ImportError:
...
def generate_json_report(folder, removed, missing, dest_path, action, old_count=None, old_size=None, new_count=None, new_size=None):
...
def main():
    ...
    root.lift()
    root.focus_force()
    label = tk.Label(root, text="Drag folders here (max 10)\nOr click to select", font=("Arial", 14), bg="#f0f0f0", relief="ridge", bd=2)
    label.pack(expand=True, fill="both", padx=20, pady=30)
    dropped_folders = []

    def process_folders(folders):
        if not folders:
            messagebox.showwarning("Perfectionist", "No folders provided.")
            return
        folders = folders[:10]
        for folder in folders:
            removed = clean_folder(folder)
            missing = validate_structure(folder)
            unwanted_found = len(removed) > 0
            dest_root = os.path.join(os.path.dirname(folder), "Perfectionist_Cleaned")
            dest_path = os.path.join(dest_root, os.path.basename(folder))
            def get_file_count_and_size(path):
                total_files = 0
                total_size = 0
                for root2, _, files2 in os.walk(path):
                    for file2 in files2:
                        total_files += 1
                        total_size += os.path.getsize(os.path.join(root2, file2))
