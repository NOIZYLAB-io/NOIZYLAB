    ...
    def do_GET(self):
        ...
        # API: Action Trigger
            ...
            if action == "ingest":
                ...
                subprocess.Popen(["python3", str(SCRIPTS_DIR / "turbo_vis.py")])
                self.wfile.write(json.dumps({"status": "vis_started"}).encode())
                return
        ...
        if path == "/" or path == "/index.html":
            ...
            if os.path.exists("universe_status.html"):
                self.serve_file("universe_status.html", 'text/html')
                return
            else:
                self.wfile.write(b"Status Report Not Generated Yet. Run turbo_vis.py")
                return
        elif path == "/map":
             if os.path.exists("volume_map.html"): # legacy name or need to update cartographer output?
                 self.serve_file("volume_map.html", 'text/html')
                 return
