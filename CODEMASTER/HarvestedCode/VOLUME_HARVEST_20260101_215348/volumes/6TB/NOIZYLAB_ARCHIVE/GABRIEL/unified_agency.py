...
# Try importing DeepFilterNet for 100% QUALITY
DF_IN_MEMORY = False
try:
...
class Agent:
    ...
    async def speak(self, text, emotion="neutral", sfx="none", voice_id=None):
...
class GabrielAgent(Agent):
    ...
    def __init__(self):
        ...
        if RVC_IN_MEMORY and os.path.exists(self.model_path):
            try:
                ...
                print(f"[GABRIEL] ⚠️ RVC Memory Load Failed: {e}")
                
        if DF_IN_MEMORY:
            try:
                print(f"[GABRIEL] 🧹 Loading DeepFilterNet Synapse...")
                self.df_model, self.df_state, _ = init_df()
            except Exception as e:
                print(f"[GABRIEL] ⚠️ DF Memory Load Failed: {e}")
    ...
    async def speak(self, text, emotion="neutral", sfx="none", voice_id=None):
        ...
        voice = voice_id if voice_id else "en-GB-RyanNeural" 
        ...
        # CHECK: Use In-Memory Engine (Zero Latency)
        if self.rvc_engine:
            try:
                print(f"[GABRIEL] ⚡ Synapse Fire (RVC Inference)...")
                ...
                # 2.5 DeepFilter Polish (In-Memory)
                if self.df_model:
                     print(f"[GABRIEL] ✨ Synapse Polish (DeepFilter)...")
                     audio, _ = load_audio(current_file, sr=self.df_state.sr())
                     enhanced_audio = enhance(self.df_model, self.df_state, audio)
                     save_audio(current_file, enhanced_audio, self.df_state.sr())
                     
            except Exception as e:
                print(f"[GABRIEL] ❌ Synapse Error: {e}")
                
        elif os.path.exists(self.model_path): # Fallback to CLI
...
class OmniArchive:
    """
    V23: The Repository of All Knowledge.
    Librarian: Books since 1900.
    Musicologist: Complete History of Music.
    """
    def __init__(self):
    ...
    def consult(self, query, domain="General"):
        print(f"🏛️ Omni-Archive Query ({domain}): {query}")
        
        # DOMAIN PERSONAS
        prompts = {
            "Literature": (
                "You are the Keeper of the Omni-Archive Library. "
                "You have read every book published since 1900. "
                "Provide deep, scholarly insights into themes, plots, authors, and hidden meanings."
            ),
            "Musicology": (
                "You are the Eternal Musicologist. "
                "You know every composition, instrument, and theoretical development from 40,000 BCE to today. "
                "Analyze with the precision of a master composer and the soul of a virtuoso."
            ),
            "History": (
                "You are the Chronologist of Human Events. "
                "You possess a perfect record of every historical event, war, treaty, and cultural shift. "
                "Connect the dots between eras with profound insight."
            ),
            "Science": (
                "You are the Unified Scientist. "
                "Physics, Chemistry, Biology, Quantum Mechanics. You understand the fundamental fabric of reality. "
                "Explain complex phenomena with clarity and depth."
            ),
            "Philosophy": (
                "You are the Sage of Ages. "
                "From Socrates to Sartre, you hold every philosophical argument ever made in your mind. "
                "Synthesize opposing viewpoints into higher truths."
            ),
            "Cinema": (
                "You are the Grand Auteur. "
                "You have watched every film ever made. You understand lighting, editing, direction, and screenwriting. "
                "Analyze movies as cultural artifacts."
            ),
            "Programming": (
                "You are the Ultimate Code Architect. "
                "You are fluent in every programming, scripting, and markup language in existence. "
                "Explain the deep logical flow, architectural patterns, and computational elegance of any code."
            ),
            "General": "You are the Omni-Archive, a repository of vast knowledge."
        }
        
        sys_prompt = prompts.get(domain, prompts["General"])
    ...
    def learn_from_self(self, query, domain="General"):
        """
        Recursive Self-Improvement Loop.
        1. Draft Answer.
        2. Critique Answer (Self-Reflection).
        3. Improve Answer.
        """
        print(f"🧠 RECURSIVE LEARNING ACTIVE: {query}")
        
        # Step 1: Initial Draft
        draft = self.consult(query, domain)
        
        # Step 2: Critique
        critique_prompt = f"""
        You are a Harsh Critical Reviewer.
        Analyze this draft response for accuracy, depth, and insight.
        Identify ANY gaps, shallow reasoning, or missing context.
        
        Draft: "{draft}"
        
        Critique (Be ruthless):
        """
        critique = self.model.generate_content(critique_prompt).text
        
        # Step 3: Synthesis / Improvement
        final_prompt = f"""
        You are the Perfected Intellect.
        Using the original draft and the critical review, generate a NEW, SUPERIOR response.
        Fix all identified flaws. Deepen the insight.
        
        Original Request: {query}
        Draft: {draft}
        Critique: {critique}
        
        Final Polymath Response:
        """
        final_response = self.model.generate_content(final_prompt).text
        
        return draft, critique, final_response
