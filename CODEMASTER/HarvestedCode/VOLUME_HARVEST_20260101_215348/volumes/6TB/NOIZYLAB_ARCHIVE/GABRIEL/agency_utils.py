...
class DreamFactory:
    ...
    V37: Added ProRes/MOV Export for CapCut Compatibility.
    """
    @staticmethod
    def create_dream(audio_path, image_url, export_format="mp4"):
        """
        export_format: 'mp4' (default, fast), 'prores' (CapCut-ready, high quality)
        """
        try:
            # 1. Paths
            ...
            # V37: Format-specific output
            if export_format == "prores":
                video_path = os.path.join(base_dir, f"dream_{dream_id}_prores.mov")
            else:
            ...
            # V37: Format-specific encoding
            if export_format == "prores":
                # Director's Cut: ProRes 422 for professional editing
                cmd = [
                    "ffmpeg", "-y",
                    "-loop", "1", "-i", image_path,
                    "-i", audio_path,
                    "-vf", "scale=1920:1080,zoompan=z='min(zoom+0.0015,1.5)':d=700:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s=1920x1080",
