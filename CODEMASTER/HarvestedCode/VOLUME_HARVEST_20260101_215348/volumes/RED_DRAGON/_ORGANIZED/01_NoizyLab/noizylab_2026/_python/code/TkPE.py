"""
     ____  ____  _   _ _   _ ____ _____ ____ _____ 
    |  _ \|  _ \| | | | \ | |  _ \_   _/ ___| ____|
    | |_) | |_) | | | |  \| | | | || || |  _|  _|  
    |  _ <|  _ <| |_| | |\  | |_| || || |_| | |___ 
    |_| \_\_| \_\\___/|_| \_|____/ |_| \____|_____|

ERNEST - The Edgy Batch Renamer
A tribute to Hemingway: never the easy way, always the beautiful way.
"""
import os
import re
import csv
...
def batch_rename(folder: str, files: List[str], func, *args, **kwargs):
    print("\n--- Ernest Batch Renaming ---")
    for f in files:
        src = os.path.join(folder, f)
        dst = os.path.join(folder, func(f, *args, **kwargs))
        try:
            os.rename(src, dst)
            print(f"[REBEL] {os.path.basename(src)} -> {os.path.basename(dst)}")
        except Exception as e:
            print(f"[OOPS] Error renaming {src}: {e}")
    print("-----------------------------\n")
...
def export_to_csv(files: List[str], out_path: str):
    with open(out_path, 'w', newline='') as csvfile:
        ...
        for f in files:
            writer.writerow([f, f])
    print(f'[LOG] Ernest log exported to {out_path}')

# --- EXAMPLE USAGE ---
if __name__ == '__main__':
    print("\nWelcome to ERNEST - The Edgy Batch Renamer\n")
    print("Channel your inner rebel. Rename with style.\n")
    ...
    # Export to CSV
    export_to_csv(files, os.path.join(FOLDER, 'ernest_log.csv'))
    print('\n[ERNEST] Batch renaming complete!\n')
