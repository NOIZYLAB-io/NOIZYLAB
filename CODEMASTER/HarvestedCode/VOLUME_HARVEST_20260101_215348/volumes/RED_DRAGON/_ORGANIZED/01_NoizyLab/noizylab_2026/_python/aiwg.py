#!/usr/bin/env python3
"""
Bubba Hand of God Big House (v5)
Trusted zone + Cha-Cha voice navigation
Auto cleanup + benchmark on start
"""
...
def in_trusted_zone(path: Path) -> bool:
    try:
        ...
        print(f"Trusted zone error: {err}")
        return False
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        # 🚀 Cruise control mode
        say_nav("Good evening Mr. Plowman. Cruise control mode engaged.")
        print(clean_big_house())
        print(benchmark_drives(2))
        say_nav("Auto-tasks complete. Bubba is standing by.")
        # drop into menu for extras
        while True:
            choice = menu()
            if choice == "a":
                print(clean_big_house())
