...
SAVE_DIR = Path.home() / "Documents" / "Noizyfish_Aquarium" / "Noizy_Workspace" / "VS_Buddy_Setup" / "Saved_Notes"
...
def main():
    ...
    # Optionally save a summary log
    if "--save" in sys.argv:
        summary = f"Processed {len(files)} files in Parallels on {datetime.datetime.now()}\n"
        save_output(summary)
