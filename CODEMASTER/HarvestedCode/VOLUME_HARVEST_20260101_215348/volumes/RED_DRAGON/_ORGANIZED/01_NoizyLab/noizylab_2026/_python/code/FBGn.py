...
def organize_chatgpt_exports():
    """Move Desktop, Downloads, ChatGPT_Exports files into Noisy Workspace projects."""
    sources = [
        Path.home() / "Desktop",
        Path.home() / "Downloads",
        WORKSPACE / "ChatGPT_Exports"
    ]
    WORKSPACE.mkdir(parents=True, exist_ok=True)
    project_map = {sub.name.lower(): sub.name for sub in WORKSPACE.iterdir() if sub.is_dir()}
    moved = []
    for src in sources:
        if not src.exists():
            continue
        for item in src.iterdir():
            if item.is_file():
                target_dir = auto_sort_file(item, project_map)
                try:
                    shutil.move(str(item), str(target_dir / item.name))
                    moved.append(f"{item.name} → {target_dir.relative_to(WORKSPACE)}")
                except Exception as e:
                    moved.append(f"❌ {item.name} — {e}")
    report = "Organizer run:\n" + "\n".join(moved) if moved else "No files moved."
    save_log("organize_chatgpt_exports", report)
    return report

def clean_big_house():
    """Full Hand of God sweep: scan all drives, fix perms, check Python."""
    drives = [
        Path.home(),  # Mission Control
        Path("/Volumes/6TB JOE MAG"),
        Path("/Volumes/4TB SYDNEY RED DRAGON"),
        Path("/Volumes/12TB for"),
        Path("/Volumes/4TB BLK"),
        Path("/Volumes/4TB utility"),
        Path("/Volumes/4TBSG"),
    ]
    report = ["=== Hand of God: Big House Sweep ==="]
    for drive in drives:
        if not drive.exists():
            report.append(f"⚠️ {drive} not found.")
            continue
        report.append(f"🔍 Scanning {drive} ...")
        for f in drive.rglob("*.py"):
            try:
                py_compile.compile(str(f), doraise=True)
                report.append(f"✅ {f.relative_to(drive)} OK")
            except Exception as e:
                report.append(f"❌ {f.relative_to(drive)} {e}")
    log = "\n".join(report)
    save_log("hand_of_god_big_house", log)
    return log

def post_restart_checklist():
    checklist = (
        "Post restart checklist for Bubba. "
        "One: finalize Cha-Cha's voice so you can hear it speaking. "
        "Two: wire Parallels Desktop and Parallels Tools into Bubba. "
        "Three: expand Super Brain from a stub into a real working module. "
        "Four: add Webador Outlook email setup. "
        "Five: optimize Bubba's speed for quicker responses. "
        "That's the plan, Mr. Plowman."
    )
    subprocess.run(["say", "-v", "Siri Voice 1", checklist], check=False)
