...
def main():
    ...
    with open(LOG, "w", encoding="utf-8") as log, open(REPORT, "w", encoding="utf-8") as report, open(ERROR_LOG, "a", encoding="utf-8") as err_log:
        for file in tqdm(files, desc="Washing audio files"):
            ...
            if info.get("ok"):
                ...
                print(f"🚫 Skipped: {file.name} (quality issue)")    print("Summary report saved to", REPORT)

    print("Summary report saved to", REPORT)_ == "__main__":
    main()
