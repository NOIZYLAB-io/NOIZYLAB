import threading
import time
def idle_prompt(api_key, voice_id):
    while True:
        time.sleep(30)
        sarah_11labs_speak("Do you need me to do anything?")
import sys
import requests
import subprocess
import threading
...
if __name__ == "__main__":
    api_key = "b2e4a5aeb966fbd4aff9e46e7bc73d77073b34f042d6ecb0dbeb75b32d961536"
    voice_id = "YOUR_SARAH_VOICE_ID"  # Replace with your actual Sarah voice ID
    # Start idle prompt thread
    prompt_thread = threading.Thread(target=idle_prompt, args=(api_key, voice_id), daemon=True)
    prompt_thread.start()
    if len(sys.argv) > 1:
