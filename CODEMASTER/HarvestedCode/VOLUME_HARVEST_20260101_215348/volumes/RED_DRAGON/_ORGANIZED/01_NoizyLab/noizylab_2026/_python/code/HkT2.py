...
def main():
    ...
    try:
        ...
        print("Safari extraction failed:", e)
        text = ""
    if not text:
        try:
            text = get_chrome_text().strip()
        except subprocess.CalledProcessError as e:
            print("Chrome extraction failed:", e)
            text = ""
    if not text:
        print("Could not extract page text from Safari or Chrome. Try selecting text manually and running read_selection_cha_cha.py.")
        sys.exit(1)
    # copy to clipboard
