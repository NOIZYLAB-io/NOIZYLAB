...
with PythonBatchRuntime(r"doit", prog_num=8):
    ...
    with Stage(r"doit", prog_num=11):
        with Stage(r"Shutting down WavesPluginaServer(s)...", prog_num=12):
        ...
        with Stage(r"Shutting down WavesPluginaServer(s)...", prog_num=19):
            # --- Begin STOP_WAVES_LOCAL_SERVER Shutting down WavesPluginaServer(s)
            with ShellCommand(r'"/Library/Application Support/Waves/WavesLocalServer/WavesLocalServer.bundle/Contents/MacOS/WavesLocalClient" wls.shutdown', message=r"Stop Waves Local Client", ignore_all_errors=True, prog_num=20, suspend=5) as shell_command_007_20:
                shell_command_007_20()
            # --- End STOP_WAVES_LOCAL_SERVER Shutting down WavesPluginaServer(s)
    with Stage(r"post_doit", prog_num=21):
        print("Done Doing it")
