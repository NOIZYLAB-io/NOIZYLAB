...
def clean_big_house(copy_mode=True):
    report = ["Bubba Big House Full Scan Report:\n"]
    categories = {
            ...
            if item.is_file():
                ...
                for project, exts in categories.items():
                    if item.suffix.lower() in exts:
                        ...
                        drive_report.append(f"{'Copied' if copy_mode else 'Moved'} {item} → {project}")
                        moved = True
                        break
                if not moved:
                    ...
                    if copy_mode:
                        ...
                        shutil.move(str(item), str(dest))
                    drive_report.append(f"{'Copied' if copy_mode else 'Moved'} {item} → Misc_Project")
        report.extend(drive_report)
        report.append(f"--- ✅ {drive.name} scan completed ---")
    log = "\n".join(report)
    save_log("cleanup_big_house", log)
    return log + "\n\nAll drives have reported completion. Comfort zone re-established."
