...
def CreateExtractor(BaseCLS,
    ...
    compat_str):
    debug('Youtube CoreVersion:[%s]' % '2025052901')
    import re
    import json
    import os
    ...
    class YoutubeIE(BaseCLS):
        ...
        def _check_formats(self, formats, video_id):
            ...
            def check_format(f):
                ...
                debug(f'begin check_format: {format_id} {check_url}')
                result = self._is_valid_url(check_url, video_id,
                    item='%s video format' % f.get('format_id') if f.get('format_id') else 'video')
                if not result and 'fragments' in f and len(f['fragments']) > 0:
                    try:
                        debug('---begin check fragments url---')
                        result = self._is_valid_url(f['fragments'][-1]['url'], video_id,
                            item='%s video format' % f.get('format_id') if f.get('format_id') else 'video')
                        debug('---end check fragments url---')
                    except:
                        pass
                debug(f'end check_format :{format_id} {check_url} {result}')
                return result

            if formats:
                valid_formats = []
                with concurrent.futures.ThreadPoolExecutor(max_workers=min(10, len(formats))) as executor:
                    futures = [executor.submit(check_format, f) for f in formats]
                    # �ȴ����м�����
                    concurrent.futures.wait(futures)

                    for i, future in enumerate(futures):
                        if future.result():
                            valid_formats.append(formats[i])
    ...
    # extracted from account/account_menu ep
        ...
        def _getErrorReason(self, player_responses):
            ...
            if subreason:
                ...
                if subreason == 'The uploader has not made this video available in your country.':
                    ...
                    if not countries:
                        regions_allowed = self._search_meta('regionsAllowed')
        ...
        def set_print_traffic(self, enable):
            #YoutubeDL.Open_print_traffic = False
            self._downloader.params['debug_printtraffic'] = enable
            self._downloader._setup_opener()            
        ...
        def _real_extract(self, url):
            ...
            url = proto + '://www.youtube.com/watch?v=%s&has_verified=1&bpctr=9999999999' % video_id
            video_webpage = self._download_webpage(url, video_id, tries=3)
            self._search_meta = ((lambda x: self._html_search_meta(x, video_webpage, default=None))
                            if video_webpage else (lambda x: None)) 
            ...
            master_ytcfg = self.extract_ytcfg(video_id, video_webpage) or self._get_default_ytcfg()
            player_responses, player_url = self._extract_player_responses(
                self._get_requested_clients(url, smuggled_data), video_id, video_webpage, master_ytcfg)
            ...
            video_title = ((self._preferred_lang and translated_title)
                       ...
                       or translated_title
                       or self._search_meta(['og:title', 'twitter:title', 'title']))
            ...
            #thumbnails 按照 height降序排列
            thumbnails = sorted(thumbnails, key=lambda x: x.get('height', 0), reverse=True)
            if not thumbnails or len(thumbnails) == 0:
                video_thumbnail = self._search_meta(['og:image', 'twitter:image'])
            else:
                #选择url为jpg的缩略图,url可能是.jpg?abcde这样
                for t in thumbnails:
                    thumbnailurl = t.get('url', '')
                    ext = compat_urllib_parse_urlparse(thumbnailurl).path.split('.')[-1]
                    if ext in ('jpg', 'jpeg', 'png'):
                        video_thumbnail = thumbnailurl
                        break
                
            video_duration = (int_or_none(get_first(video_details, 'lengthSeconds'))
                    or int_or_none(get_first(microformats, 'lengthSeconds'))
                    or parse_duration(self._search_meta('duration')) or None)
            ...
            formats = self._ExtractFormats(streaming_formats, age_gate, video_webpage, embed_webpage,video_id, player_url, None)
            if not formats:
                error = self._getErrorReason(player_responses)
                if error:
                    class YoutubeCantPlayabilityError(Exception):
                        pass
                    raise YoutubeCantPlayabilityError(error)
                       
            # formats = self._extractFormatFromAPI(url, video_webpage, embed_webpage, video_id, 
            #                                      age_gate, smuggled_data)
            # debug('_extractFormatFromAPI end')           
            #if not formats:
            #    self._reportFailReason(video_info, video_webpage, video_id)                                                  
            debug(f'_check_formats begin {len(formats) if formats else 0}')
            time_out = self._downloader._socket_timeout
            self._downloader._socket_timeout = 3    
            #import sys
            #if sys.platform == 'darwin':      
            self.set_print_traffic(False)
            self._check_formats(formats, video_id)
            self.set_print_traffic(True)
            self._downloader._socket_timeout = time_out
            debug(f'_check_formats end {len(formats) if formats else 0}')     
        ...
        def _extract_data_sync_id(self, *args):
            ...
            @params response and/or ytcfg
            """
            data_sync_id = self._configuration_arg('data_sync_id', [None], ie_key=YoutubeIE, casesense=True)[0]
            if data_sync_id:
                return data_sync_id
        ...
        def _extract_visitor_data(self, *args):
            ...
            """
            visitor_data = self._configuration_arg('visitor_data', [None], ie_key=YoutubeIE, casesense=True)[0]
            if visitor_data:
                return visitor_data
            return get_first(
        ...
        def _player_js_cache_key(self, player_url):
            ...
            player_id = self._extract_player_info(player_url)
            player_path = remove_start(compat_urlparse.urlparse(player_url).path, f'/s/player/{player_id}/')
        ...
        def _get_requested_clients(self, url, smuggled_data):
            ...
            broken_clients = []
            default = ['tv', 'ios', 'web']
            DEFAULT_AUTHED_CLIENTS = ['tv', 'web']
            default_clients = DEFAULT_AUTHED_CLIENTS if self.is_authenticated else default
            ...
            if not requested_clients:
                requested_clients = default_clients
        ...
        def _extract_player_responses(self, clients, video_id, webpage, master_ytcfg):           
            ...
            player_url = None
            player_url = visitor_data = data_sync_id = po_token = None
            while clients:
                # client = clients.pop()
        ...
        def _extractFormatFromAPI(self, url, webpage, embed_webpage, video_id, age_gate, smuggled_data):
            ...
            streaming_data = traverse_obj(player_responses, (..., 'streamingData'))
            streaming_formats = traverse_obj(streaming_data, (..., ('formats', 'adaptiveFormats'), ...))
            result = self._ExtractFormats(streaming_formats, age_gate, webpage, embed_webpage,video_id, player_url, None)
            if not result:
                error = self._getErrorReason(player_responses)
                if error:
                    class YoutubeCantPlayabilityError(Exception):
        ...
        def _attachMoreFields(self, dct, fmt, formats_spec, formats, url_data):
            ...
            if audio_track:   
                display_name = audio_track.get('displayName','')           
                dct['audioTrack_displayName'] = display_name
                language_code = audio_track.get('id', '').split('.')[0]
                is_original = 'original' in display_name.lower()
                dct['isDefaultAudio'] = audio_track.get('audioIsDefault', is_original)
                dct['is_Original_audioTrack'] = is_original
                dct['audioTrack_lang_code'] = language_code
                dct['audioTrack_display_name'] = display_name
        ...
        def _NewExtract_signature_function(self, video_id, player_url, example_sig):
            ...
            cache_spec = self._loadFromCache('youtube-sigfuncs', func_id)
            ...
            self._saveToCache('youtube-sigfuncs', func_id, cache_spec)
            return res        
        ...
        def _interpret_player_js_global_var(self, jscode, player_url):
            ...
            jsi = CreateJSInterpreterEx(array_code)
        ...
        def _loadFromCache(self, section, cache_id):
            from datetime import datetime
            cache_pref = datetime.now().date().strftime('%Y%m%d')
            return self.cache.load(f'{section}', f'{cache_pref}-{cache_id}')
        
        def _saveToCache(self, section, cache_id, data):
            from datetime import datetime
            cache_pref = datetime.now().date().strftime('%Y%m%d')
            self.cache.store(f'{section}', f'{cache_pref}-{cache_id}', data)
        ...
        def _load_nsig_code_from_cache(self, player_url):
            ...
            if func_code:
                return func_code
            func_code = self._loadFromCache('youtube-nsig', cache_id[1])
            if func_code:
        ...
        def _store_nsig_code_to_cache(self, player_url, func_code):
            ...
            if cache_id not in self._player_cache:
                self._saveToCache('youtube-nsig', cache_id[1], func_code)
                self._player_cache[cache_id] = func_code        
