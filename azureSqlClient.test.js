...
function createContext() {
    ...
    const subscriptions = [{ subscriptionId: 'id1' }, { subscriptionId: 'id2' }];
    const locations = [{ id: 'id1' }, { id: 'id2' }];
    const groups = [{ id: 'id1', location: 'l1' }, { id: 'id2', location: 'l2' }];
    const session = {
        account: accounts[0],
...
describe('Azure SQL client', function () {
    it('Should return accounts successfully', async function () {
        const testContext = createContext();
        const azureSqlClient = new azureSqlClient_1.AzureSqlClient(() => Promise.resolve(testContext.azureAccountService));
        const accounts = await azureSqlClient.getAccounts();
        should(accounts.length).equal(testContext.accounts.length);
    });
    it('Should create and return new account successfully', async function () {
        const testContext = createContext();
        const azureSqlClient = new azureSqlClient_1.AzureSqlClient(() => Promise.resolve(testContext.azureAccountService));
        const account = await azureSqlClient.getAccount();
        should(account.key).equal(testContext.accounts[0].key);
    });
    it('Should return subscriptions successfully', async function () {
        const testContext = createContext();
        const azureSqlClient = new azureSqlClient_1.AzureSqlClient(() => Promise.resolve(testContext.azureAccountService));
    ...
    it('Should return locations successfully', async function () {
        const testContext = createContext();
        const azureSqlClient = new azureSqlClient_1.AzureSqlClient(() => Promise.resolve(testContext.azureAccountService), () => Promise.resolve(testContext.azureResourceService));
    ...
    it('Should return resource groups successfully', async function () {
        const testContext = createContext();
        const azureSqlClient = new azureSqlClient_1.AzureSqlClient(() => Promise.resolve(testContext.azureAccountService), () => Promise.resolve(testContext.azureResourceService));
...
//# sourceMappingURL=azureSqlClient.test.js.map