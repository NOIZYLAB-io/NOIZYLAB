...
exports.FFmpegAudioRecorder = void 0;
...
class FFmpegAudioRecorder {
    ...
    lastFileSize = 0; // DEPRECATED: Tracking file size for growth detection
    // New variables for volumedetect system
    volumeDetectProcess = null;
    volumeTempFilePath = null;
    volumeTempFileCleanup = null;
    lastVolumeLevel = -91; // dB, start with "silence"
    volumeCheckInterval = null;
    volumeSegmentDuration = 1; // seconds for analyzing segments
    currentRecordingDevice = null; // current recording device
    constructor(events, options = {}, outputChannel // vscode.OutputChannel, but avoid importing vscode here
    ) {
        this.events = events;
    ...
    static async checkFFmpegAvailability() {
        try {
            ...
            let ffmpegPath;
            try {
                ffmpegPath = await (0, which_1.default)('ffmpeg');
            }
            catch (pathError) {
                return {
                    available: false,
            ...
            return new Promise((resolve) => {
                const versionProcess = (0, child_process_1.spawn)(ffmpegPath, ['-version']);
                let output = '';
            ...
            return {
                available: false,
                error: `Unexpected error checking FFmpeg: ${error.message}`
            };
        }
    }
    /**
     ...
     */
    static async detectInputDevices() {
        try {
            const ffmpegPath = await (0, which_1.default)('ffmpeg');
            const platform = FFmpegAudioRecorder.detectPlatform();
            return new Promise((resolve) => {
                let args = [];
                switch (platform) {
                ...
                const listProcess = (0, child_process_1.spawn)(ffmpegPath, args);
                let output = '';
    ...
    async startRecording() {
        ...
        try {
            ...
            this.ffmpegProcess = (0, child_process_1.spawn)(ffmpegCheck.path, ffmpegArgs);
    ...
    stopRecording() {
        ...
        try {
            ...
            this.events.onError(new Error(`Error stopping recording: ${error.message}`));
        }
     ...
     */
    buildFFmpegArgs(outputPath, recommendedDevice) {
        const platformCommands = FFmpegAudioRecorder.getPlatformCommands();
        const args = [];
        // Logging level
     ...
     */
    async handleRecordingComplete(exitCode) {
        ...
        try {
            ...
            this.events.onError(new Error(`Failed to process recording: ${error.message}`));
        }
        finally {
     ...
     */
    async startVolumeDetection(recommendedDevice) {
        if (!this.options.silenceDetection) {
            return;
        }
        ...
        try {
            ...
            this.volumeDetectProcess = (0, child_process_1.spawn)('ffmpeg', args);
     ...
     */
    buildVolumeDetectArgs(recommendedDevice) {
        const platformCommands = FFmpegAudioRecorder.getPlatformCommands();
        const args = [];
        ...
        if (result.ffmpegAvailable.available) {
            try {
                ...
                if (result.platform === 'macos') {
                    ...
                    const hasBuiltinMic = result.inputDevices.some(device => device.toLowerCase().includes('built-in') ||
                ...
                result.errors.push(`Failed to detect input devices: ${error.message}`);
            }
        }
        return result;
    }
     ...
     */
    static async testRecording(duration = 2) {
        const diagnostics = await FFmpegAudioRecorder.runDiagnostics();
        ...
        return new Promise((resolve) => {
            ...
            const testProcess = (0, child_process_1.spawn)(diagnostics.ffmpegAvailable.path, args);
            let errorOutput = '';
            let hasOutput = false;
            ...
            testProcess.on('close', (code) => {
                ...
                try {
                    ...
                    resolve({
                        ...
                        duration: actualDuration,
                        command: `ffmpeg ${args.join(' ')}`,
                        error: `Error checking file: ${error.message}`
                    });
                }
            });
...
exports.FFmpegAudioRecorder = FFmpegAudioRecorder;
//# sourceMappingURL=FFmpegAudioRecorder.js.map