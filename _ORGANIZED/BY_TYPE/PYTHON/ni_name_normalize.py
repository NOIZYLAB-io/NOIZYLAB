...
def run(execute: bool = False) -> None:
    ...
    # Recursive pass: collapse double spaces and trim whitespace in all nested names
    def recursive_clean(top: Path):
        for root, dirs, files in os.walk(top, topdown=False):
            ...
            for name in files:
                ...
                stem = src.stem
                ext = src.suffix
                new_stem = collapse_spaces(stem)
                # keep underscores, only collapse whitespace sequences in the base name
                new_name = new_stem + ext
                if new_name != name:
                    dst = src.with_name(new_name)
                    action, final_path = safe_rename(src, dst, execute)
                    rows.append({
                        "scope": "file",
                        "top": str(top),
                        "from": name,
