...
class MCPClient:
    ...
    def send_event(self, event_type, data):
        ...
        if event_type == "speak":
            try:
                ...
                print(f"❌ Sarah TTS failed: {e}. Falling back to Siri.")
                self.siri_fallback(data.get('text'))

    def siri_fallback(self, text):
        # Fallback to macOS say (Kate/Daniel)
        import subprocess
        for voice in ["Kate", "Daniel"]:
            try:
                subprocess.run(["say", "-v", voice, text], check=True)
                print(f"✅ Fallback succeeded with {voice}")
                return
            except Exception:
                continue
        print("❌ All fallback voices failed.")
    ...
    def diagnostics(self):
        ...
        checks = {
            ...
            "icloud_sync": self.verify_icloud_sync(),
            "secure_key_storage": self.verify_keychain(),
        }
        return checks

    def verify_icloud_sync(self):
        icloud_dir = os.path.expanduser("~/Library/Mobile Documents/com~apple~CloudDocs/SleepLearning")
        return os.path.isdir(icloud_dir) and any(f.endswith('.mp3') for f in os.listdir(icloud_dir))

    def verify_keychain(self):
        # Check if API key is stored securely (macOS Keychain)
        try:
            import subprocess
            result = subprocess.run([
                "security", "find-generic-password", "-a", os.environ.get("USER", ""), "-s", "ElevenLabsAPIKey", "-w"
...
if __name__ == "__main__":
    print("Starting MCP server on http://127.0.0.1:8788 ...")
    import uvicorn
