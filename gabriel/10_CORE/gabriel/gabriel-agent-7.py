    ...
    def do_POST(self):
        if self.path == "/exec":
            ...
            if not (is_mapped or is_allowed):
                self._json(
                    {"error": f"Command denied by AI Safety Protocol: {base_cmd}"}, 403
                )
                return
            ...
            try:
                ...
                self._json(
                    {"cmd": final_cmd, "output": output, "code": result.returncode}
                )
            except subprocess.TimeoutExpired:
                self._json(
                    {"error": "Execution timed out (Hyperdrive limit exceeded)"}, 408
                )
            except Exception as e:
...
if __name__ == "__main__":
    print("⚡ GABRIEL AGENT v2.0 ONLINE")
    print(f"⚡ Platform: {PLATFORM}")
    print(f"⚡ Port: {PORT}")
    print("⚡ AI Safety: ACTIVE")
