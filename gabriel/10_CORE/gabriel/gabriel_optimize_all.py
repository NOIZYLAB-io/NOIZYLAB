...
IGNORE_DIRS = {".git","node_modules",".venv","venv","__pycache__", ".cache", ".ds_store"}
...
def delete_empty_dirs(root: Path, dry_run: bool):
    removed = []
    # Bottom-up traversal to safely remove nested empty dirs
    for dirpath, dirnames, filenames in os.walk(str(root), topdown=False):
        d = Path(dirpath)
        try:
            if any(part in IGNORE_DIRS for part in d.parts): 
                continue
            # Logic: verify it's truly empty at this point
...
def main():
    ...
    # Safety Check for GABRIEL Volume
    if not gab.exists() and not args.dry_run:
        # If dry run, maybe we just print what we would do or error out?
        # Better to error if DEST doesn't exist unless we create it (local test)
        # But if it's /Volumes/GABRIEL and it's missing, warn.
        if "/Volumes/GABRIEL" in str(gab):
             print(f"⚠️  WARNING: Destination {gab} not found. Is the volume mounted?")
             print("   (Run check: ls /Volumes/GABRIEL)")
             # We initiate anyway if possible or return? 
             # Let's try to create it, if it fails (read-only or missing mount), we crash safely.
             try:
                 ensure_dir(gab)
             except:
                 print("❌ ERROR: Cannot create destination. Aborting.")
                 return
    ...
    print("⚡ Indexing Vault for Deduping...")
    vault_hash_index = {}
    for existing in vault_assets.rglob("*"):
        if existing.is_file():
            try:
                # Optimized: Only hash if not in some cache? 
                # For now, fast re-hash to be safe (Gold Standard)
                h = sha256_file(existing)
                vault_hash_index.setdefault(h, []).append(str(existing))
            except Exception:
                pass
    
    print(f"   Indexed {len(vault_hash_index)} vault assets.")
