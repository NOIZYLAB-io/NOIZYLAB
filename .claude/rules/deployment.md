...
5. **D1 database ID**: `f75939d5-5747-4a9c-8ac2-7710201fda09` — verify in `wrangler.toml`
...
8. **CLOUDFLARE LOGIN**: Currently `rsp@noizyfish.com` (via GoDaddy M365) — MUST change to `rsplowman@icloud.com` BEFORE killing M365
...
The Cloudflare account login is `rsp@noizyfish.com` (M365 via GoDaddy). This email dies when M365 is cancelled. **Step 0 of the GoDaddy exit**: change Cloudflare login to `rsplowman@icloud.com`. Everything else follows.
