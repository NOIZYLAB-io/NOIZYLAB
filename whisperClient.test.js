...
const WhisperClient_js_1 = require("../../core/WhisperClient.js");
describe('WhisperClient Language Settings Tests', () => {
    ...
    beforeEach(() => {
        ...
        whisperClient = new WhisperClient_js_1.WhisperClient({
            apiKey: 'test-api-key',
            timeout: 5000
        });
    });
...
describe('WhisperClient Configuration Integration Tests', () => {
    it('should validate API key format', () => {
        ...
        assert.strictEqual(WhisperClient_js_1.WhisperClient.validateApiKey('sk-1234567890123456789012345678901234567890123456'), true, 'Valid API key should pass validation');
        assert.strictEqual(WhisperClient_js_1.WhisperClient.validateApiKey('invalid-key'), false, 'Invalid API key should fail validation');
        assert.strictEqual(WhisperClient_js_1.WhisperClient.validateApiKey(''), false, 'Empty API key should fail validation');
        assert.strictEqual(WhisperClient_js_1.WhisperClient.validateApiKey('sk-short'), false, 'Short API key should fail validation');
    });
    it('should return supported formats', () => {
        const formats = WhisperClient_js_1.WhisperClient.getSupportedFormats();
    ...
    it('should return max file size', () => {
        const maxSize = WhisperClient_js_1.WhisperClient.getMaxFileSize();
    ...
    it('should initialize with any API key (validation happens at runtime)', () => {
        ...
        const whisperClient = new WhisperClient_js_1.WhisperClient({
            apiKey: 'invalid-key',
            timeout: 5000
        });
...
describe('WhisperClient Error Handling Tests', () => {
    ...
    it('should handle API errors', async () => {
        ...
        try {
            ...
            assert.ok(errorMessage.includes('rate limit') ||
                errorMessage.includes('429') ||
                errorMessage.includes('api error') ||
                errorMessage.includes('limit') ||
                errorMessage.includes('exceeded'), `Should include API error information, got: ${error.message}`);
        }
    });
    it('should handle timeout errors', async () => {
        const shortTimeoutClient = new WhisperClient_js_1.WhisperClient({
            apiKey: 'sk-1234567890123456789012345678901234567890123456',
