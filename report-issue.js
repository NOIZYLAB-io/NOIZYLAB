...
const getViscaVersion = () => {
	try {
		const extensions = execSync("code --list-extensions --show-versions").toString()
		const viscaMatch = extensions.match(/visca-ext@(\d+\.\d+\.\d+)/)
		return viscaMatch ? viscaMatch[1] : "Not installed"
	} catch (err) {
...
const createIssueUrl = (systemInfo, issueTitle) => {
	return (
		`https://github.com/visca-ai/visca-ext/issues/new?template=bug_report.yml` +
		`&title=${issueTitle}` +
		`&operating-system=${systemInfo.os}` +
		`&visca-version=${systemInfo.viscaVersion}` +
		`&system-info=${
