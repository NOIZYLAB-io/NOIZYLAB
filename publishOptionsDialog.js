...
exports.PublishOptionsDialog = void 0;
const constants = require("../common/constants");
const utils = require("../common/utils");
const deployOptionsModel_1 = require("../models/options/deployOptionsModel");
const telemetry_1 = require("../common/telemetry");
const uiConstants_1 = require("../common/uiConstants");
class PublishOptionsDialog {
    publish;
    dialog;
    optionsTab;
    disposableListeners = [];
    ...
    isResetOptionsClicked = false;
    excludeObjectTypesOptionsTab;
    excludeObjectTypesOptionsTable;
    excludeObjectTypesOptionsFlexBuilder;
    constructor(defaultOptions, publish) {
        this.publish = publish;
        this.optionsModel = new deployOptionsModel_1.DeployOptionsModel(defaultOptions);
    }
    initializeDialog() {
    ...
    initializeDeploymentOptionsDialogTab() {
        this.optionsTab?.registerContent(async (view) => {
                ...
                columns: [
                    {
                        ...
                        isReadOnly: false,
                        displayName: '',
                        headerCssStyles: uiConstants_1.cssStyles.optionsTableHeader,
                    },
                    {
            ...
            this.disposableListeners.push(this.optionsTable.onRowSelected(async (selectedRow) => {
                ...
                const displayName = this.optionsTable.dataValues[selectedRow.row][1].value;
            ...
            this.disposableListeners.push(this.optionsTable.onDataChanged((changedData) => {
                if (changedData.row !== undefined) {
                    ...
                    const displayName = this.optionsTable?.dataValues[changedData.row][1].value;
    ...
    initializeExcludeObjectTypesOptionsDialogTab() {
        this.excludeObjectTypesOptionsTab?.registerContent(async (view) => {
            ...
            this.disposableListeners.push(this.excludeObjectTypesOptionsTable.onDataChanged((changedData) => {
                const displayName = this.excludeObjectTypesOptionsTable?.dataValues[changedData.row][1].value;
                const checkboxValue = changedData.value;
    ...
    */
    async updateExcludeObjectsTable() {
    ...
    */
    execute() {
        ...
        if (this.optionsChanged) {
            telemetry_1.TelemetryReporter.sendActionEvent(telemetry_1.TelemetryViews.PublishOptionsDialog, telemetry_1.TelemetryActions.optionsChanged);
        }
    ...
    */
    async reset() {
        ...
        telemetry_1.TelemetryReporter.sendActionEvent(telemetry_1.TelemetryViews.PublishOptionsDialog, telemetry_1.TelemetryActions.resetOptions);
        await this.updateExcludeObjectsTable();
...
exports.PublishOptionsDialog = PublishOptionsDialog;
//# sourceMappingURL=publishOptionsDialog.js.map