...
async function createNewProjectFromDatabaseWithQuickpick(connectionInfo, createProjectFromDatabaseCallback) {
    ...
    // 2. Select database
    const selectedDatabase = await vscode.window.showQuickPick(dbs, { title: constants.selectDatabase, ignoreFocusOut: true });
    if (!selectedDatabase) {
        // User cancelled
        return undefined;
    }
    ...
    const folderStructure = await vscode.window.showQuickPick([constants.schemaObjectType, constants.file, constants.flat, constants.objectType, constants.schema], { title: constants.selectFolderStructure, ignoreFocusOut: true, });
    if (!folderStructure) {
        // User cancelled
        return undefined;
    }
