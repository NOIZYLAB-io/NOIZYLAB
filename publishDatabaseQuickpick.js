...
exports.getPublishDatabaseSettings = getPublishDatabaseSettings;
exports.launchPublishTargetOption = launchPublishTargetOption;
const vscode = require("vscode");
const constants = require("../common/constants");
const publishProfile_1 = require("../models/publishProfile/publishProfile");
const publishDatabaseDialog_1 = require("./publishDatabaseDialog");
 ...
 */
async function getPublishDatabaseSettings(project, promptForConnection = true) {
    ...
    const profilePicked = new Promise((resolve, reject) => {
        quickPick.onDidHide(() => {
        ...
        quickPick.onDidChangeSelection(async (items) => {
            if (items[0].label === constants.browseForProfileWithIcon) {
                const locations = await (0, publishDatabaseDialog_1.promptForPublishProfile)(project.projectFolderPath);
                if (!locations) {
                ...
                try {
                    ...
                    const profile = await (0, publishProfile_1.readPublishProfile)(publishProfileUri);
                    resolve(profile);
                }
                catch (err) {
    ...
    // 2. Select connection
    let connectionProfile = undefined;
    let dbs = undefined;
    let connectionUri;
        ...
        while (!dbs) {
            ...
            try {
                try {
                    ...
                    const firewallRuleError = azureErr;
    ...
    while (!databaseName) {
        const selectedDatabase = await vscode.window.showQuickPick(dbQuickpicks, { title: constants.selectDatabase, ignoreFocusOut: true });
        if (!selectedDatabase) {
            // User cancelled
            return;
        }
    ...
    let sqlCmdVariables = getInitialSqlCmdVariables(project, publishProfile);
    if (sqlCmdVariables.size > 0) {
        ...
        while (true) {
            ...
            const sqlCmd = await vscode.window.showQuickPick(quickPickItems, { title: constants.chooseSqlcmdVarsToModify, ignoreFocusOut: true });
            if (!sqlCmd) {
    ...
    let settings = {
        ...
        deploymentOptions: publishProfile?.options ?? await (0, utils_1.getDefaultPublishDeploymentOptions)(project),
 ...
 */
function getInitialSqlCmdVariables(project, publishProfile) {
...
async function launchPublishTargetOption(project) {
    // Show options to user for deploy to existing server or docker
    const target = project.getProjectTargetVersion();
    const name = (0, utils_2.getPublishServerName)(target);
    const logicalServerName = target === constants.targetPlatformToVersion.get("Azure SQL Database" /* SqlTargetPlatform.sqlAzure */) ? constants.AzureSqlLogicalServerName : constants.SqlServerName;
    // Options list based on target
    let options;
    if (target === constants.targetPlatformToVersion.get("Azure SQL Database" /* SqlTargetPlatform.sqlAzure */)) {
        ...
        const enablePreviewFeatures = vscode.workspace.getConfiguration(netcoreTool_1.DBProjectConfigurationKey).get(constants.enablePreviewFeaturesKey);
    ...
    const publishOption = await vscode.window.showQuickPick(options, { title: constants.selectPublishOption, ignoreFocusOut: true });
...
//# sourceMappingURL=publishDatabaseQuickpick.js.map