...
class NeuralLink {
    ...
    execute(command) {
        ...
        else if (cmd.includes('train') || cmd.includes('learn') || cmd.includes('upgrade')) {
             if (window.Rob) window.Rob.setServiceMode('OMNISCIENT');
             this.log("TRAINING: INJECTING 50TB OF OS HISTORY...");
             setTimeout(() => this.log("TRAINING COMPLETE: 100%"), 2000);
        }
        else if (cmd.includes('apple') || cmd.includes('windows') || cmd.includes('future')) {
             if (window.Rob) {
                 const fact = window.Rob.queryOmnibus(cmd);
                 if (fact) this.log(`> DATA: ${fact}`);
             }
        }
        else if (cmd.includes('service') || cmd.includes('remote')) {
             if (window.Rob) window.Rob.setServiceMode('SERVICE');
             this.log("PROTOCOL: REMOTE AI SERVICE ENABLED");
        }
