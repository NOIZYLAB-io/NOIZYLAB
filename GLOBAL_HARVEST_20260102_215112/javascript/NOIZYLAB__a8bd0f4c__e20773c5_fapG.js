...
window.addEventListener('noizy-beat', (e) => {
    ...
    // KICK: Scale Pulse
    if (type === 'kick') {
        // If Voice is Active (Glitch Intensity Up)
        const voiceGlitch = (window.AudioSys && window.AudioSys.micStream) ? 0.1 : 0;
        
        canvas.style.transform = `scale(${1.02 + voiceGlitch})`;
        canvas.style.filter = `brightness(${1.1 + voiceGlitch * 2}) hue-rotate(${voiceGlitch * 360}deg)`;
...
document.getElementById('autoDirectBtn').addEventListener('click', () => {
    ...
    const originalText = "Auto-Direct";
    btn.innerText = "PRE-COG: 0ms";
