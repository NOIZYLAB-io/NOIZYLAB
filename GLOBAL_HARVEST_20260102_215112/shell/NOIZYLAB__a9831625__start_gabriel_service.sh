...
# Kill existing instances (Fast Kill)
echo "[$(date)] 🧹 Cleaning up previous instances..."
pkill -f "portal_server.py"
pkill -f "signaling_server.py"

# Wait for cleanup (Max 2s)
for i in {1..20}; do
    if ! pgrep -f "portal_server.py" > /dev/null; then break; fi
    sleep 0.1
done
...
# Quick Poll for Signaling Port (5181)
for i in {1..50}; do
    if nc -z localhost 5181 >/dev/null 2>&1; then break; fi
    sleep 0.1
done
echo "[$(date)]    PID: $PID_SIGNAL (Port 5181 Ready)"
...
# Quick Poll for Portal Port (5173)
for i in {1..50}; do
    if nc -z localhost 5173 >/dev/null 2>&1; then break; fi
    sleep 0.1
done
echo "[$(date)]    ✅ Portal Ready on Port 5173"
