...
if [ -n "$URL" ]; then
  ...
  echo "$URL" | npx wrangler secret put GABRIEL_PORTAL_URL > /dev/null 2>&1
  
  if [ $? -eq 0 ]; then
    echo "[$(date)] ✅ INFINITY LINK ESTABLISHED. Slack Worker Updated."
    echo "[$(date)] 🎯 Target: $URL"
  else
    echo "[$(date)] ❌ FAILED TO UPDATE SLACK WORKER. Check Wrangler Auth."
    # Fallback to local logs for debugging
    npx wrangler secret list > /dev/null 2>&1
  fi
else
  echo "[$(date)] ❌ TIMEOUT: Could not find Tunnel URL in logs."
  echo "[$(date)] ⚠️  Check tunnel.log for errors."
fi
