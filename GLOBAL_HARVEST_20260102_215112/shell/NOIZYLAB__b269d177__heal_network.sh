...
for INTF in "${INTERFACES[@]}"; do
    ...
    OUTPUT=$(sudo networksetup -setMTU "$INTF" 9000 2>&1)
    if [ $? -eq 0 ]; then
        echo "✅ $INTF set to 9000"
    else
        if [[ "$OUTPUT" == *"valid MTU range"* ]]; then
            echo "⚠️  $INTF Hardware Limit Detected (Keeping Max 1500)"
        else
            echo "❌ Failed to set $INTF: $OUTPUT"
        fi
    fi
done
