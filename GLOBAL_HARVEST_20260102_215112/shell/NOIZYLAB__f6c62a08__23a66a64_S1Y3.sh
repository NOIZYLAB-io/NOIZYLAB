...
# 2. Check & Connect to The Vault (Old Mac Pro)
echo "🔌 CONNECTING TO THE VAULT (10.90.90.30)..."
python3 Audio_Unitor/Scripts/turbo_mount.py
...
# 6. Launch Quest Bridge (VR UDP Stream)
echo "🥽 ACTIVATING QUEST BRIDGE (Holo-Gabriel Link)..."
python3 Audio_Unitor/Scripts/turbo_quest_bridge.py &
BRIDGE_PID=$!

# 7. Launch Studio Hand (Auto-Repair Watchdog)
echo "🖐️  ACTIVATING STUDIO HAND (VoiceFixer Watchdog)..."
python3 Audio_Unitor/Scripts/turbo_studio_hand.py &
HAND_PID=$!

# 8. Launch Gabriel (The Brain) - Foreground
...
# Trap Ctrl+C to kill background processes
trap "kill $CARRIER_PID $SERVER_PID $SENTINEL_PID $BRIDGE_PID $HAND_PID; exit" INT
