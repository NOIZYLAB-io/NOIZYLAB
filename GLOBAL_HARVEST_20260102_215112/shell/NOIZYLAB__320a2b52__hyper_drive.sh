...
# OPTIMIZATION: Disable Spotlight Indexing on this volume (Save CPU/IO)
echo ">>> Disabling Search Indexing on HyperDrive..."
sudo mdutil -i off "/Volumes/$DRIVE_NAME"

# OPTIMIZATION: Remount with NOATIME (Don't update access times on read)
echo ">>> Remounting with 'noatime' (Latency Reduction)..."
# mount -u (update) -o noatime
sudo mount -u -o noatime "/Volumes/$DRIVE_NAME"
