...
class CortexRouter:
    ...
    def route(self, user_in, context):
        ...
        # Check specific keywords for instant trigger
        intent = None
        for key, val in self.reflex_patterns.items():
            if key in user_in_lower.split(): # Simple token check
                intent = val
                break
                
        if intent:
            latency = (time.time() - start_ts) * 1000
            print(f"⚡ [ROUTER] REFLEX TRIGGER: {intent} ({latency:.2f}ms)")
            return self.execute_reflex(intent, user_in)
            
        # 2. DEEP LAYER (LLM) - >500ms
        print(f"🧠 [ROUTER] DEEP COGNITION REQUIRED...")
        return self.cortex.think(user_in, context)
