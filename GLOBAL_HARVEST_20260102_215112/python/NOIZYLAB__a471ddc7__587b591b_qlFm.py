...
def deploy():
    ...
    cfg.system_log(f"Deploying Code to {cfg.GIT_REMOTE_URL}...", "INFO")
    try:
        # Check if remote exists
        check_git = subprocess.run(["git", "remote", "get-url", "origin"], cwd=STAGING_AREA, capture_output=True, text=True)
        if "NOIZYLAB-io" not in check_git.stdout:
            subprocess.run(["git", "remote", "remove", "origin"], cwd=STAGING_AREA, capture_output=True)
            subprocess.run(["git", "remote", "add", "origin", cfg.GIT_REMOTE_URL], cwd=STAGING_AREA, check=True)
        
        # Add, Commit, Push
        subprocess.run(["git", "add", "."], cwd=STAGING_AREA, check=True)
        subprocess.run(["git", "commit", "-m", "Miracle Deployment: Zero Friction Update"], cwd=STAGING_AREA)
        subprocess.run(["git", "push", "-u", "origin", "main"], cwd=STAGING_AREA, check=True)
