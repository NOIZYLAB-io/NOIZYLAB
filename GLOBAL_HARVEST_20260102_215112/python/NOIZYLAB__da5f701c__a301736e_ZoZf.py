...
with st.sidebar:
    ...
    online_count = sum([bool(api_key_gemini), bool(api_key_claude), bool(api_key_nvidia)])
    if online_count == 3:
        st.success("🚀 ALL 3 ENGINES ONLINE")
    elif online_count > 0:
        st.info(f"{online_count}/3 Engines Online")
    
    st.divider()
    st.write("NOIZYLAB v4.0 // TRI-ENGINE")
...
class SmartRouter:
    ...
    def get_provider_for_agent(agent):
        ...
        mapping = {
            ...
            "GABRIEL": "NVIDIA",     # NVIDIA for system ops (fast inference)
            "POPS": "Gemini",        # Fast wisdom
            "GENERAL": "NVIDIA"      # Default to NVIDIA for speed
        }
