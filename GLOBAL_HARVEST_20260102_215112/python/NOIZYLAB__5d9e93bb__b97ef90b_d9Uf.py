...
class GabrielDoctor:
    ...
    def run(self, auto_heal=False):
        ...
        if auto_heal and self.problems:
            print("\n  💉 INITIATING HEALING PROTOCOLS...")
            fixes = []
            prob_str = " ".join(self.problems).lower()
            if "offline" in prob_str: fixes.append(self.heal_network())
            if "not mounted" in prob_str: fixes.append(self.heal_mounts())
            
            for f in fixes: print(f"  - {f}")
            return {"score": self.health_score, "problems": self.problems, "fixes": fixes}
            
        print("="*50 + "\n")
        return {"score": self.health_score, "problems": self.problems, "fixes": []}
