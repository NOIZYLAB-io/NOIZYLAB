...
class TurboNetControl:
    ...
    def scan_network(self):
        """Map the physical network using ARP (Fast Mode)."""
        print("🕸️ SCANNING PHYSICAL NETWORK...")
        devices = []
        try:
            # Use -an to avoid DNS lookups (Zero Latency)
            output = subprocess.check_output(["arp", "-an"]).decode()
            for line in output.split('\n'):
                if "at" in line:
                    parts = line.split()
                    # format: ? (192.168.1.1) at aa:bb:cc:dd:ee:ff on en0 ...
                    # with -an: ? (192.168.1.1) at aa:bb:cc:dd:ee:ff ...
                    # Actually standard mac arp output is: ? (10.0.0.1) at ...
                    try:
                        ip = parts[1].strip('()')
                        mac = parts[3]
