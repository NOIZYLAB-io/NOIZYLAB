...
class SystemDoctor:
    ...
    def scan_disks(self) -> ModuleResult:
        ...
        # Get disk usage with df - use -P for POSIX portable format (no line wrapping)
        stdout, _, _ = self._run_cmd(["df", "-Ph"])
        if stdout:
            for line in stdout.strip().split("\n")[1:]:
                # POSIX format: Filesystem Size Used Avail Capacity Mounted on
                # Split on whitespace but keep mount point (may have spaces)
                parts = line.split()
                if len(parts) >= 6 and parts[0].startswith("/dev/"):
                    ...
                    # Get a cleaner name for the mount point
                    if mount == "/":
                        display_name = "Macintosh HD"
                    elif mount.startswith("/Volumes/"):
                        display_name = mount.replace("/Volumes/", "")
                    elif mount.startswith("/System/Volumes/"):
                        display_name = mount.split("/")[-1] + " (System)"
                    else:
                        display_name = mount.split("/")[-1] or mount
                    ...
                    if pct >= 95:
                        ...
                        msg = f"{pct}% used ({avail} free)"
                        fix = f"Free up space on {display_name} immediately!"
                    elif pct >= 85:
                        status = HealthStatus.WARNING
                        msg = f"{pct}% used ({avail} free)"
                        fix = f"Consider cleaning up {display_name}"
                    else:
                        status = HealthStatus.OK
                        msg = f"{avail} free / {size}"
                        fix = None
                    ...
                    module.results.append(DiagnosticResult(
                        ...
                        status=status,
                        message=msg,
                        details={"filesystem": filesystem, "mount": mount, "size": size, "used": used, "available": avail, "percent": pct},
                        fix_suggestion=fix
                    ))
