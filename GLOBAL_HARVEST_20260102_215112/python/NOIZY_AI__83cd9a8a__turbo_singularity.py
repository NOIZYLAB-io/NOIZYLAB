...
# Configuration
DEST_ROOT = cfg.ASSETS_DIR.parent # library root usually
TIME_MACHINE_ROOT = Path.expanduser(Path("~/Universal/Time_Machine"))
DB_PATH = cfg.UNIVERSE_DB_PATH
SESSION_BACKUP_DIR = TIME_MACHINE_ROOT / "Session_Backups"
...
def get_semantic_context(fpath):
    ...
    try:
        ...
        ignored = {'users', 'downloads', 'universal', 'library', 'assets', 'audio', 'image', 'video', 'text', 'desktop', 'documents', 'temp_unpack', 'scavenge_bin', 'volumes'}
        for p in parents[:2]: 
...
def run_singularity(source_dir):
    cfg.print_header("🌌 THE SINGULARITY", "MEMORY CORE ACTIVE")
    ...
    cfg.system_log(f"Engaging Singularity Engine on {len(files)} items...", "INFO")
    ...
    with ThreadPoolExecutor(max_workers=THREADS) as executor:
        ...
        for i, future in enumerate(as_completed(future_to_file)):
            ...
            if status == 1: count += 1
                ...
                errors += 1
                if errors < 10: cfg.system_log(f"Error: {msg}", "ERROR")
            if (i+1) % 100 == 0: print(f"CORE > ...processed {i+1}...", end='\r')
                
    duration = time.time() - start_t
    cfg.system_log(f"SINGULARITY COMPLETE ({duration:.2f}s)", "SUCCESS")
    print(f"CORE > Processed: {count} | Errors: {errors}")

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(f"Usage: python3 {sys.argv[0]} <source_dir>")
    else:
