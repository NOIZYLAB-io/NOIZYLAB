...
def build_global_index(target_dir=None):
    ...
    print("👁️  THE ORACLE IS AWAKENING (PARALLEL MODE)...")
    start_time = time.time()
    all_files = []
    
    if target_dir:
        print(f"🎯 Targeted Scan: {target_dir}")
        all_files.extend(scan_volume(target_dir))
    else:
        ...
        print(f"🚀 Deploying Scanners to {len(volumes)} Volumes...")
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            ...
            for future in as_completed(future_to_vol):
                ...
                try:
                    ...
                    print(f"      ✅ {vol_name}: Indexed {len(files)} files")
