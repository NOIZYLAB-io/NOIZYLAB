...
def generate_manifest(root_dir):
    print(f"{BOLD}{CYAN}👷 ENGR KEITH: BUILDING MANIFEST...{RESET}")
    ...
    print(f"   Analyzing {len(albums)} Albums...")
        ...
        for i, future in enumerate(as_completed(future_to_album)):
            ...
            if res:
                ...
                if res["issues"]:
                    ...
                    print(f"   {YELLOW}⚠ Issue in {Path(res['path']).name}: {res['issues']}{RESET}")
            
            if (i+1) % 10 == 0:
                print(f"   ...scanned {i+1}...", end='\r')

    # Save Manifest
    try:
        with open(MANIFEST_PATH, 'w') as f:
            json.dump(universe, f, indent=2)
        print(f"\n{GREEN}✨ MANIFEST GENERATED: {MANIFEST_PATH}{RESET}")
        print(f"   Albums: {universe['library']['stats']['albums']}")
        print(f"   Tracks: {universe['library']['stats']['tracks']}")
        print(f"   Issues: {universe['library']['stats']['issues']}")
    except Exception as e:
        print(f"\n{RED}❌ Manifest Failed: {e}{RESET}")
...
if __name__ == "__main__":
    ...
    if target.exists():
        ...
        print(f"{RED}No Music Library found at {target}{RESET}")
