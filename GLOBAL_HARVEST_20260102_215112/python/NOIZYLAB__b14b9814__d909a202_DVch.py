...
def main_menu():
    while True:
        ...
        print(f"   [C] 💬 CHAT WITH PARTNER (Direct Connection)")
        print(f"   [Q] 🚪 EXIT")
        ...
        elif choice == '9': run_script("turbo_memcell.py")
        elif choice == 'c':
             # Launch MemCell in Chat Mode (requires upgrade to memcell script to support CLI arg or direct mode)
             # Easier: Just run memcell and user hits 'c'. Or pass arg.
             # I will assume user knows to hit 'c' inside memcell for now, OR I can run it with input?
             # Better: Update run_script to allow subprocess interaction.
             run_script("turbo_memcell.py")
        elif choice == 'q':
