...
SORTING_RULES = {
    'IMAGES': {'.png', '.jpg', '.jpeg', '.gif', '.svg', '.bmp', '.tiff', '.psd', '.ai', '.ico', '.webp'},
    'AUDIO': {'.wav', '.mp3', '.aif', '.aiff', '.flac', '.ogg', '.m4a', '.midi', '.mid'},
    'VIDEO': {'.mp4', '.mov', '.avi', '.mkv', '.webm'},
    'CODE_PY': {'.py', '.pyw'},
    'CODE_JS': {'.js', '.ts', '.jsx', '.tsx', '.json', '.vue'},
    'CODE_WEB': {'.html', '.css', '.scss', '.less'},
    'CODE_C': {'.c', '.cpp', '.h', '.hpp', '.cs', '.java'},
    'SHELL': {'.sh', '.bash', '.zsh', '.bat', '.ps1', '.fish'},
    'CONFIG': {'.yaml', '.yml', '.toml', '.ini', '.cfg', '.conf', '.env', '.xml', '.plist'},
    'DOCS': {'.pdf', '.doc', '.docx', '.txt', '.rtf', '.md', '.markdown', '.rst'},
    'DATA': {'.csv', '.sql', '.db', '.sqlite', '.log'},
...
class NoizyCleaner:
    ...
    def clean_caches(self):
        """Aggressively delete cache files and directories."""
        self.deleted_caches = 0
        targets = {'__pycache__', '.pytest_cache', '.mypy_cache', '.DS_Store'}
            ...
            for d in dirs:
                if d in targets:
                    ...
                    try:
                        ...
                        print(f"    🔥 Incinerated: {dir_path.relative_to(ROOT_DIR)}")
                        self.deleted_caches += 1
                    except Exception as e:
                        print(f"    ⚠️ Could not delete {dir_path}: {e}")

            # Delete cache files
            for f in files:
                if f in targets or f.endswith('.pyc'):
                    file_path = current_path / f
                    try:
                        file_path.unlink()
                        print(f"    🔥 Incinerated: {file_path.relative_to(ROOT_DIR)}")
                        self.deleted_caches += 1
                    except Exception as e:
    ...
    def execute(self):
        ...
        print(f"   🔥 Caches Nuked: {self.deleted_caches}")
