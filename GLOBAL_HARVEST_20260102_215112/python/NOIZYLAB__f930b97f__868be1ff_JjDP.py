...
class MemCell:
    ...
    def extract_keywords(self, content):
        ...
        # Note: Original content needed for this as we stripped clean_content
        explicit_tags = [w.lstrip('#') for w in content.split() if w.startswith('#')]
        candidates.extend(explicit_tags)
    ...
    def recall(self, query):
        ...
        # Search content, tags, author, and event_type
        self.cursor.execute("""
    ...
    def inject_omniscience(self, context_window=2000):
        ...
        # 1. Recent Events (Short Term Memory)
        self.cursor.execute("SELECT author, content, vibe_score FROM memory_events ORDER BY id DESC LIMIT 10")
        recent = self.cursor.fetchall()
        if recent:
            context.append("---[ RECENT MEMORY ]---")
            for r in reversed(recent):
                context.append(f"{r[0]}: {r[1]} (Vibe: {r[2]})")

        # 2. Active Cells (Topics)
