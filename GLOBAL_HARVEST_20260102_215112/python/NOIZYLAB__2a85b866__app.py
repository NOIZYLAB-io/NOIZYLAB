                ...
                for i, (path, name, vol, size, mtype, mtime) in enumerate(res):
                    with cols[i % 4]:
                        with st.container(border=True):
                            ...
                            # MAXFLOW VIDEO FIX - Proper validation
                            try:
                                from pathlib import Path as P
                                file_path = P(path)
                                if file_path.exists() and file_path.stat().st_size > 0:
                                    if mtype == 'video':
                                        # Video formats Streamlit supports
                                        supported_video = ('.mp4', '.webm', '.ogg', '.mov', '.m4v')
                                        if file_path.suffix.lower() in supported_video:
                                            st.video(str(file_path))
                                        else:
                                            st.caption(f"⚠️ Format: {file_path.suffix}")
                                    elif mtype == 'image':
                                        supported_img = ('.png', '.jpg', '.jpeg', '.gif', '.webp', '.bmp')
                                        if file_path.suffix.lower() in supported_img:
                                            st.image(str(file_path))
                                        else:
                                            st.caption(f"🖼️ {file_path.suffix}")
                                    elif mtype == 'audio':
                                        supported_audio = ('.mp3', '.wav', '.ogg', '.flac', '.m4a', '.aac')
                                        if file_path.suffix.lower() in supported_audio:
                                            st.audio(str(file_path))
                                        else:
                                            st.caption(f"🎵 {file_path.suffix}")
                                else:
                                    st.caption("📍 File moved/missing")
                            except Exception as e:
