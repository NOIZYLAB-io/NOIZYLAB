...
def check_tool_installed(tool_key):
...
def list_tools():
    ...
    for key, tool in AI_AUDIO_TOOLS.items():
        installed = check_tool_installed(key)
...
def separate_stems(input_file, tool="demucs"):
    ...
    brain.log_event(brain.covenant_id, "AI_AUDIO", f"Stem separation started: {input_path.name}", vibe=80, author="AUDIO_AI")
    
    cfg.ensure_dirs([OUTPUT_DIR])
    
    if tool == "demucs":
        if not check_tool_installed("demucs"):
        ...
        try:
            ...
            brain.log_event(brain.covenant_id, "AI_AUDIO_COMPLETE", f"Stems created for: {input_path.name}", vibe=100, author="AUDIO_AI")
        except subprocess.CalledProcessError as e:
            cfg.system_log(f"Demucs failed: {e}", "ERROR")
            
    elif tool == "spleeter":
        if not check_tool_installed("spleeter"):
...
def transcribe_audio(input_file, model="base"):
    ...
    brain.log_event(brain.covenant_id, "AI_TRANSCRIBE", f"Transcription started: {input_path.name}", vibe=80, author="WHISPER_AI")
    
    if not check_tool_installed("whisper"):
    ...
    try:
        ...
        brain.log_event(brain.covenant_id, "AI_TRANSCRIBE_COMPLETE", f"Transcript created: {input_path.name}", vibe=100, author="WHISPER_AI")
    except subprocess.CalledProcessError as e:
...
def repair_audio(input_file):
    ...
    brain.log_event(brain.covenant_id, "AI_REPAIR", f"Repair started: {input_path.name}", vibe=80, author="VOICEFIXER")
    ...
    try:
        ...
        vf = VoiceFixer()
        vf.restore(input=str(input_path), output=str(output_path), cuda=False, mode=0) # cuda=False for Mac (uses CPU or MPS if supported by lib)
        
        cfg.system_log(f"✨ Repaired Audio saved to: {output_path}", "SUCCESS")
        brain.log_event(brain.covenant_id, "AI_REPAIR_COMPLETE", f"Repair finished: {input_path.name}", vibe=100, author="VOICEFIXER")
