...
def test_response_cache():
    ...
    cache = ResponseCache()
    unique_key = f"test_prompt_{time.time_ns()}"  # Truly unique
    
    # Miss
    r1 = cache.get(unique_key)
    if r1 is not None:
        return TestResult("ResponseCache", False, "Should miss on new key")
    
    # Set
    cache.set(unique_key, "test_response")
    
    # Hit
    r2 = cache.get(unique_key)
    if r2 != "test_response":
...
async def benchmark_cache_hit():
    """Benchmark cache hit performance using reflex responses."""
    from gabriel_brain import GabrielBrain

    brain = GabrielBrain()
    await brain.wake_up()

    # Use a status query (reflex, gets cached)
    test_query = "status check benchmark test"
    
    # First call (sets cache)
    first_result = await brain.process_input(test_query)

    # Subsequent calls (should hit cache)
    latencies = []
    for _ in range(20):
        start = time.perf_counter()
        result = await brain.process_input(test_query)
        latencies.append((time.perf_counter() - start) * 1000)

    avg = mean(latencies)
    source = result["meta"]["source"]
    passed = avg < 0.5  # Cache hits should be <0.5ms

    return TestResult(
        "Benchmark:CacheHit",
        passed,
        f"avg={avg:.2f}ms, source={source}",
        avg,
    )
