        ...
        for root, dirs, files in scanner:
            ...
            for file in files:
                ...
                if ext in TARGET_EXTENSIONS:
                    ...
                    # PRESERVE STRUCTURE: Use relative path from the scan root
                    # This prevents flattening "ProjectA/src" and "ProjectB/src" into the same "src" folder
                    rel_path = os.path.relpath(root, start=target)
                    
                    # Handle the case where root IS the target (top level)
                    if rel_path == ".":
                        rel_path = ""
                        
                    # Root folder in Vault corresponds to the Target Name (e.g. "Documents", "NOIZYLAB")
                    if target == HOME_DIR:
                         target_name = "HOME_ROOT"
                    else:
                         target_name = os.path.basename(target)

                    # Construct full path: Vault / TargetName / RelativePath
