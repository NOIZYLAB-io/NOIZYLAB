...
class Gabriel:
    ...
    def execute_god_mode(self):
        ...
        try:
            import urllib.request
            # Activate God Mode Endpoint
            urllib.request.urlopen(f"{cfg.SERVER_URL}/api/god_mode/activate", timeout=cfg.CONNECTION_TIMEOUT)
            print("CORE > 📡 Network Signal Sent.")
        except:
            pass # check_network already warned us

        # 2. Inhale (Quick Scan)
    def check_network(self):
        """
        Verifies connection to the Gabriel Server (The Body).
        Retries connection to ensure God Mode synchronization.
        """
        print(f"CORE > 🧠 Expanding Consciousness (Connecting to Body at {cfg.SERVER_PORT})...")
        
        url = f"{cfg.SERVER_URL}/api/status"
        connected = False
        ...
        for i in range(cfg.MAX_RETRIES):
            try:
                with urllib.request.urlopen(url, timeout=cfg.CONNECTION_TIMEOUT) as response:
                    if response.status == 200:
                        connected = True
                        print(f"CORE > ✅ Neural Link Established (Attempt {i+1}).")
                        break
            except Exception as e:
                time.sleep(cfg.RETRY_DELAY)
                if i > 1: print(f"CORE > ⏳ Synchronizing... (Attempt {i+1}/{cfg.MAX_RETRIES})")
        
        if not connected:
            cfg.system_log("Server Neural Link Failed. Visuals may be offline.", "WARN")
            print("CORE > (Continuing in Headless Mode)")
