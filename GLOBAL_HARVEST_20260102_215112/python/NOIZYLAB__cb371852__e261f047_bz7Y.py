...
"""
DR. GABRIEL ALMEIDA - OMEGA VOICE SYSTEM (V12)
Features: Gemini Director Mode (AI Analysis), Polyglot, WordNet, Emotion Awareness
"""
...
FX = {
    ...
    "echo":      "aecho=0.8:0.9:1000:0.3,loudnorm",
    "radio":     "highpass=f=200,lowpass=f=3000,compand,loudnorm",
    "none":      "loudnorm"
}
...
def analyze_with_director(text):
    ...
    Analyzes text context to determine the best emotion and FX.
    """
    ...
    try:
        ...
        Determine:
        1. Emotion/Style (choose one: weathered, bright, warm, dark, echo, radio)
        2. Language Code (en, fr, es, it, ja, zh) - only if text is clearly foreign.
...
def speak_sync(text, lang="en", emotion="weathered", use_director=False):
    ...
    try:
        if use_director:
            ...
            # Only override if director found something interesting, otherwise keep defaults
            if d_emotion != "weathered": emotion = d_emotion
...
class H(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if p.path == "/speak":
            ...
            # If 'director=true' is passed, or if we want it default, we can toggle here
        ...
        # 📖 DEFINE API
            ...
            self._json({**CHARACTER, "languages": list(VOICES.keys()), "director_mode": bool(GEMINI_API_KEY)})
...
if __name__ == "__main__":
    print(f"\n⚡ GABRIEL OMEGA - POLYGLOT VOICE SERVER (V12)")
    print(f"🧠 Director Mode: {'ENABLED' if GEMINI_API_KEY else 'DISABLED'}")
    print(f"🌍 Languages: {list(VOICES.keys())}")
    
    # Startup Sound
    speak_sync("Gabriel system online. Director mode active.", "en")
