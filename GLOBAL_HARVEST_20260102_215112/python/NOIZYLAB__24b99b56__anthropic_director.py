...
# NOIZYLAB ANTHROPIC DIRECTOR v3.0
# "The Auteur" Module: Advanced AI Reasoning & Creative Direction
# OPTIMIZED: MemCell v3.0 Context, Omni-Personas, Vibe Sync Architecture (SHIRL/ENGR)

print(">>> [NOIZYLAB DIRECTOR v3.0] INITIALIZING...")
...
# Import Cortex after header
try:
    from noizy_cortex import analyze_track_advanced
except ImportError:
    pass # Let it fail later if needed, or we just want the header for now
...
def get_claude_direction(audio_file, persona="DIRECTOR"):
    ...
    # v3.0 INTELLIGENCE: TEMPORAL OVERLAP & VIBE SYNC
    overlap = memory_core.analyze_temporal_overlap()
    vibe = overlap["overlap_status"]
    
    system_prompt = f"""
    YOU ARE THE DIRECTOR (v3.0).
    CURRENT SYSTEM VIBE: {vibe}
    
    RELATIONSHIPS:
    - SHIRL (Time/Logic) is your Producer.
    - ENGR (Tech/Ops) is your Technical Director.
    
    MISSION:
    1. SYNTHESIZE a MASTERPIECE from the input.
    2. BE SPECIFIC, RADICAL, AND PERFECT.
    3. ALIGN your output with the current VIBE: {vibe}.
    4. NO LATENCY. DIRECT ANSWER.
