...
def main():
    ...
    if not ingest_pool:
        ...
        print(f"Subject: MemCell Sweep: 0 new, 0 clusters, steady-state (window {WINDOW_START.strftime('%H:%M')} to {NOW.strftime('%H:%M')})")
        print("-" * 80)
        print("OVERLAP MAP (DELTA)")
        print("(No Changes)")
        print("-" * 80)
        print("GOLD CONFIDENCE UPDATES")
        print(">> INCREMENTING CONFIDENCE FOR ALL ACTIVE GOLD RECORDS...")
    ...
    for cid, members in clusters.items():
        ...
        # 1. DRIFT RISK & CONFIDENCE
        drift_risk = "LOW"
        confidence = 0
        
        if classification == "EXACT_DUPE":
            drift_risk = "LOW"
            confidence = 90
        elif classification == "STRONG_OVERLAP":
            drift_risk = "MED"
            confidence = 75
            ...
            drift_risk = "LOW"
            confidence = 80 # Baseline for unique
        ...
        if any(x in text_lower for x in ['todo', 'fix', 'urgent', 'error', 'fail']):
            action_flag = "ACTION"
        elif (NOW - anchor['created_at'].astimezone()).days > 30:
            action_flag = "ARCHIVE"
        ...
        # Trigger Logic: Actionability = Action OR Confidence >= 70
        is_actionable = action_flag == "ACTION"
        is_high_conf = confidence >= 70
    ...
    # E) ACTION QUEUE GENERATION (TASK SPINE SCHEMA)
    action_queue_path = DB_PATH.parent / "ACTION_QUEUE.md"
    
    # Filter based on new trigger logic (already marked in Meta if triggered, or re-eval here)
    # We added 'Trigger' to Meta for those that qualified.
    action_items = [g for g in gold_corpus if 'Trigger' in g['Meta']]
    
    if action_items:
        with open(action_queue_path, 'w') as f:
            f.write("# ⚡ GABRIEL ACTION QUEUE (TASK SPINE)\n")
            f.write(f"Updated: {NOW.strftime('%Y-%m-%d %H:%M')}\n\n")
            
            # Table Header
            f.write("| Task ID | Objective | Trigger | Priority | Owner | Status | Review |\n")
            f.write("|---|---|---|---|---|---|---|\n")
            
            for item in action_items:
                # Derive fields
                task_id = f"TSK_{item['GOLD_ID'][5:]}"
                parent_id = item['GOLD_ID']
                ...
                # Priority Logic
                prio = "Standard"
                if "URGENT" in item['Body'] or "RISK" in item['Tags']: prio = "Critical"
                elif "OPTIMIZATION" in item['Tags']: prio = "High"
                
                status = "Queued"
                review = "Daily"
                
                # Table Row
                f.write(f"| Only Ref (See Below) | **{objective}** | {trigger} | {prio} | {owner} | {status} | {review} |\n")
            
            f.write("\n## 📋 Task Details\n")
            for item in action_items:
                 task_id = f"TSK_{item['GOLD_ID'][5:]}"
                 f.write(f"### {task_id}: {item['Title']}\n")
                 f.write(f"- **Parent GOLD ID**: `{item['GOLD_ID']}`\n")
                 f.write(f"- **Body**: {item['Body']}\n")
                 f.write(f"- **Trigger**: {item['Meta']['Trigger']}\n")
                 f.write(f"- **Tags**: {', '.join(item['Tags'])}\n\n")

        print(f"ACTION QUEUE: Generated {len(action_items)} Task Spines in {action_queue_path.name}")
    else:
        # Create empty or clear if no actions
