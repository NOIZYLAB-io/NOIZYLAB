...
LOCAL_MIRROR = "/Users/m2ultra/Audio_Unitor/Staging_Area"
VAULT_ROOT = "/Volumes/6TB/Audio_Universe"
PROCESSED_ARCHIVES = "/Volumes/6TB/Audio_Universe/Processed_Archives"
...
def organize_folder(source, destination, dry_run=True, level=0):
    ...
    for item in source_path.iterdir():
        ...
        if item.is_file() and is_archive(item):
            ...
            if not dry_run:
                ...
                if extract_archive(item, temp_extract_dir):
                    ...
                    # Cleanup
                    item_dest = Path(PROCESSED_ARCHIVES)
                    ...
                    try:
                        ...
                        print(f"   ↳ Archive archived to {PROCESSED_ARCHIVES}")
        ...
        if item.is_file() and is_audio_file(item):
            ...
            if item.parent == source_path and level == 0:
                 ...
                 final_dest_folder_vault = Path(VAULT_ROOT) / category / "Loose_Files"
                 final_dest_folder_local = Path(LOCAL_MIRROR) / category / "Loose_Files"
            else:
                 final_dest_folder_vault = Path(VAULT_ROOT) / category / clean_parent
                 final_dest_folder_local = Path(LOCAL_MIRROR) / category / clean_parent

            final_vault_path = final_dest_folder_vault / item.name
            final_local_path = final_dest_folder_local / item.name
            
            print(f"{'[LIVE]' if not dry_run else '[DRY]'} Move: {item.name} -> VAULT/{category}/... -> ALIAS/{category}/...")
            
            if not dry_run:
                try:
                    # 1. Create Vault Directory
                    final_dest_folder_vault.mkdir(parents=True, exist_ok=True)
                    
                    # 2. Move File to Vault
                    shutil.move(str(item), str(final_vault_path))
                    
                    # 3. Create Local Directory
                    final_dest_folder_local.mkdir(parents=True, exist_ok=True)
                    
                    # 4. Create Symlink (Local -> Vault)
                    # Remove if exists (to avoid error)
                    if final_local_path.exists() or final_local_path.is_symlink():
                        final_local_path.unlink()
                        
                    os.symlink(str(final_vault_path), str(final_local_path))
    ...
    if level == 0 and items_moved > 0 and not dry_run:
        notify_user("Audio Unitor", f"Processed {items_moved} files to Vault + Aliases.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dest", "-d", help="Ignored in Zero Footprint Mode (uses Vault)", default="")
    parser.add_argument("--live", action="store_true", help="Actually move files (disable simulation)")
    
    args = parser.parse_args()
    
    print("MUSIC UNITOR: ZERO FOOTPRINT PROTOCOL")
    ...
    if not args.source:
        ...
        # We ignore destination arg because we have hardcoded Vault logic now
        organize_folder(args.source, VAULT_ROOT, dry_run=not args.live)
