...
def process_atom(src_path):
    ...
    try:
        ...
        # Name handling (Force .wav for Audio, keep others)
        is_audio = p.suffix.lower() in AUDIO_EXTS
        dest_name = p.stem + ".wav" if is_audio else p.name
        dest_path = dest_dir / dest_name
        
        # Collision
        if dest_path.exists():
            ts = int(time.time_ns())
            dest_name = f"{p.stem}_{ts}{'.wav' if is_audio else p.suffix}"
            dest_path = dest_dir / dest_name
