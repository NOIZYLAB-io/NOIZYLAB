...
def run_organizer(source_dir):
    print(f"{BOLD}🏛️  TURBO ORGANIZER: THE ARCHITECT{RESET}")
    print(f"   Source: {source_dir}")
    print(f"   Dest:   {DEST_ROOT}")
    
    src = Path(source_dir)
    if not src.exists():
        print(f"   {RED}❌ Source not found.{RESET}")
        return

    files_to_move = []
    print("   Scanning chaos...")
    ...
    print(f"   Found {len(files_to_move)} items. Constructing Order...")
    ...
    with ThreadPoolExecutor(max_workers=THREADS) as executor:
        ...
        for i, future in enumerate(as_completed(future_to_file)):
            ...
            if (i+1) % 100 == 0:
                print(f"   ...sorted {i+1}/{len(files_to_move)}...", end='\r')

    duration = time.time() - start_time
    rate = moved_count / duration if duration > 0 else 0
    
    print(f"\n{GREEN}✨ ARCHITECTURE COMPLETE ({duration:.2f}s){RESET}")
    print(f"   Rate:   {rate:.1f} files/sec")
    print(f"   Sorted: {moved_count}")
