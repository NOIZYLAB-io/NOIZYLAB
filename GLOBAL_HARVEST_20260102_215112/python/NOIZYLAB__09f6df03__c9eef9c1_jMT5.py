...
@app.route('/')
def index():
    """Root endpoint - redirect to universal portal"""
    return send_from_directory(MC96_MISSION_CONTROL_PATH, 'portal.html')

@app.route('/portal')
@app.route('/portal/')
def universal_portal():
    """Universal Portal - accessible from any device"""
    return send_from_directory(MC96_MISSION_CONTROL_PATH, 'portal.html')

@app.route('/api/info')
def api_info():
    """API info - ultra-fast cached banner"""
    cached = cache.get('api_info')
    if cached:
        return cached
    ...
    cache.set('api_info', result, ttl_seconds=60)
    return result
...
@app.route('/api/nodes')
def get_nodes():
    """Get all nodes - for MC96_MISSION_CONTROL"""
    if GRAPH_DATA:
...
@app.route('/api/links')
def get_links():
    """Get all links/edges - for MC96_MISSION_CONTROL"""
    if GRAPH_DATA:
