    ...
    def main_loop(self):
        ...
        while True:
            ...
            if choice == '1':
                ...
                if self.spirit:
                    print("  👁️  Opening Neural Stream...")
                    # Spirit runs in bg, we can just print status
                    print("  Spirit is actively broadcasting thoughts.")
                else:
                    print("  Spirit not loaded.")
                input("  [Press Enter to return]")
            
            elif choice == "8":
                if self.vision:
                    print("  👁️  ENTERING MATRIX MODE...")
                    self.vision.animate(10) # Run for 10s
                else:
                    print("  Vision system offline.")

            elif choice == "9":
                if self.sonic:
                    self.sonic.start_session()
                else:
                    print("  Sonic Core offline.")
            
            elif choice == "10":
                self.run_player_interface() # This was originally 9, now 10
            elif choice == '11': # This was originally 10, now 11
                self.dashboard.generate_live_report()
                input("\nPress Enter to continue...")
            elif choice == '12': # This was originally 11, now 12
