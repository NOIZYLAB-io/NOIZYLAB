...
async def handle_hardware_stats(request):
    ...
    if real_data:
         return web.json_response({"status": "success", "source": "sensors", "data": real_data})
    
    # Fallback to simulated/cached
    sim_data = await hardware.get_thermal_pressure()
    return web.json_response({"status": "success", "source": "simulation", "data": sim_data})
...
async def handle_signaling(request):
    ...
    try:
        ...
        # print(f"[SIGNALING] Client disconnected. Total: {len(signaling_connections)}")
