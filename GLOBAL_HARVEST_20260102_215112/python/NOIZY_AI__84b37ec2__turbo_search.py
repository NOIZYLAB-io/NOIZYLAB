...
def search_db(query, limit=50):
    if not DB_PATH.exists():
        print(f"{YELLOW}CORE > ❌ Universe Database not found. Run Turbo Indexer first.{RESET}")
        return

    conn = sqlite3.connect(str(DB_PATH))
    c = conn.cursor()
    
    print(f"{BOLD}CORE > 🔎 Searching Universe DB for: {CYAN}{query}{RESET}")
    
    # Try Hash Search first (Exact Dedup)
    if len(query) == 64: # SHA256 length
        print(f"CORE > Detected Hash Query...")
    ...
    if not results:
        print(f"CORE > ⚪ No matches found.")
        return

    print(f"CORE > ✅ Found {len(results)} matches:\n")
    ...
    if len(results) == limit:
        print(f"\nCORE > ... (Showing first {limit} results)")
