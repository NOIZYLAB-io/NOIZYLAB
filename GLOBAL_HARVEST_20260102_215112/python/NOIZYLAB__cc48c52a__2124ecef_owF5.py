...
def check_jumbo_frames():
    ...
    try:
        ...
        if "9000" in output:
            print_status("JUMBO FRAMES ACTIVE (MTU 9000)", CHECK)
            print_status("-> Optimized for DGS-1210-10 & Large Data Transfers.", INFO)
        else:
            print_status(f"STANDARD MTU DETECTED: {output}", WARN)
            print_status("-> Run 'networksetup -setMTU en0 9000' to enable if supported.", INFO)
            
    except Exception as e:
        print_status(f"Error checking MTU: {e}", FAIL)

def main():
    print("========================================")
    print("🚀 TURBO NETWORK DIAGNOSTIC v3.0") # Keeping original version as no explicit instruction to change
    print("========================================")
