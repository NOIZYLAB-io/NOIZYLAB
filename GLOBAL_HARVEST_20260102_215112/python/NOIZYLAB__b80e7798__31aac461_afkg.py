...
# MODULE: 🎹 PRO STUDIO (DAW & UAD COMMANDER)
    ...
    if not os.path.exists("NOIZYLAB_DB/visual_index.db"):
        ...
        if not df.empty:
            # Tool Filter
            tool = st.selectbox("FILTER WORKSTATION", ["ALL", "LOGIC PRO X", "FILMORA", "REAPER", "PRO TOOLS", "FMOD", "UNITY", "ABLETON", "RESOLVE"])
...
with st.sidebar:
    st.divider()
    st.caption("GABRIEL DOCTOR")
    try:
        c1, c2 = st.columns(2)
        with c1:
            if st.button("RUN DIAGNOSTICS"):
                st.toast("Running System Scan...")
                res = subprocess.run(["python3", "GABRIEL_UNIFIED/core/gabriel_doctor.py"], capture_output=True, text=True)
                if "SYSTEM HEALTH: 100%" in res.stdout:
                    st.success("100% HEALTHY")
                else:
                    st.error("ISSUES DETECTED")
                    st.code(res.stdout[-300:])
        with c2:
            if st.button("AUTO-HEAL WORLD"):
                 st.toast("Initiating Healing Protocols...")
                 res = subprocess.run(["python3", "GABRIEL_UNIFIED/core/gabriel_doctor.py", "--heal"], capture_output=True, text=True)
                 st.info("Healing Complete.")
    ...
    if st.button("RECAST NET (Deep Drill)"):
         subprocess.Popen(["python3", "GABRIEL_UNIFIED/core/visual_scanner.py"])
         st.toast("Deep Drill Initiated...")
