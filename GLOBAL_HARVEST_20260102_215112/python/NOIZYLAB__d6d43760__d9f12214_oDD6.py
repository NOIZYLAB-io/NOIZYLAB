...
class ZeroLatencyHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # API: MemCell Graph (for Neural Vis)
        if self.path == '/api/memcell/graph':
            nodes = []
            links = []
            
            # Nodes
            for cid, cell in self.rmt.memcell.cells.items():
                nodes.append({
                    "id": cid,
                    "label": cell.subject,
                    "category": cell.category.value,
                    "persona": cell.persona_affinity.value,
                    "val": cell.access_count
                })
            
            # Links
            seen_links = set()
            for cid, cell in self.rmt.memcell.cells.items():
                for rel_id in cell.related_cells:
                    link_id = tuple(sorted((cid, rel_id)))
                    if link_id not in seen_links:
                        links.append({"source": cid, "target": rel_id, "type": "relation"})
                        seen_links.add(link_id)
            
            for ov in self.rmt.memcell.overlaps:
                if len(ov.cells_involved) >= 2:
                    src = ov.cells_involved[0]
                    tgt = ov.cells_involved[1]
                    link_id = tuple(sorted((src, tgt)))
                    if link_id not in seen_links:
                         links.append({"source": src, "target": tgt, "type": "overlap", "strength": ov.strength})
                         seen_links.add(link_id)

            self._respond_json({"nodes": nodes, "links": links})
            return
