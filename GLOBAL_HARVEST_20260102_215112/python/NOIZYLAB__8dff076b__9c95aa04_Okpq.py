...
READ_SIZE = 10 * 1024 * 1024  # 10MB DEPTH

# MASTER BFA SIGNATURES (V9 Expanded)
KNOWN_PRODUCTS = {
...
def log(msg):
    # Direct print for worker threads (buffered, but works for simple logging)
    print(msg)
    sys.stdout.flush()
...
def extract_best_name_and_folder(filepath):
    try:
        with open(filepath, 'rb') as f:
            ...
            # 2. CONTEXT AWARENESS (Parent & Grandparent)
            ...
            # Check Grandparent (e.g., LibName / Instruments / file.nki)
            for pattern, product_folder in KNOWN_PRODUCTS.items():
                if re.search(pattern, grandparent_dir, re.IGNORECASE):
                    internal_name = _heuristic_name(data)
                    return product_folder, internal_name
...
def _heuristic_name(data):
    # Optimize: limit search window if data is huge, or just run regex
    matches = []
    # Only scan first 50KB for heuristic names if we didn't find a signature? 
    # No, pattern matching needs the whole thing.
...
def process_volume_parallel(root_dir, active_execute=False, nuke_mode=False):
    log(f"TURBO SCAN V9: ULTRA-DEEP SWARM")
    log(f"CPUs: {MAX_WORKERS}")
    log(f"Scan Depth: {READ_SIZE/1024/1024:.0f} MB")
    log("-" * 60)
    ...
    log(f"Targeting {len(files_to_process)} NKI files.")
    ...
    with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(futures):
            ...
            # Reduce verbosity slightly or print everything? User wants SPEED.
            # Printing can block. Let's just print.
            log(msg)
            if success:
                success_count += 1
            else:
    ...
    log("-" * 60)
    log(f" MISSION COMPLETE. Time: {duration:.2f}s")
