import streamlit as st
import google.generativeai as genai
import os
import json
from datetime import datetime
from pathlib import Path

# --- PAGE CONFIGURATION (Real UI) ---
st.set_page_config(layout="wide", page_title="NOIZYLAB PORTAL v3.0", page_icon="🎛️")

# --- THE VOID: AESTHETIC ENGINE ---
st.markdown("""
<style>
    /* MAIN VOID */
    .stApp {
        background-color: #000000;
        color: #ffffff;
        font-family: 'Courier New', Courier, monospace;
    }
    
    /* SIDEBAR */
    section[data-testid="stSidebar"] {
        background-color: #0a0a0a;
        border-right: 1px solid #333;
    }
    
    /* INPUTS */
    .stTextInput > div > div > input, .stTextArea > div > div > textarea {
        background-color: #111;
        color: #0f0; /* Matrix Green Text */
        border: 1px solid #333;
    }
    
    /* BUTTONS */
    .stButton > button {
        background-color: #222;
        color: #fff;
        border: 1px solid #555;
        border-radius: 0px; /* Brutalist */
        font-weight: bold;
        transition: all 0.3s ease;
    }
    .stButton > button:hover {
        background-color: #fff;
        color: #000;
        border-color: #fff;
    }
    
    /* HEADERS */
    h1, h2, h3 {
        color: #fff !important;
        letter-spacing: -1px;
    }
    
    /* TABS */
    .stTabs [data-baseweb="tab-list"] {
        gap: 20px;
    }
    .stTabs [data-baseweb="tab"] {
        color: #888;
    }
    .stTabs [data-baseweb="tab"][aria-selected="true"] {
        color: #fff;
        border-bottom-color: #fff;
    }
</style>
""", unsafe_allow_html=True)


# --- CONFIGURATION & CONSTANTS ---
AGENTS = {
    "GABRIEL": {"role": "System Bridge & Messenger", "traits": ["reliable", "fast", "connected"], "icon": "🌐"},
    "SHIRL": {"role": "Business Operations Manager", "traits": ["organized", "efficient", "warm"], "icon": "💼"},
    "POPS": {"role": "Creative Director", "traits": ["artistic", "experienced", "quality-focused"], "icon": "🎨"},
    "ENGR_KEITH": {"role": "Technical Engineering Lead", "traits": ["precise", "methodical", "thorough"], "icon": "🔧"},
    "DREAM": {"role": "Strategic Visionary", "traits": ["imaginative", "strategic", "ambitious"], "icon": "🔮"},
    "HEX": {"role": "Visual Architect (Web & Art)", "traits": ["aesthetic", "bold", "pixel-perfect"], "icon": "💅"}
}

# --- SIDEBAR: SETTINGS ---
with st.sidebar:
    st.header("🔌 SYSTEM CORE")
    api_key = st.text_input("Enter Gemini API Key", type="password")
    if api_key:
        genai.configure(api_key=api_key)
        st.success("SYSTEM ONLINE")
    else:
        st.warning("ENTER KEY TO INITIALIZE")
    
    st.divider()
    st.write("NOIZYLAB v3.0 // POWERED BY GOOGLE DEEPMIND")
    
    # SYSTEM AGENTS
    st.sidebar.markdown("### 🧬 NOIZY TEAM")
    selected_agent = st.sidebar.selectbox(
        "Select Active Agent",
        list(AGENTS.keys())
    )
    
    agent_data = AGENTS[selected_agent]
    st.sidebar.info(f"**{agent_data['icon']} {selected_agent}**\n\n{agent_data['role']}")


# --- DATABASE (MemCells) ---
DB_PATH = "noizylab_memory.json"

def init_db():
    if not os.path.exists(DB_PATH):
        with open(DB_PATH, "w") as f:
            json.dump([], f)

def load_memories():
    init_db()
    try:
        with open(DB_PATH, "r") as f:
            return json.load(f)
    except:
        return []

def save_memory(agent, input_text, output_text, type="TEXT"):
    memories = load_memories()
    new_memory = {
        "timestamp": str(datetime.now()),
        "agent": agent,
        "type": type,
        "input": input_text,
        "output": output_text
    }
    memories.append(new_memory)
    with open(DB_PATH, "w") as f:
        json.dump(memories, f, indent=4)
    return new_memory

# --- MAIN FUNCTIONS ---

# --- GENIUS ARCHITECTURE: VECTOR RETRIEVAL & SELF-CORRECTION ---

def get_context(query, threshold=0.3):
    """
    Simulates Vector Retrieval (RAG) using simple keyword matching for now.
    In a full production scale, this would connect to Pinecone/Weaviate.
    """
    memories = load_memories()
    if not memories:
        return ""
    
    # 0 Latency "Vector" Search (Keyword Match Simulation)
    relevant_chunks = []
    query_tokens = set(query.lower().split())
    
    for mem in memories:
        mem_tokens = set(str(mem['input']).lower().split() + str(mem['type']).lower().split())
        # Jaccard Similarity as a proxy for Vector Distance
        intersection = len(query_tokens.intersection(mem_tokens))
        union = len(query_tokens.union(mem_tokens))
        if union > 0:
            score = intersection / union
            if score > threshold:
                relevant_chunks.append(f"[{mem['type']}] {mem['output'][:200]}...") # Inject snippet
    
    if not relevant_chunks:
        return "No specific past context found."
    return "\n".join(relevant_chunks[:3]) # Top 3 chunks

def recursive_critic(input_text, draft_response, agent_name):
    """
    The 'Critic' AI that reviews the draft for accuracy and tone.
    """
    critic_model = genai.GenerativeModel('gemini-1.5-flash')
    critic_prompt = f"""
    CRITIQUE THIS RESPONSE.
    Role: Senior Editor & Fact Checker.
    Agent Persona: {agent_name}
    User Input: {input_text}
    Draft Response: {draft_response}
    
    Task: Rate accuracy (0-100) and Tone Match (0-100).
    If Score < 90, rewrite the response to be PERFECT.
    If Score >= 90, output "PASS".
    """
    result = critic_model.generate_content(critic_prompt).text
    if "PASS" in result:
        return draft_response
    else:
        return result # Return the optimized version

def analyze_prompt(text_input, agent_name="GABRIEL"):
    """
    GENIUS MODE:
    1. Vector Retrieval (Context Injection)
    2. Dynamic Role Assignment
    3. Recursive Self-Correction
    """
    agent_data = AGENTS.get(agent_name, AGENTS["GABRIEL"])
    
    # 1. VECTOR RETRIEVAL
    retrieved_knowledge = get_context(text_input)
    
    # 2. DYNAMIC CONTEXT INJECTION
    system_prompt = f"""
    DYNAMIC_ROLE: {agent_name} // {agent_data['role']}
    TRAITS: {', '.join(agent_data['traits'])}
    PHILOSOPHY: GORUNFREE - One command = everything done.
    
    RETRIEVED_MEMCELLS (Context):
    {retrieved_knowledge}
    
    TASK: Convert raw idea into technical execution.
    """
    
    model = genai.GenerativeModel('gemini-1.5-flash')
    
    # 3. CHAIN OF THOUGHT ENFORCEMENT
    full_prompt = f"{system_prompt}\n\nUSER_INPUT: {text_input}\n\nINSTRUCTION: Think step-by-step before answering."
    
    # Generate Draft A
    draft_a = model.generate_content(full_prompt).text
    
    # 4. RECURSIVE SELF-CORRECTION (The Genius Loop)
    final_output = recursive_critic(text_input, draft_a, agent_name)
    
    # Save Atomic Truth
    save_memory(agent_name, text_input, final_output, "INTELLIGENCE_GENIUS")
    
    return final_output

def perform_lighthouse_audit(content):
    """Simulates a Lighthouse Audit for Code/Prompts"""
    model = genai.GenerativeModel('gemini-1.5-flash')
    prompt = f"""
    Act as Google Lighthouse but for Generative Content and Code.
    Audit the following input for:
    1. Performance (Efficiency of the prompt/code)
    2. Accessibility (Clarity and inclusivity)
    3. Best Practices (Formatting, safety, modern standards)
    4. SEO (Keyword relevance or searchability if applicable)
    
    Rate each category 0-100 and provide specific actionable improvements.
    
    INPUT TO AUDIT:
    {content}
    """

    
    response = model.generate_content(prompt)
    save_memory("LIGHTHOUSE", content, response.text, "AUDIT")
    return response.text

def generate_design_system(concept):
    """Generates a Visual Language for a concept"""
    model = genai.GenerativeModel('gemini-1.5-pro')
    prompt = "You are HEX. Generate a comprehensive Design System for: " + concept + ". Output must be valid JSON: { \"palette\": { \"primary\": \"#...\", \"secondary\": \"#...\", \"bg\": \"#...\", \"text\": \"#...\" }, \"typography\": { \"headings\": \"font\", \"body\": \"font\" }, \"moodboard\": [\"keyword1\", \"keyword2\", \"keyword3\"], \"css_suggestion\": \"css code snippet for a button...\" }"
    
    response = model.generate_content(prompt)
    save_memory("HEX", concept, response.text, "DESIGN_SYSTEM")
    return response.text

# --- THE UI LAYOUT ---
st.title("🎛️ NOIZYLAB // MASTER CONTROL")

# TABS for distinct workflows
# TABS for distinct workflows
# TABS for distinct workflows
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["🧠 INTELLIGENCE", "🔊 SENSORS", "⚡ AUDIT", "🎨 STUDIO", "🗄️ ARCHIVES", "🎥 VIDEOFX"])

# TAB 6: VIDEOFX (Google Veo)
with tab6:
    st.header("🎥 Google Veo // Cinematic Engine")
    st.write("Generate high-fidelity prompts for Google VideoFX (Veo). Focus on Atmosphere Loops.")
    
    col1, col2 = st.columns(2)
    with col1:
        scene_type = st.selectbox("Scene Type", ["Atmosphere Loop", "Establishing Shot", "Macro Detail", "Character Action"])
        camera_move = st.selectbox("Camera Movement", ["Static Tripod", "Slow Pan Left", "Slow Pan Right", "Dolly Zoom (Vertigo)", "Rack Focus", "Handheld Shake"])
    
    with col2:
        lighting = st.selectbox("Lighting", ["Cyberpunk Neon", "Natural Sunlight", "Golden Hour", "Studio Softbox", "Bioluminescent"])
        fps = st.select_slider("Frame Rate", options=["24fps (Cinematic)", "30fps (Standard)", "60fps (Smooth)"])

    raw_concept = st.text_area("Describe the Video Scene (e.g., 'Rain falling on a neon sign in Tokyo')", height=100)
    
    if st.button("GENERATE VEO PROMPT"):
        with st.spinner("Compiling Cinematic Instructions..."):
            veo_prompt = f"""
            Task: Generate a prompt for Google Veo (VideoFX).
            Format: Professional Cinematography Instructions.
            
            Parameters:
            - Type: {scene_type}
            - Camera: {camera_move}
            - Lighting: {lighting}
            - FPS: {fps}
            
            Concept: {raw_concept}
            """
            
            model = genai.GenerativeModel('gemini-1.5-pro')
            result = model.generate_content(veo_prompt).text
            
            st.subheader("🎬 Veo Prompt")
            st.code(result, language="markdown")
            save_memory("VEO_DIRECTOR", raw_concept, result, "VIDEO_PROMPT")

    
# TAB 5: ARCHIVES (Database View)
with tab5:
    st.header("🗄️ NOIZYLAB ARCHIVES")
    st.write("Persistent Memory of all Consoles Operations.")
    
    memories = load_memories()
    if not memories:
        st.info("No data recorded yet. Start using the tools!")
    else:
        # Reverse to show newest first
        for mem in reversed(memories):
            with st.expander(f"[{mem['timestamp']}] {mem['agent']} // {mem['type']}"):
                st.write(f"**INPUT:** {mem['input']}")
                st.write(f"**OUTPUT:** {mem['output']}")


# TAB 1: INTELLIGENCE
with tab1:
    st.header("Human-System Interface")
    user_idea = st.text_area("Input your raw idea (e.g., 'Sad robot walking in rain')", height=100)
    
    if st.button("OPTIMIZE PROMPT"):
        if not api_key:
            st.error("⚠️ API KEY MISSING")
        else:
            with st.spinner(f"{selected_agent} is Processing..."):
                result = analyze_prompt(user_idea, selected_agent)
                st.subheader(f"{AGENTS[selected_agent]['icon']} {selected_agent}'s Output:")
                st.write(result)

# TAB 3: LIGHTHOUSE AUDIT
with tab3:
    st.header("⚡ Content Lighthouse")
    st.write("Audit your Prompts, Code, or Ideas against Google's High Standards.")
    
    audit_input = st.text_area("Content to Audit")
    
    if st.button("RUN LIGHTHOUSE"):
        if not api_key:
            st.error("⚠️ API KEY MISSING")
        else:
            with st.spinner("Running Lighthouse Analysis..."):
                audit_result = perform_lighthouse_audit(audit_input)
                st.subheader("📊 Audit Report")
                st.markdown(audit_result)

# TAB 4: DESIGN STUDIO
with tab4:
    st.header("💅 HEX // VISUAL ARCHITECT")
    st.write("Generate professional Grade Design Systems & Art Direction.")
    
    design_concept = st.text_input("Enter Design Concept (e.g., 'Cyberpunk Coffee Shop', 'Minimalist AI Dashboard')")
    
    if st.button("GENERATE VISUAL LANGUAGE"):
        if not api_key:
            st.error("⚠️ API KEY MISSING")
        else:
            with st.spinner("HEX is crafting the aesthetic..."):
                design = generate_design_system(design_concept)
                st.subheader("🎨 Design System")
                
                # Simple parsing attempt to show colors visually if possible, or just raw text
                st.code(design, language="json")
                st.info("Copy this JSON for your frontend developer or into your CSS variables.")

# TAB 2: SENSORS
with tab2:
    st.header("Wideband Audio Sensor")
    st.write("Upload a sound file. Gemini will listen to it and describe the video that should match it.")
    
    uploaded_file = st.file_uploader("Upload Audio (MP3/WAV)", type=["mp3", "wav"])
    
    if uploaded_file is not None:
        # Display Audio Player
        st.audio(uploaded_file, format='audio/mp3')
        
        if st.button("ANALYZE AUDIO VIBE"):
            if not api_key:
                st.error("⚠️ API KEY MISSING")
            else:
                with st.spinner("Listening to file..."):
                    # 1. Save file temporarily so Gemini can read it
                    temp_filename = "temp_audio.mp3"
                    with open(temp_filename, "wb") as f:
                        f.write(uploaded_file.getbuffer())
                    
                    # 2. Upload to Gemini (Real Multimodal Function)
                    myfile = genai.upload_file(temp_filename)
                    
                    # 3. Generate Analysis
                    model = genai.GenerativeModel("gemini-1.5-flash")
                    result = model.generate_content(
                        [myfile, "Listen to this audio. Describe the exact mood, BPM, and visual scene that should accompany this sound. Give me a prompt for a Video Generator."]
                    )
                    
                    st.success("Analysis Complete")
                    st.markdown(result.text)
                    
                    # Cleanup
                    os.remove(temp_filename)
