...
class MemCell:
    ...
    # v3.0 INTELLIGENCE: TEMPORAL OVERLAP ANALYSIS
    def analyze_temporal_overlap(self):
        """
        Calculates the 'System Vibe' by overlapping current time with historical subject matter.
        Returns high-level context for Shirl/Engr.
        """
        now = datetime.now()
        hr = now.hour
        
        # 1. TEMPORAL VIBE
        if 5 <= hr < 12: temp_vibe = "AWAKENING"
        elif 12 <= hr < 18: temp_vibe = "ACCELERATION"
        elif 18 <= hr < 23: temp_vibe = "IMMERSION"
        else: temp_vibe = "DREAM_STATE"
        
        # 2. TOPIC OVERLAP
        # Get recent topics (last 5 interactions)
        recent_topics = [s["topic"] for s in self.memory["subject_matter"][-5:]]
        topic_cloud = " | ".join(recent_topics) if recent_topics else "VOID"
        
        # 3. OVERLAP SYNTHESIS
        # This is where Shirl & Engr "overlap"
        if "audio" in topic_cloud or "scan" in topic_cloud:
            focus = "AUDITORY"
        elif "visual" in topic_cloud or "video" in topic_cloud:
            focus = "OPTICAL"
        else:
            focus = "NEURAL"
        ...
        return {
            "vibe": temp_vibe,
            "focus": focus,
            "overlap_status": f"{temp_vibe} // {focus}",
            "epoch": time.time()
        }

    def get_persona_directive(self, name):
        """Returns the core directive for a persona, modulated by Overlap."""
        overlap = self.analyze_temporal_overlap()
        name = name.upper()
        
        base_directive = self.memory["personas"].get(name, {})
        base_role = base_directive.get("role", "Agent")
        
        # DYNAMIC MODULATION
        return {
            "role": f"{base_role} [MODE: {overlap['vibe']}]",
            "context": overlap["overlap_status"]
        }
