...
# Connect to Microsoft 365 (Azure AD)
Connect-MgGraph -Scopes "User.Read.All", "User.ReadWrite.All"

# Find the user by email
$user = Get-AzureADUser -ObjectId "rp@fishmusicinc.com"
if ($user) {
    Remove-AzureADUser -ObjectId $user.ObjectId
    Write-Host "User rp@fishmusicinc.com deleted."
} else {
    Write-Host "User not found."
}

Get-AzureADGroup | Where-Object { (Get-AzureADGroupMember -ObjectId $_.ObjectId).UserPrincipalName -contains "rp@fishmusicinc.com" }

Install-Module Microsoft.Graph -Scope CurrentUser

Get-Mailbox -Identity "rp@fishmusicinc.com"
Get-MailUser -Identity "rp@fishmusicinc.com"
Get-Recipient -Identity "rp@fishmusicinc.com"
Remove-Mailbox -Identity "rp@fishmusicinc.com"

$userName = "R.S Plowman"
$userId = (Get-MgUser -Filter "displayName eq '$userName'").Id
Remove-MgUser -UserId $userId -Confirm:$false

$timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
Add-Content -Path "cockpit_logs/deletion.log" -Value "$timestamp - Deleted user $userName ($userId)"

sudo launchctl list | grep noizy

tail -f /Users/rsp_ms/Desktop/MissionControl96/state/auto_launch_stdout.log

E:\RescueTools\usb_rescue_setup.ps1
