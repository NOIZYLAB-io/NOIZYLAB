...
def get_memcell_graph():
    ...
    family_edges = [
        {"source": "MC96", "target": "GABRIEL"},
        {"source": "MC96", "target": "LIFELUV"},
        {"source": "GABRIEL", "target": "SHIRL"},
        {"source": "GABRIEL", "target": "KEITH"},
    ...
    if fs_root:
        family_edges.append({"source": "MC96", "target": fs_root["id"]})
        
    # Merge
    graph_data["nodes"].extend(family_nodes)
    # Convert edges from/to to source/target for ForceGraph3D
    converted_edges = []
    for e in graph_data.get("edges", []):
        converted_edges.append({"source": e.get("from", e.get("source")), "target": e.get("to", e.get("target"))})
    graph_data["edges"] = converted_edges + family_edges
    
    # Allow limiting via query param
    limit = request.args.get('limit', type=int)
    if limit:
        # Keep family nodes (5) + limited file nodes
        family_ids = {n["id"] for n in family_nodes}
        other_nodes = [n for n in graph_data["nodes"] if n["id"] not in family_ids]
        # Prioritize directories
        dirs = [n for n in other_nodes if n.get("type") == "dir"]
        files = [n for n in other_nodes if n.get("type") == "file"]
        limited = dirs[:limit//2] + files[:limit//2]
        limited_ids = {n["id"] for n in limited} | family_ids
        graph_data["nodes"] = family_nodes + limited
        graph_data["edges"] = [e for e in graph_data["edges"] 
                               if e.get("source") in limited_ids or e.get("target") in limited_ids]
    
    return jsonify({\"nodes\": graph_data[\"nodes\"], \"links\": graph_data[\"edges\"]})


@app.route('/api/memcell/graph/lite')
def get_memcell_graph_lite():
    """Lightweight graph - only top 200 nodes for fast 3D rendering"""
    # Load and return a minimal version
    data_path = os.path.join(os.path.dirname(__file__), 'golang_ecosystem', 'brain.json')
    
    # Family nodes always included
    family_nodes = [
        {"id": "GABRIEL", "label": "GABRIEL (CORE)", "type": "agent_core", "size": 10000000},
        {"id": "SHIRL", "label": "SHIRL (VOICE)", "type": "agent_sub", "size": 5000000},
        {"id": "KEITH", "label": "ENGR. KEITH", "type": "agent_sub", "size": 5000000},
    ...
    if os.path.exists(data_path):
        try:
            ...
            # Get top-level dirs only (depth = 2 means direct children of root)
            ...
            # Convert edges from/to to source/target
            limited_edges = [{"source": e.get("from", e.get("source")), "target": e.get("to", e.get("target"))}
                             for e in edges if e.get("to", e.get("target")) in limited_ids and e.get("from", e.get("source")) in limited_ids]
            
            # Connect MC96 to root
            if root:
                family_edges.append({"source": "MC96", "target": root["id"]})
            
            return jsonify({
                "nodes": family_nodes + limited_nodes,
                "links": family_edges + limited_edges
            })
            
        except Exception as e:
            print(f"Error loading lite graph: {e}")
    
    return jsonify({"nodes": family_nodes, "edges": family_edges})
