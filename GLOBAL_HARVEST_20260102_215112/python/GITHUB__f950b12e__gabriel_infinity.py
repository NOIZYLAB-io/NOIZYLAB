...
class HyperCortex:
    ...
    def think(self, prompt: str, context: str = "") -> str:
        if not self._loaded:
            return "Cortex not loaded. Using fallback."
        ...
        try:
            ...
            response = self._generate(model, tok, prompt=full_prompt, max_tokens=200, verbose=False)
    ...
    while True:
        try:
            ...
            if "schedule" in user_in.lower():
                ...
                print(f"{'OK' if success else 'CONFLICT'}: {msg}")
                continue
            ...
            response = await loop.run_in_executor(None, brain.think, user_in, context)
