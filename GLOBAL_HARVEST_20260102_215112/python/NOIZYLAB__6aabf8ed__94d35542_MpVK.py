...
class GabrielAgent(Agent):
    ...
    async def speak(self, text, emotion="neutral"):
        ...
        # Using a deep, authoritative voice for Gabriel
        ...
        if os.path.exists(rvc_model):
            print(f"[GABRIEL] 🎭 Applying RVC (Gabriel Live Mode)...")
            rvc_out = "gabriel_rvc.wav"
            # Call the isolated RVC runner
            cmd_rvc = [
                XTTS_PYTHON, # Recycle the heavy venv python
                "/Users/m2ultra/NOIZYLAB/GABRIEL/bin/rvc_runner.py",
                "--input", base_file,
                "--model", rvc_model,
                "--out", rvc_out,
                "--pitch", "-12" # Octave down for depth, adjustable
            ]
            process = subprocess.run(cmd_rvc, capture_output=True, text=True)
            if process.returncode == 0:
                final_file = rvc_out
                print(f"   ✨ RVC Success: {final_file}")
            else:
                 print(f"   ❌ RVC Failed: {process.stderr}")
        else:
             print(f"   (No RVC model found at {rvc_model}. Playing raw EdgeTTS.)")
