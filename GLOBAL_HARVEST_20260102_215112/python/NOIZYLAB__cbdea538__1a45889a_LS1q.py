...
def run_librarian(target_dir):
    print(f"{BOLD}{CYAN}📚 TURBO LIBRARIAN: HYPER-SPEED METADATA INJECTOR{RESET}")
    print(f"   Target: {target_dir}")
    print(f"   Method: MacOS Extended Attributes (Parallelized)")
    ...
    print(f"   Found {len(files_to_process)} candidates. Engaging Juggernaut Drive...")
    ...
    with ThreadPoolExecutor(max_workers=THREADS) as executor:
        ...
        for i, future in enumerate(as_completed(future_to_file)):
            ...
            if processed % 100 == 0:
                print(f"   ...injected {processed}/{len(files_to_process)}...", end='\r')

    duration = time.time() - start_time
    rate = processed / duration if duration > 0 else 0
    
    print(f"\n{GREEN}✨ LIBRARY CATALOGED IN {duration:.2f}s{RESET}")
