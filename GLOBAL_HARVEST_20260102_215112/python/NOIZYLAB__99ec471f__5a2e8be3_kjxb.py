...
def run_omega(target_dir):
    ...
    # 4. Sanitize (Renaming)
    if not run_step("SANITIZER (Filename Fix)", "turbo_sanitizer.py", [str(target_path)]): return

    # 5. Alchemist (Format Fix)
    if not run_step("ALCHEMIST (Audio Transmutation)", "turbo_converter.py", [str(target_path)]): return

    # 6. Librarian (Metadata Injection)
    if not run_step("LIBRARIAN (Soul Injection)", "turbo_librarian.py", [str(target_path)]): return
    
    # 7. ARCHITECT (Structural Optimization)
    # We run this on the target_path. 
    # Note: If target_path is ~/Universal, and Library is IN ~/Universal, the organizer handles excluding the destination.
    if not run_step("ARCHITECT (Final Sort)", "turbo_organizer.py", [str(target_path)]): return

    # 8. Final Index (Update DB with new structure)
