...
AUDIO_EXTS = {'.wav', '.mp3', '.aif', '.aiff', '.flac', '.ogg', '.m4a', '.mid', '.midi'}
DAW_EXTS = {'.als', '.logicx', '.ptx', '.ptf', '.flp', '.wve', '.wsve', '.prproj', '.drp', '.rpp', '.fspro', '.unity', '.unitypackage', '.tmfs', '.tmws'} # Logic, Filmora, Premiere, Resolve, Live, Reaper, Pro Tools, FMOD, Unity, RME 
...
class FishnetIndex:
    ...
    def scan_volume(self, vol_path: Path):
        ...
        for root, dirs, files in os.walk(vol_path):
            ...
            # CHECK FOR BUNDLES (Logic Pro .logicx is a directory)
            for d in dirs:
                if d.endswith('.logicx'):
                    full_path = Path(root) / d
                    try:
                        # Add bundle as a single asset
                        batch.append((
                            str(full_path), d, vol_path.name,
                            0, 0, 'project' # Size/Time hard to get for bundle cheaply, just mark existence
                        ))
                    except: pass
