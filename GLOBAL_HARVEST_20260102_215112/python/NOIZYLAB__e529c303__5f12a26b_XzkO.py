...
# MODULE: 👁️ MEDIA FISHNET (Global Asset Scan)
elif app_mode == "👁️ VISUAL VAULT (Asset Scan)":  # Label stays same for menu consistency
    st.subheader("/// MEDIA FISHNET (UNIVERSAL INDEX)")
    ...
    if vis_db.exists():
        ...
        with c1:
            search_query = st.text_input("🔍 SEARCH ASSETS", placeholder="e.g. 'render', 'final', 'scene'")
        with c2:
            media_filter = st.selectbox("TYPE", ["ALL", "VIDEO", "IMAGE"])
        with c3:
            sort_order = st.selectbox("SORT", ["NEWEST", "LARGEST", "NAME"])

        # Construct Query
        try:
            query = "SELECT path, filename, volume, size, media_type FROM assets WHERE 1=1"
            params = []
            ...
            # Stats
            total_v = conn.execute("SELECT COUNT(*) FROM assets WHERE media_type='video'").fetchone()[0]
            total_i = conn.execute("SELECT COUNT(*) FROM assets WHERE media_type='image'").fetchone()[0]
            st.caption(f"🕸️ CAUGHT: {total_v:,} VIDEOS | {total_i:,} IMAGES")
        ...
        # CINEMA GRID
        if res:
            for path, name, vol, size, mtype in res:
                with st.expander(f"{'🎬' if mtype=='video' else '🖼️'} {name}  |  {vol}  |  {size/1024/1024:.1f} MB"):
                    try:
                        if mtype == 'video':
                             st.video(path)
                        else:
                        ...
                        st.error("File offline or protected.")
        else:
            st.info("Net empty. Try broader search.")
            
    else:
        st.warning("Fishnet Building... System is trawling the Universe.")
    
    if st.button("RECAST NET (Rescan)"):
         subprocess.Popen(["python3", "GABRIEL_UNIFIED/core/visual_scanner.py"])
         st.toast("Fishnet Deployed...")
