...
# Only index specific audio/visual extensions for speed, or * for everything?
# User said "FIND EVERYTHING". Let's cover the main media types + common docs.
EXTENSIONS = {
    # Audio
    ...
    # Projects/Docs
    '.als', '.logicx', '.ptx', '.cpr', '.flp', '.asd', '.pdf', '.txt', '.md', '.json', '.xml', '.zip'
}
...
THREADS = max(4, os.cpu_count() or 4) * 2 # Aggressive threading
...
def fast_sha256(path: Path):
    # Memory-map hashing for speed
    h = hashlib.sha256()
    try:
        with open(path, "rb") as f:
            # Check size first, empty files are quick
        ...
        # Fallback for empty files or special files where mmap fails
        try:
            with open(path, "rb") as f:
...
def index_file(fpath, vol_name):
    try:
        ...
        # 2. Hash (The expensive part - maybe skip for massive files?)
        # User said "SUPER DEEP". We hash.
...
def run_turbo_index(target_vol=None):
    ...
    if target_vol:
        ...
        print(f"   Targeting {len(volumes)} Volumes...")
        
        for vol in volumes:
            if vol.name == 'M2Ultra': continue # Skip boot drive for now unless asked
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n⚠️ Interrupted. Database is safe (WAL mode).")
