...
if AGENCY_AVAILABLE:
    ...
    st.caption("The Total Repository of Human Knowledge. Literature & Musicology.")
    
    archive = OmniArchive()
    agency = get_agency()
    
    # Tabs for Domains
    tab_lit, tab_mus = st.tabs(["📚 THE LIBRARIAN", "🎼 THE MUSICOLOGIST"])
    
    with tab_lit:
        st.subheader("Keeper of Books")
        st.write("Specialty: Every book published since 1900.")
        
        query_lit = st.text_area("Ask the Librarian:", placeholder="Tell me about the symbolism of green light in Gatsby...", height=100)
        
        if st.button("Consult Library", type="primary"):
            if query_lit:
                with st.spinner("Searching the infinite shelves..."):
                    response = archive.consult(query_lit, domain="Literature")
                    st.markdown(f"### 📖 The Insight\n\n{response}")
                    st.session_state['archive_response'] = response
                    
    with tab_mus:
        st.subheader("Eternal Musicologist")
        st.write("Specialty: Composition, Theory, Instruments (40,000 BCE - Present).")
        
        query_mus = st.text_area("Ask the Musicologist:", placeholder="Explain the evolution of the Synthesizer...", height=100)
        
        if st.button("Consult Music History", type="primary"):
            if query_mus:
                with st.spinner("Listening to the echoes of time..."):
                    response = archive.consult(query_mus, domain="Musicology")
                    st.markdown(f"### 🎼 The Analysis\n\n{response}")
                    st.session_state['archive_response'] = response
    ...
    if 'archive_response' in st.session_state:
        ...
        if st.button("🗣️ Read to Me (Gabriel)", use_container_width=True):
            ...
            # We need to run async in a sync context here properly
            # Re-using the casting logic or just direct speak?
            # Direct speak for Gabriel is best.
            with st.spinner("Gabriel is preparing to lecture..."):
                # Use Gabriel (Index 0)
                gabriel = agency.agents[0]
                # Truncate if too long? RVC handles long chunks okay but let's be safe? 
                # For now, pass full text.
