    ...
    def main_loop(self):
        ...
        while True:
            try:
                ...
                # 1. INTENT ANALYSIS (Simple Keyword + LLM Refinement)
                # In a full build, we'd use function calling or a router.
                # For zero latency, we check keywords first, then ask Cortex.
                ...
                # --- VIDEO GENERATION ---
                if "video" in lower_in and ("create" in lower_in or "generate" in lower_in or "make" in lower_in):
                    if self.video_forge:
                        ...
                        prompt = user_in.replace("create a video", "").replace("generate video", "").strip()
                        if not prompt: prompt = "Abstract cybernetic flow"
                        
                        # Async trigger? For now sync to ensure quality
                        print(f"⚡ [TOOLS] Generating Video: {prompt}")
                        # Default to Veo for speed/quality balance
                        path = self.video_forge.generate_veo(prompt)
                        
                        if path:
                            msg = f"Video complete: {path.name}"
                            self.orator.speak("Visuals rendered.")
                        else:
                            msg = "Video generation failed."
                            self.orator.speak("Visuals failed.")
                ...
                # --- AUDIO REPAIR ---
                    ...
                    # In this CLI mode, we might need a file path. 
                    # Usually Studio Hand watches a folder.
                    # But we can force a scan?
                    self.orator.speak("Scanning repair sector.")
                    # Trigger an ingest or specific repair?
                    # For now, acknowledge the Studio Hand is watching.
