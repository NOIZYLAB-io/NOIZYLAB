...
def test_gabriel_brain():
    ...
    # Test IntentRouter
    router = IntentRouter()
    assert router.classify("what is the status") == "status"
    assert router.classify("who are you") == "who"
    assert router.classify("optimize the code") == "optimize"
    assert router.classify("hello there") == "greeting"
    assert router.classify("explain quantum physics") == "query"
