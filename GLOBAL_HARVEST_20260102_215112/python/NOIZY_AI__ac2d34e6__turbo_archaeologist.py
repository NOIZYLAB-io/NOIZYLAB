...
def run_archaeologist(root_dir):
    print(f"{BOLD}{CYAN}CORE > 🏺 THE ARCHAEOLOGIST: DIGGING FOR ARTIFACTS...{RESET}")
    ...
    print(f"CORE > Found {len(artifacts)} Sites with Artifacts.")
    ...
    for site in artifacts:
        ...
        if info_str:
            print(f"CORE > 📍 {Path(site['path']).name}: {', '.join(info_str)}")

    # Integration: Write to JSON if requested
    if "--json" in sys.argv:
        output_path = SCRIPTS_DIR.parent / "Database" / "archaeologist_latest.json"
        try:
             with open(output_path, "w") as f:
                 json.dump(artifacts, f, indent=2)
             print(f"CORE > 💾 JSON Artifacts saved to {output_path}")
        except Exception as e:
             print(f"CORE > ❌ JSON Export Failed: {e}")
