...
with st.sidebar:
    ...
    api_key_nvidia = st.text_input("NVIDIA NIM Key", type="password")
    ...
    online_count = sum([bool(api_key_gemini), bool(api_key_claude), bool(api_key_nvidia), bool(api_key_openai), bool(api_key_deepseek)])
    if online_count == 5:
        st.success("🌌 ALL 5 ENGINES ONLINE")
    elif online_count > 0:
        st.info(f"{online_count}/5 Engines Online")
    
    st.divider()
    st.write("NOIZYLAB v5.0 // OMNI-ENGINE")
...
class SmartRouter:
    ...
    def get_provider_for_agent(agent):
        ...
        mapping = {
            ...
            "DREAM": "OpenAI",        # GPT-4o for creative
            "GABRIEL": "NVIDIA",      # NVIDIA for system ops
            "POPS": "DeepSeek",       # DeepSeek R1 for reasoning
            "GENERAL": "Gemini"       # Gemini for fast general
        }
...
class KnowledgeGraph:
    ...
    def __init__(self):
        # We need persistant state for the graph across reruns
    ...
    def add_text(self, text):
        entities = self.mock_ner(text)
        
        # Add Nodes
        for e in entities:
            st.session_state.kg_nodes.add(e)
            
        # Add Clique Edges
        import itertools
        for a, b in itertools.combinations(entities, 2):
            edge = tuple(sorted((a, b)))
            if edge not in st.session_state.kg_edges:
                st.session_state.kg_edges.append(edge)
                
        return len(entities)

    def mock_ner(self, text):
        """Python Port of KNOWLEDGE_GRAPH_AI.ts logic"""
        entities = []
        
        # Core Project Entities
        if re.search(r"Project Omega", text, re.I): entities.append("Project Omega")
        if re.search(r"Launch Date", text, re.I): entities.append("Launch Date")
        
        # OpenAI Voice Technologies
        if re.search(r"(TTS|Text-to-Speech)", text, re.I): entities.append("OPENAI_TTS")
        if re.search(r"(Realtime API|gpt-realtime)", text, re.I): entities.append("REALTIME_API")
        if re.search(r"(GPT-4o|Omni)", text, re.I): entities.append("GPT-4O_OMNI")
        
        # Technical Concepts
        if re.search(r"(latency|low-latency)", text, re.I): entities.append("LATENCY")
        if re.search(r"(multimodal)", text, re.I): entities.append("MULTIMODAL")
        
        # Google AI
        if re.search(r"(Gemini)", text, re.I): entities.append("GEMINI")
        if re.search(r"(Vertex AI)", text, re.I): entities.append("VERTEX_AI")
        
        # Anthropic
        if re.search(r"(Claude|Anthropic)", text, re.I): entities.append("CLAUDE")
        
        # Gabriel
        if re.search(r"(Gabriel|System|God Mode)", text, re.I): entities.append("GABRIEL_SYSTEM")
    ...
    def get_dot(self):
        ...
        dot = 'graph {\n'
        dot += '  rankdir=LR;\n'
        dot += '  node [shape=box, style=filled, color="#00ff00", fontcolor="#0e1117"];\n'
        
        for node in st.session_state.kg_nodes:
            dot += f'  "{node}";\n'
            
        for a, b in st.session_state.kg_edges:
            dot += f'  "{a}" -- "{b}";\n'
...
def generate_nvidia(prompt, key, system_prompt=""):
    """Uses NVIDIA NIM (OpenAI-compatible API)"""
    url = "https://integrate.api.nvidia.com/v1/chat/completions"
    headers = {"Authorization": f"Bearer {key}", "Content-Type": "application/json"}
    full_content = f"{system_prompt}\n\n{prompt}" if system_prompt else prompt
    data = {"model": "nvidia/llama-3.1-nemotron-70b-instruct", "messages": [{"role": "user", "content": full_content}], "max_tokens": 1024}
    req = urllib.request.Request(url, data=json.dumps(data).encode(), headers=headers)
    try:
        with urllib.request.urlopen(req) as response:
...
def generate_openai(prompt, key, system_prompt=""):
    """Uses OpenAI GPT-4o"""
    url = "https://api.openai.com/v1/chat/completions"
    headers = {"Authorization": f"Bearer {key}", "Content-Type": "application/json"}
    messages = [{"role": "system", "content": system_prompt}] if system_prompt else []
    messages.append({"role": "user", "content": prompt})
    data = {"model": "gpt-4o", "messages": messages, "max_tokens": 1024}
    req = urllib.request.Request(url, data=json.dumps(data).encode(), headers=headers)
    try:
        with urllib.request.urlopen(req) as response:
            return json.loads(response.read().decode())['choices'][0]['message']['content']
...
def generate_deepseek(prompt, key, system_prompt=""):
    """Uses DeepSeek Chat/Reasoner"""
    url = "https://api.deepseek.com/chat/completions"
    headers = {"Authorization": f"Bearer {key}", "Content-Type": "application/json"}
    messages = [{"role": "system", "content": system_prompt}] if system_prompt else []
    messages.append({"role": "user", "content": prompt})
    data = {"model": "deepseek-chat", "messages": messages, "max_tokens": 1024}
    req = urllib.request.Request(url, data=json.dumps(data).encode(), headers=headers)
    try:
        with urllib.request.urlopen(req) as response:
            return json.loads(response.read().decode())['choices'][0]['message']['content']
...
def analyze_prompt_smart(text_input, manual_provider, autopilot, keys):
    ...
    if provider == "Gemini":
        ...
        if not keys['nvidia']: return "⚠️ MISSING NVIDIA KEY"
        return generate_nvidia(text_input, keys['nvidia'], system_instruction)
    
    elif provider == "OpenAI":
        if not keys['openai']: return "⚠️ MISSING OPENAI KEY"
        return generate_openai(text_input, keys['openai'], system_instruction)
    
    elif provider == "DeepSeek":
        if not keys['deepseek']: return "⚠️ MISSING DEEPSEEK KEY"
        return generate_deepseek(text_input, keys['deepseek'], system_instruction)
...
tab1, tab2, tab3, tab4 = st.tabs(["🧠 BRAIN (Text)", "🔊 EARS (Audio Analysis)", "🔑 ADMIN", "🕸️ MEMORY"])
...
with tab1:
    ...
    if st.button("OPTIMIZE PROMPT"):
        keys = {"gemini": api_key_gemini, "claude": api_key_claude, "nvidia": api_key_nvidia}
        
        # Check if we have at least one key
        proceed = any([api_key_gemini, api_key_claude, api_key_nvidia])
            
        if not proceed:
            st.error("⚠️ NEED AT LEAST ONE API KEY")
        else:
