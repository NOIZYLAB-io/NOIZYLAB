...
# Paths (from config)
BASE_DIR = cfg.SCRIPTS_DIR.parent.parent
...
def run_step(step_name, command):
    print(f"\n{BOLD}CORE > 🔄 STEP: {step_name}{RESET}")
    # print(f"   Cmd: {' '.join(command)}")
    try:
        subprocess.run(command, check=True)
        print(f"CORE > {GREEN}✅ {step_name} Complete.{RESET}")
    except subprocess.CalledProcessError as e:
        print(f"CORE > {RED}❌ {step_name} Failed: {e}{RESET}")
        pass # Keep going if possible

def run_harvester(source_dir=None, detox=False):
    print(f"{BOLD}{CYAN}CORE > 🌪️  THE HARVESTER: CHAOS INGESTION ENGINE{RESET}")
    
    # 1. Determine Source
    if not source_dir:
        default_dl = Path.home() / "Downloads"
        # i = input(f"   🎯 Target Directory [Default: {default_dl}]: ").strip()
        source_dir = str(default_dl)
        print(f"CORE > Target: {source_dir}")
        
    src_path = Path(source_dir)
    if not src_path.exists():
        print(f"CORE > {RED}❌ Source not found: {src_path}{RESET}")
        return
    ...
    if detox and src_path.exists():
        print(f"\n{BOLD}CORE > 🧹 SOURCE DETOX (Cleaning {src_path.name})...{RESET}")
        try:
            ...
            print(f"CORE > {GREEN}✅ Source Cleaned.{RESET}")
        except: pass

    duration = time.time() - start_time
    print(f"\n{GREEN}CORE > ✨ HARVEST COMPLETE ({duration:.2f}s){RESET}")
    print(f"CORE > The Chaos has been consumed, sorted, sanitized, indexed, and optimized.")
