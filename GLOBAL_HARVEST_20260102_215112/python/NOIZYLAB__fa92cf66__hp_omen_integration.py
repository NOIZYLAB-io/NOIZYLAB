...
class HPOMENIntegration:
    ...
    async def check_connection(self) -> bool:
        ...
        try:
            # Try VPN first, then LAN
            for ip in [self.HP_OMEN_VPN_IP, self.HP_OMEN_IP]:
    ...
    def get_network_volumes(self) -> List[Dict]:
        ...
        try:
            ...
            for line in result.stdout.split('\n'):
                if self.HP_OMEN_IP in line or 'rsp@RSP' in line or 'HP-OMEN' in line:
                    parts = line.split()
                    if len(parts) >= 6:
                        mount = ' '.join(parts[5:])
                        volumes.append({
    ...
    def generate_integration_report(self) -> str:
        ...
        report.append(f"🌐 LAN IP: {self.HP_OMEN_IP}")
        report.append(f"🔒 VPN IP: {self.HP_OMEN_VPN_IP}")
