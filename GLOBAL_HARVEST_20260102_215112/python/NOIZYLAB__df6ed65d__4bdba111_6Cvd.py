...
def run_hunt():
    print(f"{BOLD}🦅 VISUAL HUNTER INITIALIZED{RESET}")
    print(f"   Target: {KEYWORDS}")
    print(f"   Types:  {EXTENSIONS}")
    ...
    print(f"🚀 Deploying Visual Scanners to {len(volumes)} Volumes...")
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        ...
        for future in as_completed(future_to_vol):
            ...
            try:
                ...
                if count > 0:
                    print(f"   ✅ {vol_name}: Found {count} candidates")
    ...
    print(f"\n{GREEN}✨ HUNT COMPLETE{RESET}")
    print(f"   Total Captured: {total_found} visual assets")
    print(f"   Location: {HUNT_DIR}")
