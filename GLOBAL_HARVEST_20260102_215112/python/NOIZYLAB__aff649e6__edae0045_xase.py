...
def main():
    ...
    for cid, members in clusters.items():
        ...
        gold_corpus.append(gold_entry) # Restore this line!
        changelog['created_gold'].append(gold_id)

        if confidence >= 80 and action_flag == "ACTION":
            changelog['held_for_review'].append(gold_id) # Using this list for "Actions" tracking for now
    ...
    # E) ACTION QUEUE GENERATION
    action_queue_path = DB_PATH.parent / "ACTION_QUEUE.md"
    action_items = [g for g in gold_corpus if g['Meta']['Actionability'] == "ACTION"]
    ...
    if action_items:
        with open(action_queue_path, 'w') as f:
            ...
            for item in action_items:
                f.write(f"- [ ] **{item['Title']}** (Conf: {item['Meta']['Confidence']}%)\n")
                f.write(f"  - {item['Body']}\n")
                f.write(f"  - Ref: `{item['GOLD_ID']}`\n\n")
        print(f"ACTION QUEUE: Generated {len(action_items)} items in {action_queue_path.name}")
    else:
        # Create empty or clear if no actions
        if action_queue_path.exists():
            with open(action_queue_path, 'w') as f:
                f.write("# ⚡ GABRIEL ACTION QUEUE\n\nAll clear. Go Run Free.\n")
