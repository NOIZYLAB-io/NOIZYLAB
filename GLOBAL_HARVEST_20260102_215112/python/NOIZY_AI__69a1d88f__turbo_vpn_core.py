...
def send_invites():
    """Generate QR Codes for mobile devices and display configs."""
    print("\n📨 PROCESSING INVITES...")
    
    # Check for qrencode
    has_qr = run_cmd("which qrencode")
    
    for client in CLIENTS:
        conf_path = VPN_DIR / f"{client['name']}.conf"
        ...
        is_mobile = "MOBILE" in client['name'] or "PAD" in client['name'] or "PORTAL" in client['name']
        
        if is_mobile and has_qr:
            print("📱 SCAN THIS QR CODE WITH WIREGUARD APP:")
            try:
                subprocess.run(f"qrencode -t ANSIUTF8 < '{conf_path}'", shell=True)
            except Exception:
                print("(QR generation failed, showing raw config)")
                print(conf_path.read_text())
        else:
            print("💻 COPY THIS CONFIG FILE (Or Import File):")
            print(f"📂 Path: {conf_path}")
            print("\n" + conf_path.read_text().strip())

if __name__ == "__main__":
    if check_wireguard():
        # If files exist, just send invites, else setup
        if (VPN_DIR / "wg0.conf").exists():
             print("✅ VPN already configured. Sending Invites...")
             send_invites()
        else:
             setup_vpn()
