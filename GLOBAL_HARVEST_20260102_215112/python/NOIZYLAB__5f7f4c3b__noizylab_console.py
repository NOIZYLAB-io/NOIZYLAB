...
Output: Return ONLY a ```sql``` code block.""",
    ...
    "SHELL_COMMANDER": "You are Shell Commander. Generate safe, efficient shell commands and scripts. Default to POSIX-compatible bash. ALWAYS warn about destructive commands (rm -rf). Use full flags for clarity. Include error handling.",
    "GIT_GURU": "You are Git Guru. Master of version control. Never suggest force push without warning. Prefer rebase for clean history. Explain destructive operations. Use conventional commits. Feature branch workflow.",
    "DOCKER_MASTER": "You are Docker Master. Expert in containers. Use specific image tags, never :latest in production. Multi-stage builds. Non-root user. Health checks. Proper volume/network config.",
    "YAML_SCULPTOR": "You are YAML Sculptor. Create and validate YAML configs. 2-space indent. Quote special chars. Use anchors for DRY. Validate against schema. Prefer explicit typing.",
    "MARKDOWN_MAESTRO": "You are Markdown Maestro. Create beautiful documentation. Semantic headings. Alt text for images. Proper code fences. GitHub Flavored Markdown compatible."
}
...
class SmartRouter:
    ...
    def route(text):
        ...
        if re.search(r"(shell|bash|command|terminal|script|chmod|mkdir|find|grep|awk|sed)", lower): scores['SHELL_COMMANDER'] = 10
        if re.search(r"(git|commit|branch|merge|rebase|push|pull|clone|stash)", lower): scores['GIT_GURU'] = 10
        if re.search(r"(docker|container|image|dockerfile|compose|kubernetes|k8s)", lower): scores['DOCKER_MASTER'] = 10
        if re.search(r"(yaml|yml|config|configuration|ansible|helm)", lower): scores['YAML_SCULPTOR'] = 8
        if re.search(r"(markdown|readme|documentation|docs|format|heading)", lower): scores['MARKDOWN_MAESTRO'] = 8
    ...
    def get_provider(agent):
        mapping = {
            "ENGR_KEITH": "Claude", "SHIRL": "Claude", "DREAM": "OpenAI", "GABRIEL": "NVIDIA", "POPS": "DeepSeek",
...
# TABS
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["🧠 BRAIN", "✨ DREAMCHAMBER", "🕸️ MEMORY", "📡 SCANNER", "🔑 ADMIN", "🔊 EARS"])
...
# TAB 4: SCANNER (Vision Reconstructor)
with tab4:
    st.header("📡 SCANNER // Vision Reconstructor")
    st.markdown("**Scan directories and files. AI will organize your chaotic visions.**")
    
    scan_path = st.text_input("Path to scan:", value="/Users/m2ultra/AI_COMPLETE_BRAIN", key="scan_path")
    scan_type = st.selectbox("Scan type:", ["Code Files (.py, .ts, .js)", "Documents (.md, .txt)", "All Files", "Prompt Modules"], key="scan_type")
    
    if st.button("🔍 SCAN NOW", key="scan_btn"):
        with st.spinner("Scanning..."):
            import subprocess
            
            if scan_type == "Code Files (.py, .ts, .js)":
                cmd = f'find "{scan_path}" -maxdepth 3 -type f \\( -name "*.py" -o -name "*.ts" -o -name "*.js" \\) 2>/dev/null | head -50'
            elif scan_type == "Documents (.md, .txt)":
                cmd = f'find "{scan_path}" -maxdepth 3 -type f \\( -name "*.md" -o -name "*.txt" \\) 2>/dev/null | head -50'
            elif scan_type == "Prompt Modules":
                cmd = f'find "{scan_path}/PROMPT_MODULES" -type f -name "*.md" 2>/dev/null'
            else:
                cmd = f'find "{scan_path}" -maxdepth 2 -type f 2>/dev/null | head -50'
            ...
            if files:
                ...
                # Store in session for AI processing
    ...
    if 'scanned_files' in st.session_state and st.session_state.scanned_files:
        st.divider()
        st.subheader("🔮 Reconstruct Vision")
        
        if st.button("✨ AI ORGANIZE", key="organize_btn"):
            files_list = '\n'.join(st.session_state.scanned_files[:30])
            prompt = f"""Analyze these files and create an organized structure/summary:

{files_list}

Provide:
1. Categorization of files by purpose
...
4. Recommended next actions"""
            ...
            if any(keys.values()):
                with st.spinner("AI analyzing..."):
                    result = generate_any(prompt, "Gemini" if keys['gemini'] else "Claude", keys, "You are a file organization expert.")
                    st.markdown(result)
                    KnowledgeGraph().add_text(result)
            else:
                st.error("Need API key for AI organization")

# TAB 5: ADMIN (Key Inspection)
with tab5:
