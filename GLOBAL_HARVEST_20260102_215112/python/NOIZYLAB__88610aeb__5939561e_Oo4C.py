...
def main_menu():
    ...
    print("14.[Hunt]   Visual Logo Hunter (Fish/Noizy)")
    print("15.[Map]    Deep System Cartography (Hotspots)")
    print("16.[Turbo]  Deep Hashing Indexer (Universe DB)")
    print("17.[Find+]  Turbo Search (Universe DB)")
    print("18.[Dedup]  Turbo Duplicate Report")
    print("19.[Sort]   Turbo Organizer (Universal Vault)")
    print("20.[Vacuum] The Harvester (Ingest Chaos)")
    print("Q. Quit")
    ...
    if choice == '0':
      ...
      # 5. Deep Cartography (Map)
        if not run_step("Cartography", "volume_cartographer.py"):
            return
        ...
        subprocess.run(["python3", "Audio_Unitor/Scripts/logo_hunter.py"])
    elif choice == '15':
        subprocess.run(["python3", "Audio_Unitor/Scripts/volume_cartographer.py"])
        print(f"{GREEN}Opening Map...{RESET}")
        subprocess.run(["open", "system_map.html"])
    elif choice == '16':
        print("Launching Turbo Indexer...")
        subprocess.run(["python3", "Audio_Unitor/Scripts/turbo_indexer.py"])
    elif choice == '17':
        q = input("🔎 Turbo Search Query > ")
        subprocess.run(["python3", "Audio_Unitor/Scripts/turbo_search.py", q])
    elif choice == '18':
        subprocess.run(["python3", "Audio_Unitor/Scripts/turbo_dedup.py"])
    elif choice == '19':
        src = input("📂 Source Directory to Organize > ")
        subprocess.run(["python3", "Audio_Unitor/Scripts/turbo_organizer.py", src])
    elif choice == '20':
        subprocess.run(["python3", "Audio_Unitor/Scripts/turbo_ingest.py"])
    elif choice == 'q':
