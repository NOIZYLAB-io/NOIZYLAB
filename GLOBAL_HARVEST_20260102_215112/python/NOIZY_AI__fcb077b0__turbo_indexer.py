...
def db_init(conn):
    ...
    try: c.execute("ALTER TABLE files ADD COLUMN last_modified TEXT")
    except sqlite3.OperationalError: pass
...
def run_turbo_index(target=None):
    ...
    try:
        brain = MemCell()
        brain.log_event(brain.covenant_id, "INDEX_START", f"Indexer Protocol Initiated", vibe=80, author="ENGR_KEITH")
    except: pass
    ...
    for r in all_results:
        if r['tags']:
            normalized = [t.strip().upper() for t in r['tags'].split(',')]
    ...
    # MemCell Update (TURBO SYNAPSE)
    try:
        # Report Bulk Stats
        brain.log_event(brain.covenant_id, "INDEX_COMPLETE", f"Indexed {len(all_results)} files", vibe=100, author="ENGR_KEITH")
        
        # Report Knowledge Discoveries (The "Get Smarter" Part)
        for tag, count in tag_counts.most_common(10):
            if count > 5: # Threshold for "Knowledge"
                discovery_msg = f"Knowledge Cluster Detected: {tag} ({count} items)"
                brain.log_event(brain.covenant_id, "KNOWLEDGE_DISCOVERY", discovery_msg, vibe=90, author="INDEXER", tags=tag)
                print(f"{cfg.GREEN}CORE > 🧠 Synapse Firing: Learned importance of {tag}{cfg.RESET}")
