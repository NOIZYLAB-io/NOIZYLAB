...
# ==============================================================================
# TURBO BFA GOD MODE: THE FINAL UNIFIED TOOL
# ==============================================================================
# Features:
# 1. OMEGA REGEX: O(1) Product Identification.
# 2. ARCHITECT: Rebuilds Library Trees (Moves Folders).
# 3. HEALER: Renames .nki files to their True Internal Names.
# 4. VISUALIZER: Immediate, reliable single-threaded output.
# ==============================================================================

REPAIR_DIR_NAME = "_KTK_To_Fix_2026"
DEFAULT_ROOT = "/Volumes/JOE"
LOG_FILE = "/Volumes/JOE/NKI/turbo_god_log.txt"

# --- OMEGA SIGNATURES ---
# Map: RegexGroup -> (RegexPattern, OfficialProductName)
RAW_SIGS = {
...
def extract_internal_name(mm_data):
    # Scan first 2MB for heuristic name
    scan_limit = min(len(mm_data), 2 * 1024 * 1024)
...
def identify_library_root_and_name(filepath, mm, header):
    ...
    # 2. Context ID (if no binary match)
    path_product = None
    library_root = None
    curr = filepath
    ...
    if not product_name:
        ...
        # If we have binary product name, we still need the root.
        # Assume root is 2-4 levels up? Or stop at "Instruments" parent?
        # Safe guess: If current dir has "Instruments", it's root/Instruments.
        # Moving up until we hit a signature or root heuristic?
        # For safety, let's treat the parent of "Instruments" as root, or just the parent of the NKI folder if obscure.
        pass

    # Use the one we found
    final_product = product_name or path_product
    
    # Internal Name for File Renaming
    internal_name = extract_internal_name(mm)
    
    # Determine Root
    if not library_root:
        # If we found product ID but no root context, assume standard structure:
        # BFA Lib / Instruments / ...
        # Climb up 4 levels watching for "Documents" or "Samples" or just taking the top most useful folder.
        curr = filepath
...
def main():
    ...
    root_dir = args.root
    stats = {'lib_moves': 0, 'file_renames': 0, 'errors': 0}

    # Robust Single-Thread Walk
    for dirpath, _, filenames in os.walk(root_dir):
        if REPAIR_DIR_NAME in dirpath: continue # Skip our own output
                ...
                try:
                    ...
                    with open(full_path, 'rb') as f:
                        with mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ) as mm:
                            head_len = min(len(mm), 100*1024*1024)
                            header = mm[:head_len]
                            
                            product, lib_root, internal_name = identify_library_root_and_name(full_path, mm, header)

                            # --- FILE RENAMING (The "Real Name") ---
                            target_fname = fname
                            if internal_name:
                                clean_n = clean_filename(internal_name) + ".nki"
                                if clean_n != fname:
                                    target_fname = clean_n
                                    # We will rename it during the move phase or in place?
                                    # If we move the library, the file moves with it.
                                    # We should rename it IN PLACE first, or AFTER move?
                                    # Renaming IN PLACE before move is easier to track.
                                    if args.execute:
                                        ...
                                        full_path = new_full_path # Update ref
                                        stats['file_renames'] += 1
                                        log(f"[RENAME] {fname} -> {target_fname}")
                                    else:
                                        log(f"[DRY-RENAME] {fname} -> {target_fname}")

                            # --- LIBRARY REBUILDING (The "Architect") ---
                            if product and lib_root:
                                # Don't move if we are already in the target product folder!
                                # Logic: Target is RepairDir/Product.
                                # If lib_root IS RepairDir/Product, skip.
                                ...
                                if os.path.abspath(lib_root) != os.path.abspath(target_lib_path):
                                    if args.execute:
                                        if not os.path.exists(target_lib_path):
                                            ...
                                            # If we moved the lib, we don't need to process other files in this dir in this loop iteration?
                                            # os.walk might get confused if we delete the dir under it.
                                            # But shutil.move removes it. 
                                            # Just catch exceptions on next files.
                                        else:
                                            # Target exists. Library already moved?
                                            pass
