...
# Load Node Renderer (V36)
NODE_HTML_PATH = "/Users/m2ultra/NOIZYLAB/GABRIEL/components/node_renderer.html"
def get_node_renderer():
    if os.path.exists(NODE_HTML_PATH):
        with open(NODE_HTML_PATH, "r") as f:
            return f.read()
    return ""
...
<style>
    ...
    /* GLASSMORPHIC CONTAINERS (V29 Polish) - UPGRADED TO FLOATING CELLS (V36) */
    .stSelectbox, .stTextArea, .stButton, .stRadio, .stSlider {
        background-color: rgba(74, 4, 4, 0.1) !important;
        backdrop-filter: blur(10px);
        border: 1px solid rgba(194, 178, 128, 0.1) !important;
    ...
    .stSelectbox:hover, .stTextArea:hover, .stButton:hover {
        transform: translateY(-2px);
        box-shadow: 0 8px 40px rgba(74, 4, 4, 0.3);
...
# --- SIDEBAR ---
with st.sidebar:
    # V36: RENDER NODE SYSTEM (Floating Cells)
    node_html = get_node_renderer()
    if node_html:
        components.html(node_html, height=200)
    ...
    # V37: EXPORT FORMAT SELECTOR (CapCut Bridge)
    st.markdown("### 🎬 EXPORT FORMAT")
    EXPORT_FORMATS = {
        "MP4 (Fast)": "mp4",
        "ProRes (Director's Cut)": "prores"
    }
    selected_export_key = st.selectbox("Output Quality:", list(EXPORT_FORMATS.keys()))
    st.session_state.export_format = EXPORT_FORMATS[selected_export_key]
...
if MODE == "Director":
        ...
        with c1:
                    ...
                    for i, scene_text in enumerate(scenes):
                        ...
                        with st.spinner(f"🎨 Synthesizing Scene {i+1}..."):
                            ...
                            video_path = DreamFactory.create_dream(audio, image_url, st.session_state.get('export_format', 'mp4'))
    ...
    st.title("EXAM MODE")
    st.caption("Test Gabriel's knowledge depth.")
    
    domain = st.selectbox("Subject:", ["Literature", "Musicology", "History", "Science"])
    question = st.text_area("Exam Question:")
    
    if st.button("Administer Test", type="primary"):
        if question:
            st.info("Gabriel is thinking...")
