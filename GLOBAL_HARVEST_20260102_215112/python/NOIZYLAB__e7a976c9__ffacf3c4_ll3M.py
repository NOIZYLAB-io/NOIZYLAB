...
class GeminiLiveProvider(VoiceProvider):
    ...
    async def connect(self) -> bool:
        try:
            ...
            # Define Omni-Presence Tools
            computer_tool = Tool(function_declarations=[
                FunctionDeclaration(
                    ...
                    parameters=Schema(
                        type=Type.OBJECT,
                        properties={
                            "action": Schema(type=Type.STRING, description="Action: mouse_move, mouse_click, type, press, scroll, screenshot"),
                            "x": Schema(type=Type.INTEGER, description="X coordinate for mouse actions"),
                            "y": Schema(type=Type.INTEGER, description="Y coordinate for mouse actions"),
                            "text": Schema(type=Type.STRING, description="Text to type"),
                            "key": Schema(type=Type.STRING, description="Key to press (e.g., 'enter', 'esc')"),
                            "amount": Schema(type=Type.INTEGER, description="Scroll amount")
                        },
                        required=["action"]
                    )
                ),
                FunctionDeclaration(
                    ...
                    description="Execute command on D-Link Switch.",
                    parameters=Schema(
                        type=Type.OBJECT,
                        properties={
                            "ip": Schema(type=Type.STRING, description="IP address of the switch"),
                            "command": Schema(type=Type.STRING, description="Command to execute")
                        },
                        required=["ip", "command"]
                    )
                )
            ...
            config = LiveConnectConfig(
                ...
                tools=[Tool(google_search=GoogleSearch()), computer_tool],
    ...
    async def receive(self) -> AsyncGenerator[dict, None]:
        ...
        # Initialize Tools
        from core.gabriel_tools import ComputerTool, NetworkTool
        comp = ComputerTool()
        net = NetworkTool()
        ...
        async for response in self.session.receive():
            ...
            if response.tool_call:
                for fc in response.tool_call.function_calls:
                    result = "Error"
                    if fc.name == "computer_execute":
                        args = fc.args
                        result = comp.execute(
                            action=args.get("action"),
                            params={k:v for k,v in args.items() if k != "action"}
                        )
                    elif fc.name == "network_execute":
                        args = fc.args
                        result = net.execute_command(
                            ip=args.get("ip"),
...
class ElevenLabsProvider(VoiceProvider):
    """ElevenLabs Text-to-Speech (High Quality)"""
    
    def __init__(self, api_key: str, voice_id: str = "21m00Tcm4TlvDq8ikWAM"): # Rachel
        self.api_key = api_key
    ...
    async def connect(self) -> bool:
        try:
            ...
            logger.error(f"❌ ELEVENLABS CONNECT FAILED: {e}")
            return False
...
class MultiVoiceManager:
    ...
    def __init__(
        ...
        if elevenlabs_key:
            self.providers.append(
                ElevenLabsProvider(elevenlabs_key)
            )
