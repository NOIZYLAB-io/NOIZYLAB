...
# Lazy imports used in methods to avoid circular dependency
...
class NoizyRMT:
    ...
    def process_voice_command(self, command: str) -> Dict:
        """Process voice command with zero latency execution."""
        ...
        execution_result = "Logged Only"
        
        # ════════════════════════════════════════════════════
        # REAL-TIME EXECUTION LOGIC
        # ════════════════════════════════════════════════════
        try:
            # Lazy Import to prevent circular ref
            from gabriel_genesis import GabrielGenesis
            from turbo_world_healer import TurboWorldHealer
            from turbo_net_control import TurboNetControl

            # 1. NETWORK CONTROLS
            if "scan network" in cmd_lower:
                net = TurboNetControl()
                res = net.scan_network()
                execution_result = f"Network Scanned: {len(res)} devices found."
                
            elif "optimize network" in cmd_lower or "optimize switch" in cmd_lower:
                net = TurboNetControl()
                threading.Thread(target=net.optimize_switch).start()
                execution_result = "Network Optimization Triggered (Background)"
                
            elif "connect" in cmd_lower and ("gabriel" in cmd_lower or "node" in cmd_lower):
                subprocess.Popen(["/Users/m2ultra/NOIZYLAB/GABRIEL/connect_gabriel.sh"])
                execution_result = "Launching Remote Desktop Connection..."

            # 2. HEALING
            elif "heal" in cmd_lower:
                healer = TurboWorldHealer()
                threading.Thread(target=healer.heal_world).start()
                execution_result = "Global Healing Initiated."

            # 3. GENESIS (Create Project)
            elif "create" in cmd_lower and "project" in cmd_lower:
                # Extract project name (very basic parsing)
                # "Create project Omega" -> "Omega"
                parts = cmd_lower.split("project")
                if len(parts) > 1:
                    name = parts[1].strip().replace(" ", "_").title()
                    # Default to 'ableton' unless code specified
                    style = "code" if "code" in cmd_lower or "python" in cmd_lower else "ableton"
                    genesis = GabrielGenesis()
                    threading.Thread(target=genesis.run_genesis, args=(name, style, "Voice Command Vibe")).start()
                    execution_result = f"Genesis Protocol Started: {name} ({style})"
                else:
                    execution_result = "Genesis Error: No Name Specified."
