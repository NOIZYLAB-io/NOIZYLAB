...
def run_dedup_report(nuke=False):
    ...
    print(f"\n{BOLD}🧹 GENERATING CLEANUP SCRIPT...{RESET}")
    ...
    with open(script_path, "w") as f:
        f.write("#!/bin/bash\n")
        f.write("# TURBO DEDUP CLEANUP SCRIPT\n")
        f.write("# Generated by Audio Unitor\n")
        f.write(f"# Total Wasted Space to Reclaim: {format_size(grand_total_wasted)}\n\n")
        
        for row in all_dupes:
            file_hash = row[0]
            c.execute("SELECT path, volume FROM files WHERE hash=?", (file_hash,))
            copies = c.fetchall()
            
            # Smart Selection Logic:
            # 1. Prefer 'Universal' or 'Audio_Universe' vault
            # 2. Prefer shortest path
            
            best_copy = None
            sorted_copies = sorted(copies, key=lambda x: (
                0 if "Universal" in x[0] or "Audio_Universe" in x[0] else 1, 
                len(x[0])
            ))
            
            best_copy = sorted_copies[0]
            to_delete = sorted_copies[1:]
