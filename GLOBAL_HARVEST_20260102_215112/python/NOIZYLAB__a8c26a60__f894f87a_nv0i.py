...
def run_dedup_report(nuke=False):
    ...
    with open(script_path, 'w') as f:
        ...
        for row in all_dupes:
            f.write(f"# Keeping: {best_copy[0]}\n")
            for kill in to_delete:
                safe_path = kill[0].replace('"', '\\"') # Escape quotes
                cmd = f'rm "{safe_path}"'
                f.write(cmd + "\n")
                nuke_commands.append(kill[0])
            f.write("\n")
            
    os.chmod(script_path, 0o755)
    print(f"   ✨ Script Generated: {GREEN}{script_path}{RESET}")
    
    if nuke:
        print(f"\n{BOLD}{RED}☢️  AUTO-NUKE INITIATED ☢️{RESET}")
        print(f"   Deleting {len(nuke_commands)} redundant files...")
        deleted_count = 0
        ...
        print(f"   {GREEN}✅ NUKE COMPLETE. Deleted {deleted_count} files.{RESET}")
    else:
        print(f"   👉 Review it, then run: {CYAN}./{script_path}{RESET}")
        print(f"   (Or run with --nuke to auto-delete)")
