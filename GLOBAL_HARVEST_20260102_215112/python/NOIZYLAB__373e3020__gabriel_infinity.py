...
# LAZY LOAD DOCTOR (For Zero Latency Startup, but keep it accessible)
...
async def start_api_server(brain, mem, dream, instance):
    ...
    await runner.setup()
    site = web.TCPSite(runner, '0.0.0.0', 5177)
    await site.start()
    print("🌐 DREAMCHAMBER API SERVER ONLINE (PORT 5177)")
...
async def main():
    ...
    portal_pid = "OFFLINE"
    try:
        res = subprocess.check_output(["pgrep", "-f", "portal_server.py"]).decode().strip()
        if res: portal_pid = f"ONLINE (PID {res.split()[0]})"
    except: pass
    print(f"   👁️  PORTAL:        {portal_pid}")
    ...
    async def watch_filesystem():
        ...
        try:
            ...
            print(f"👁️  SENTINEL FATAL ERROR: {e}")
    ...
    while True:
        try:
            ...
            if "/check" in user_in:
                print("🏥 GABRIEL SYSTEM DOCTOR: Starting Scan...")
                if GabrielDoctor:
                    doc = GabrielDoctor()
                    doc.run(auto_heal=False)
                else:
                    print("❌ GabrielDoctor module not found.")
                continue
