# 🤖 SYSTEM FILE: memcell_sweeper.py
...
DB_PATH = Path("/Users/m2ultra/.gemini/antigravity/scratch/NOIZYLAB/PROJECTS_MASTER/GABRIEL_CORE/data/memcell_db/memcell_db.json")
# MC96 DATA ROOTS
...
def generate_smart_plan(pack_name, items):
    """
    MC96 BUILDER (Codemaster) Logic:
    Generates a 3-Step Atomic Plan based on the context of the Work Pack.
    """
    # 1. Analyze Context from Tags and Titles
    all_tags = set()
    for i in items: all_tags.update(i['Tags'])
    titles = " ".join([i['Title'] for i in items]).upper()
    ...
    if "STABILIZATION" in pack_name:
        plan_steps = [
            "[Drift] Review raw Provenance for corruption and isolate drift source.",
            "[Fix] Update GOLD content or Tags to strictly meet schema requirements.",
            "[Verify] Re-run Sweeper to clear Drift flags and confirm 'Steady State'."
        ]
    elif "CODE" in all_tags or "PYTHON" in all_tags or "SCRIPT" in titles:
         plan_steps = [
            "[Analysis] Review existing code architecture and test coverage.",
            "[Execution] Implement changes in atomic commits (Red -> Green -> Refactor).",
            "[Close] Verify with automated tests and merge PR."
        ]
    elif "PROMPT" in all_tags or "DOCTRINE" in all_tags:
         plan_steps = [
            "[Analysis] Review current prompt performance and identify ambiguity.",
            "[Execution] Update prompt text with precise constraints (No fluff).",
            "[Close] Verify output against 'Strict Format' requirements."
        ]
    elif "LAUNCH" in all_tags or "URGENT" in all_tags:
         plan_steps = [
            "[Analysis] Identify critical path and blockers immediately.",
            "[Execution] Execute minimum viable fix to unblock the pipeline.",
            "[Close] Verify system stability and document the incident."
        ]
    else:
        # Default Ops
        plan_steps = [
            "[Analysis] Review P0 objectives and Context for this Work Pack.",
            "[Execution] Perform atomic changes defined in the Task Spine.",
            "[Close] Verify results are 'Gold Standard' and archive Task ID."
        ]

    return plan_steps

def generate_kill_list(pack_name, items):
    """
    MC96 BUILDER Limiters:
    Anti-patterns to avoid for this specific type of work.
    """
    kill_list = ["Ignoring drift warnings.", "Manual polling."] # Defaults

    all_tags = set()
    for i in items: all_tags.update(i['Tags'])

    if "STABILIZATION" in pack_name:
        kill_list.append("Overwriting GOLD without evidence.")
        kill_list.append("Deleting without archiving.")

    if "CODE" in all_tags:
        kill_list.append("Committing broken builds.")
        kill_list.append("Hardcoding secrets/paths.")

    if "PROMPT" in all_tags:
        kill_list.append("Vague instructions ('be helpful').")
        kill_list.append("Unbounded outputs.")
...
def main():
    ...
    for cid in sorted_cluster_ids:
        ...
        # Sort to ensure Anchor is correct (Longest + Newest)
        members.sort(key=lambda x: x['created_at'], reverse=True)
        members.sort(key=lambda x: len(x['body']), reverse=True)
    ...
    for item in prioritized_items:
        ...
        # Priority-based Pack Assignment
        if "LAUNCH" in tags or "URGENT" in tags: pack_key = "OPERATIONS_CRITICAL"
        elif "DOCTRINE" in tags or "RULESET" in tags: pack_key = "DOCTRINE_MAINTENANCE"
        elif "SYSTEM" in tags or "OPTIMIZATION" in tags: pack_key = "SYSTEM_OPS"
    ...
    if action_items:
        with open(action_queue_path, 'w') as f:
            ...
            for pack_name, items in work_packs.items():
                ...
                # EXECUTION STEPS (MC96 BUILDER x1000 - SMART PLAN)
                plan = generate_smart_plan(pack_name, items)
                f.write("\n**▶️ 3-STEP PLAN**\n")
                ...
                kill_list = generate_kill_list(pack_name, items)
