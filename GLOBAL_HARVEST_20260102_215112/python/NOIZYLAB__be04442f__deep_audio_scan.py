...
def deep_scan(root_dir, output_file): # Kept output_file to maintain syntactic correctness of the full file
    overlap = memory_core.analyze_temporal_overlap()
    vibe = overlap["overlap_status"]
    
    print(f"\n>>> [DEEP SCAN v3.0] INITIATING PROTOCOL ON: {root_dir}")
    print(f"    -> SYSTEM VIBE: {vibe}")
    memory_core.log_interaction(f"Deep Scan Init: {root_dir} [{vibe}]", "DEEP_SCAN_START", "ENGR")
