    ...
    while True:
        try:
            ...
            if result.returncode == 0:
                # Basic ping doesn't easily give ms in return code, but we can parse stdout if we didn't suppress it.
                sys.stdout.write(f"\r>>> STATUS: GREEN | SUPER-SONIC LINK ESTABLISHED | LATENCY: OPTIMIZED | STABLE")
                sys.stdout.flush()
                consecutive_fails = 0
            else:
