...
DB_PATH = "NOIZYLAB_DB"
CODE_VAULT_PATH = "mined_code/4TB_Blue_Fish"
...
# MODULE 4: CODE VAULT (NEW)
    ...
    if not code_files:
        st.info("VAULT EMPTY. MINER IS RUNNING...")
    else:
        st.metric("MINED FILES", len(code_files))
        selected_file = st.selectbox("SELECT EXTRACTED CODE", [str(f.relative_to(base_path)) for f in code_files])
        
        if selected_file:
            full_path = base_path / selected_file
            file_stats = full_path.stat()
            st.caption(f"SIZE: {file_stats.st_size} bytes | MODIFIED: {datetime.fromtimestamp(file_stats.st_mtime)}")
