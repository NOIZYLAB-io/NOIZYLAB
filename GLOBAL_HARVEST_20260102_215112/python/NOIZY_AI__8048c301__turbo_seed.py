...
def seed_database():
    ...
    for seed in SEEDS:
        try:
            ...
            # Insert into Plugins (Master List)
            c.execute("INSERT OR IGNORE INTO plugins (name, developer, type, last_seen) VALUES (?, ?, ?, ?)",
                      (seed['name'], seed['developer'], seed['type'], ts))
            
            # Insert into Files (Virtual Assets for Portal Search)
            # We map: filename=name, path=developer, size=0, bpm=0, key=type, tags="PLUGIN,GEM,GOD_MODE"
            c.execute("INSERT OR IGNORE INTO files (filename, path, size, bpm, key, tags, last_modified) VALUES (?, ?, ?, ?, ?, ?, ?)",
                      (seed['name'], f"/Applications/Plugins/{seed['developer']}/{seed['name']}", 0, 0, seed['type'], "PLUGIN,GEM,GOD_MODE", ts))
