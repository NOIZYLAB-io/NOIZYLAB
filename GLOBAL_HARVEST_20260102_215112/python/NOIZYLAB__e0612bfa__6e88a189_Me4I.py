...
# MODULE 4: CODE VAULT (Mining)
elif app_mode == "💎 CODE VAULT (Mining)":
    st.subheader("/// MINED ASSET VAULT")
    base_path = Path(CODE_VAULT_PATH)
    
    st.markdown("### 📡 GLOBAL EXTRACTION STATUS")
    
    # Scan for all volume folders
    if base_path.exists():
        vol_dirs = [d for d in base_path.iterdir() if d.is_dir()]
        vol_stats = []
        for v in vol_dirs:
            # Quick count (approximate for speed)
            files_in_vol = list(v.rglob("*"))
            count = len([f for f in files_in_vol if f.is_file()])
            size = sum(f.stat().st_size for f in files_in_vol if f.is_file()) / (1024*1024) # MB
            vol_stats.append({"name": v.name, "count": count, "size_mb": size})
        
        # Display Grid
        if vol_stats:
            cols = st.columns(4)
            for i, stats in enumerate(sorted(vol_stats, key=lambda x: x['count'], reverse=True)):
                with cols[i % 4]:
                    st.metric(
                        label=f"💿 {stats['name']}", 
                        value=f"{stats['count']} Files", 
                        delta=f"{stats['size_mb']:.1f} MB"
                    )
        else:
            st.info("Waiting for miners to report back...")
            
    st.divider()

    st.markdown("### 🕵️‍♂️ CODE INSPECTOR")
    if base_path.exists():
        # Flattened list of all code files across all volumes
        # Limit to 1000 newest for performance
        all_code_files = sorted(list(base_path.rglob("*.*")), key=os.path.getmtime, reverse=True)[:1000]
        
        if not all_code_files:
            st.warning("No assets found yet.")
        else:
            selected_file_path = st.selectbox(
                "SELECT RECENT ASSET", 
                options=[str(f) for f in all_code_files],
                format_func=lambda x: f"{Path(x).parent.name} / {Path(x).name}"
            )
            
            if selected_file_path:
                p = Path(selected_file_path)
                st.caption(f"📍 SOURCE: {p} | 📦 SIZE: {p.stat().st_size} bytes")
                try:
                    with open(p, "r", errors='ignore') as f:
                        st.code(f.read())
                except:
                    st.error("Binary or Unreadable Asset")
...
# MODULE 6: DREAMCHAMBER (PC LINK)
    ...
    with c2:
        if st.button("⚡ SEND WAKE-ON-LAN"):
            ...
            st.info("Triggering Magic Packet via Bridge...")
            # We could call a python snippet here or rely on the bridge auto-logic
            st.toast("Magic Packet Sent (Broadcast)")
