...
def check_cpu():
    ...
    try:
        if platform.system() == 'Darwin':  # macOS
            ...
            usage = int([x for x in output.split() if x.isdigit()][0])
        else:  # Linux
            output = subprocess.check_output("top -bn1 | grep 'Cpu(s)'", shell=True).decode()
            usage = int(float(output.split()[1].replace(',', '.')))
        
        return {'value': f'{usage}%', 'status': 'good' if usage < 70 else ('warning' if usage < 90 else 'critical')}
...
def check_ram():
    ...
    try:
        if platform.system() == 'Darwin':
            ...
            parts = output.split()
            used = parts[1].replace('G', '').replace('M', '')
            # Get total RAM
            total = subprocess.check_output("sysctl -n hw.memsize", shell=True).decode()
            total_gb = int(total) // (1024**3)
            return {'value': f'{used}/{total_gb}GB', 'status': 'good'}
        elif platform.system() == 'Windows':
    ...
    try:
        if platform.system() == 'Windows':
            ...
            if disks:
                ...
                return {'value': main['used'], 'status': 'good' if pct < 70 else ('warning' if pct < 85 else 'critical'), 'all': disks}
        else:
            output = subprocess.check_output("df -h / | tail -1", shell=True).decode()
            parts = output.split()
            usage = parts[4]
            pct = int(usage.replace('%', ''))
            return {'value': usage, 'status': 'good' if pct < 70 else ('warning' if pct < 85 else 'critical')}
    except Exception as e:
        return {'value': '—', 'status': 'good', 'error': str(e)}
...
def check_network():
    ...
    try:
        if platform.system() == 'Windows':
            result = subprocess.run(['ping', '-n', '1', '-w', '1000', '8.8.8.8'], capture_output=True, timeout=3)
        else:
            result = subprocess.run(['ping', '-c', '1', '-t', '1', '8.8.8.8'], capture_output=True, timeout=3)
        
        return {'value': 'ONLINE', 'status': 'good'} if result.returncode == 0 else {'value': 'OFFLINE', 'status': 'critical'}
    except:
