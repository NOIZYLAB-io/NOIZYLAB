...
# MODULE: VISUAL VAULT
    ...
    st.subheader("/// VISUAL ASSET INDEX")
    ...
    if vis_db.exists():
        ...
        # SEARCH
        search_query = st.text_input("🔍 SEARCH ASSETS", placeholder="e.g. 'logo', 'background', 'screenshot'")
        
        # Stats
        total = conn.execute("SELECT COUNT(*) FROM images").fetchone()[0]
        st.caption(f"INDEXED ASSETS: {total:,}")
        
        if search_query:
            query = f"%{search_query}%"
            res = conn.execute("SELECT path, filename, volume FROM images WHERE filename LIKE ? LIMIT 50", (query,)).fetchall()
        else:
            res = conn.execute("SELECT path, filename, volume FROM images ORDER BY mtime DESC LIMIT 20").fetchall()
        ...
        if res:
            cols = st.columns(4)
            for i, (path, name, vol) in enumerate(res):
                with cols[i % 4]:
                    try:
                        st.image(path, caption=f"{name} ({vol})")
                        st.caption(path)
                    except:
                        st.error("Missing/Protected")
        else:
            st.info("No assets found matching query.")
            
    else:
        st.warning("Visual Index Building... Check back in a moment.")
    
    if st.button("RESCAN UNIVERSE"):
         subprocess.Popen(["python3", "GABRIEL_UNIFIED/core/visual_scanner.py"])
         st.toast("Deep Scan Initiated...")
