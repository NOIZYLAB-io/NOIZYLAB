...
export class UnifiedAIWorker {
    ...
    try {
      ...
      switch (request.type) {
        ...
        case 'graph':
          ...
          // Support both simple string queries and JSON objects for complex queries
          let query = graphPayload.query;
          // Type guard for overlap payload (uses a properly typed interface)
          interface OverlapRequest { type: 'overlap'; range: { start: number; end: number } }
          if (typeof graphPayload === 'object' && 'type' in graphPayload && (graphPayload as unknown as OverlapRequest).type === 'overlap') {
            const overlap = (graphPayload as unknown as OverlapRequest).range;
            query = `overlap:${overlap.start},${overlap.end}`;
          }
          data = await this.knowledgeGraph.queryOrUpdate(query);
          break;
        default:
  ...
  private async retrieveContext(userId: string): Promise<{ lastAgent: AgentPersona; userId: string }> {
    return Promise.resolve({ lastAgent: 'GENERAL', userId });
  }
