...
async function verifySynapses() {
    ...
    const patterns = patternResString.data as any[]; 
...
async function verifyTemporalAPI() {
    ...
    if (!Array.isArray(res.data)) throw new Error("Temporal API did not return an array");
    
    console.log(`- Found ${res.data.length} overlapping nodes via API.`);
    console.log("- PASS");
}
